...
\draw[>=stealth,->] (0,-.15)--(0,{\sy*\l+.25}) node[above]{\small $p_X$};
%
\draw[thick] ({-\sx},0)--(0,0);
\draw (\r,\r) arc (90:270:\r);
\draw[dotted] (0,0)--(0,{\sy*\l}) node[scale=.4]{$\bullet$};
\draw[thick,domain=0:\mx,samples=50,smooth] plot ({\x*\sx},{\sy*\l*exp(-\l*\x)});
%
\draw (0,{\l*\sy})--(-.1,{\l*\sy}) node[left,scale=.7]{$\lambda$};
\draw (0,0)--(0,-.1) node[below,scale=.7]{$0$};
\draw ({1/\l*\sy},0)--({1/\l*\sy},-.1) node[below,scale=.7]{$\displaystyle \frac1\lambda$};
\draw ({2/\l*\sy},0)--({2/\l*\sy},-.1) node[below,scale=.7]{$\displaystyle \frac2\lambda$};
%
...
%
% cumulativa
\draw[thick,domain=0:\mx,samples=50,smooth] (-.5,0)--(0,0) plot({\x*\sx},{(1-exp(-\l*\x))*\sy});
%
