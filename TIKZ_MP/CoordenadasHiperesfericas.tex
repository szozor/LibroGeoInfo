...
%
\pgfmathsetmacro\r{3.25} % rayo del circulo (d=2) y de la esfera (d=3)
%
%
...
%\pgfmathsetmacro\tp{55} % theta punto x
%\pgfmathsetmacro\pp{45} % phi punto x
\pgfmathsetmacro\tu{60} % theta_1 punto x
\pgfmathsetmacro\td{40} %theta_2 punto x
...
\draw ({\xpbi/2},{1.05*\ypbi/2}) node[above]{$\boldsymbol{r}$};
\draw[->,>=stealth,thick] ({\r/2},0)  arc (0:\tpbi:{\r/2});
\draw ({\r*cos(\tpbi/2)/2},{\r*sin(\tpbi/2)/2}) node[right] {$\boldsymbol{\theta_1}$};
%
...
\draw[thick,dashed] ({projx(\xp,\yp},{projy(\xp,\yp,\zp})
-- ({projx(\xp,\yp},{projy(\xp,\yp,0});
\draw[thick,dashed] (0,0)
-- ({projx(\xp,\yp},{projy(\xp,\yp,0});
%
%
% notacion del rayo
\draw ({projx(.45*\xp,.45*\yp},{projy(.45*\xp,.45*\yp,.45*\zp})
node[above] {$\boldsymbol{r}$};
%
%
% notacion del angulo theta_1
\pgfmathsetmacro{\proptu}{.525} % "rayo" para dibujar el angulo
\draw[->,>=stealth]
({projx(\proptu*\xp,0)},{projy(\proptu*\xp,0,0)}) to [bend right=20]
node[pos=0.4,below,scale=.75] {$\boldsymbol{\theta_1}$}
({projx(\proptu*\xp,\proptu*\yp)},{projy(\proptu*\xp,\proptu*\yp,0)});
%
%
% notacion del angulo theta_2
\pgfmathsetmacro{\propp}{.625} % "rayo" para dibujar el angulo
\draw[->,>=stealth]
({projx(\propp*\xp,\propp*\yp)},{projy(\propp*\xp,\propp*\yp,0)}) to [bend right=20]
node[pos=0.4,right,scale=.75] {$\boldsymbol{\theta_2}$}
({projx(\propp*\xp,\propp*\yp)},{projy(\propp*\xp,\propp*\yp,\propp*\zp)});
%
