...
estad\'istica
% termoestad\'istica,
%%%%%termost\'atica, ??? => Thermostatistique en francés, thermostatistics en inglés
la famosa f\'ormula $S = k_B \log W$~\cite{Bol96, Bol98, Jay65}.
...
\begin{definicion}[Entrop\'ia de Shannon]\label{def:SZ:Shannon}
  Sea $X$ una  variable aleatoria definida sobre una alfabeto  discreto $\X = \{
  x_1 , \ldots , x_\alpha \}$ de cardinal $\alpha = |\X| < + \infty$ finito. Sea
  $p_X$ la  distribuci\'on de probabilidad  de $X$, \ie  $ \forall \, x  \in \X,
  \quad  p_X(x) = \Pr[X  = x]$.   La entrop\'ia  de Shannon  de la  variable $X$
  est\'a definida por
  %
  \[
...
$H$ es el equivalente de la entrop\'ia de Gibbs en mec\'anica estad\'istica.
%termodin\'amica. %%%termost\'atica. ???
La letra $H$ viene del teorema-H debido a\ldots Ludwig Boltzmann~\cite{Jay65}.
...
\item\label{prop:SZ:positividad} {\it Positividad:} La entrop\'ia es acotada por
  ...
  \]
  %
  con igualdad si y solamente si existe un $x_0 \in \X$ tal que $p_X(x_0) = 1$ \
  y \  $p_X(x) = 0$ \ para  \ $x \ne x_0$,
  %
  \[
...
\item\label{prop:SZ:cotamaxima} {\it Maximalidad:}  La entrop\'ia es acotada por
  ...
  t_i \log  t_i + \mu  t_i)$.  Se obtiene  sencillamente que $\log t_i  = -\mu$,
  dando       la      distribuci\'on      uniforma.\newline       La      figura
  Fig.~\ref{fig:SZ:EntropiaBinaria} representa la entrop\'ia de un sistema a dos
  estados, de probabilidades \ $p$ \ y  \ $1-p$ \ (ley de Bernoulli de parametro
  ...
  invariancia bajo una permutaci\'on ($h(p) = H(p,1-p) = H(1-p,p) = h(1-p)$).
  %
  \begin{figure}[h!]
  %
  \begin{center}\input{TIKZ_SZ/EntropiaBinaria}\end{center}
  %
  \leyenda{Entrop\'ia binaria  (de una variable de Bernoulli)  $h(p) = H(p,1-p)$
...
\item\label{prop:SZ:expansabilidad} {\it Expansibilidad:}  A\~nadir un estado de
  ...
  \]
  %
  Esta propiedad es obvia, consecuencia  de $\displaystyle \lim_{t \to 0} t \log
  t = 0$.
%
...
\item\label{prop:SZ:recursividad} {\it Recursividad:} Juntar dos estados baja la
  ...
  por la  probabilidad de ocurrencia de  este conjunto de  estados, y vice-versa,
  \ie  sean \  $X$ \  definido  sobre \  $\X$ \  y  \ $\overline{X}$  \ sobre  \
  $\overline{\X}$,
  %
  \[
  \left\{  \begin{array}{l}\overline{\X} =  \{  x_1 ,  \ldots  , x_{\alpha-2}  ,
      ...
      p_{\overline{X}}(x_i)  =  p_X(x_i),  \quad  1  \le i  \le  \alpha-1  \quad
      \mbox{y}       \quad      p_{\overline{X}}(\overline{x}_{\alpha-1})      =
      p_X(x_{\alpha-1}) +
...
\item\label{prop:SZ:concavidad} {\it Concavidad:} La entrop\'ia es c\'oncava, en
  ...
  \pi_i = 1  \quad \mbox{and cualquier conjunto de  distribuciones} \quad \{ p_i
  \}_{i=1}^n,
  \]
  %
  \[
  H\left( \sum_{i=1}^n \pi_i p_i \right) \ge \sum_{i=1}^n \pi_i H(p_i)
  \]
  %
  Esta relaci\'on es  conocida como desigualdad de Jensen.   Es una consecuencia
  ...
  ejemplo   (probabilidad   $\pi_1   =   1   -  \pi_2$   de   elegir   el   dado
  izquierda).\newline
  %
  \begin{figure}[h!]
  %
  ...
  \leyenda{(a) $\phi(t)  = t \log t$ es  convexa: la curva es  siempre debajo de
    ...
    los  $\sum_i \pi_i  p_i(x)$ y  sumando sobre  los $x$  da la  desigualdad de
...
determinante de la matriz.  Recordando  que $p_X(x) = p_Y(g(x)) |\Jac_g(x)|$, se
obtiene
%
...
%
\begin{definicion}[Rearreglo sim\'etrico]
...
Una pregunta natural es de saber lo  que pasa en termino de mayorizaci\'on en el
