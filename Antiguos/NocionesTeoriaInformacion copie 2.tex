...
\begin{proof}
  ...
  mas  general  es  dada   por  Huffman~\cite{Huf52}  y  se  consigue  tambi\'en
  por parte en~\cite{Pig03}. Las etapas son
  %
  \begin{itemize}
  \item Notar  que si  $p_j > p_k$,  entonces $l^\opt(x_j) \le  l^\opt(x_j)$. De
    hecho, si \ $c^\opt$ \ es un c\'odigo  optimo, y \ $c$ \ un c\'odigo tal que
    \ $l(x_i) = l^\opt(x_i),  \: i \ne j , k$ \ y \  $l(x_j) = l^\opt(x_k) \: \&
    \: l(x_k) = l^\opt(x_j)$,  \ se obtiene \ $0 \le L(c)  - L^\opt = \sum_i p_i
    \left(l(x_i)  - l^\opt(x_i)  \right)  =  (p_j -  p_k)  \left( l^\opt(x_k)  -
      l^\opt(x_j) \right)$. \ La desigualda  viene de la positividad estricta de
    \ $p_j - p_k$.
  %
  \item Hay por lo menos dos simbolos de largo m\'aximo. De hecho, si habia solo
    un, este  tendr\'ia un nudo padre con  solo el como hoja.  Se podr\'ia poner
    este simbolo  sobre el nudo padre el  lugar del hijo, sin  perder el aspecto
    libre  de  prefijo  del  c\'odigo  as\'i   hecho,  y  ganar  un  dit  en  la
    c\'odificaci\'on.  Eso   ser\'ia  contradictorio  con   la  optimalidad  del
    c\'odigo.  Ademas, de la observaci\'on precediente, los simbolos asociados a
    hojas  de  largo  m\'aximo  son  necesariamente los  de  probabilidades  mas
    debiles.
  %
  \item Sea \ $m$ \ el numero de hojas de palabra c\'odigo de largo m\'aximo y \
    $m' =  \min(m,d)$. Cualquier permutaci\'on  de los simbolos  asociados estas
    hojas no cambia el aspecto libre  de prefijo ni la optimalidad. Entonces, se
    puede considerar el c\'odigo tal que los \ $m'$ \ simbolos de probabilidades
    mas debiles comparten el mismo nudo padre.
  %
  \item Sea \  $\X^{(2)} = \{ x^{(2)}_i \}_{i=1}^{n-m'+1}$ \  con \ $x^{(2)}_i =
    x_i,  \:  1  \le  i  \le  n-m'$  \  y  \  $x^{(2)}_{n-m'+1}  \equiv  \{  x_i
    \}_{i=n-m'+1}^n$ \ de probabilidades correspondientes (se ``plega'' las $m'$
    hojas en un  super-simbolo).  \ Sea \ $c^{\opt,(1)}$  \ el c\'odigo \'optimo
    correspondiente a  \ $\X$ \  y \ $c^{(2)}$  \ el c\'odigo ``padre''  sobre \
    $\X^{(2)}$  \  ($c^{\opt,(1)}$  quitando  el  \'ultimo dit  de  los  estados
    juntados, \ie  con la  raiz com\'un de  estos).  De  la misma manera,  sea \
    $c^{\opt,(2)}$ un  c\'odigo \'optimo sobre $\X^{(2)}$  \ y \  $c^{(1)}$ \ el
    que se  obtiene deplegando el  super-simbolo \ $x^{(2)}_{n-m'+1}$ \  en $m'$
    hojas. De \ $L^{\opt,(1)} = L\left(  c^{(2)} \right) + p_{n-m'+1} + \cdots +
    p_n$ \ (pasar de $\X^{(2)}$ a $\X$ se a\~nade un solo dit a los simbolos del
    super-simbolo) \ y \ $L\left(  c^{(1)} \right) = L^{\opt,(2)} + p_{n-m'+1} +
    \cdots +  p_n$ \ se obiene  \ $\Big( L^{\opt,(1)} -  L\left( c^{(1)} \right)
    \Big) +  \Big( L^{\opt,(2)} - L\left( c^{(2)}  \right) \Big) \: =  \: 0$.  \
    Cada termino entre parentesis siendo positivo, valen necesariamente cero (la
    suma  de terminos positivos  vale cero  si y  solo si  todos son  nulos). En
    conclusi\'on, \  $c^{(2)}$ \ padre de  \ $c^{\opt,(1)}$ \  queda \'optimo, \
    $c^{(2)} \equiv c^{\opt,(2)}$ y vice-versa.
  %
  \item  Por inducci\'on, se  pasa de  \ $c^{\opt,(k)}$  \ a  $c^{\opt,(k+1)}$ \
    juntando  (por lo  menos 2)  super-simbolos de  probabilidades  mas debiles,
    hasta tener un super-simobolo  tendiendo todos los estados, $\left| \X^{(K)}
    \right| = 1$, raiz del arbol.
  %
  \item  Deplegando  el  proceso,   saliendo  de  la  raiz,  $\X^{(K-1)}$  tiene
    necesariamente $d$ super-simbolos, \ie la  profundez 1 del arbol es llena en
    el sentido de que tiene solamente hojas y  prefijos. Si $q = 0$ ($n = d$) es
    obvio. Sino, \ $q > 0$ \ y  \ por lo menos un nudo, llamamoslo $\eta$, tiene
    un super-simbolo que no sea  un singleton/simbolo \ $x_i$.  Si quedar\'ia un
    nudo de la profundez 1 sin hijos (no prefijo, libre), se podr\'ia transferir
    un hijo del  nudo $\eta$ (\ie de la  profundez 2), al este nudo  vacio de la
    profundez 1,  sin cambiar  el aspecto libre  de prefijo del  nuevo c\'odigo,
    pero  ganando  un  dit/unidad  de  largo  para  el  nudo  transferido.   Eso
    disminuir\'ia el  largo promedio,  lo que es  contradictorio con  el aspecto
    \'optimo del  c\'odigo $c^{\opt,(K-1)}$.
  %
  \item Suponimos que  son $m \ge 1$ nudos de la  profundez 1 con super-simbolos
...
$\frac{L^{\opt,n}}{n}$ \ no es necesariamente decreciente con respeto a \ $n$. \SZ{Ejemplo/figure}
...
\SZ{Ssi no, otros codigo (ej.  aritmeco, no more entropico.  \cite{Gal78} }
