...
\hline
%
$\tau_\sigma[\cdot]$ & Permutaciones de indices seg\'un la permutaci\'on \
...
\Mat_{d,d}(\Kset) \tq \forall \: 1 \le j < i \le d, \: M_{i,j} = 0 \et \forall
...
\Mat_{d,d}(\Kset) \tq \forall \: 1 \le i < j \le d, \: M_{i,j} = 0 \et \forall
\: 1 \le i \le d, \: M_{i,i} \ge 0 \right\}$.\\[2.5mm]
\hline
%
...
\right\}$.\vspace{1mm}\newline ($\Unit_d(\Rset) \equiv \Ort_d(\Rset)$)\\[2.5mm]
\hline
%
% Se    notar\'a    que    estas    variedades   son    $\Sti_{d,d'}(\Rset)    =
% \Ort_d(\Rset)/\Ort_{d-d'}(\Rset)$}
$\Sti_{d,d'}(\Kset)$,\newline $d' \le d$\newline $\Kset = \Rset$ o $\Cset$ &
Variedad de Stiefel~\footnote{Ver~\cite{Sti35} para la el estudio de este
variedad, o~\cite{Ste51, Jam76} para estudios detallados de esta
variedad.}:\vspace{1mm}\newline $\Sti_{d,d'}(\Kset) = \left\{ Q \in
...
\hline
%
$\cdot^{\odot k}$ & exponente al la Schur o componente a componente \vspace{1mm}\newline,
 $\left( M^{\odot k} \right)_{i_1,\ldots,i_K} = M_{i_1,\ldots,i_K}^{\odot k}$.
\end{notation}

Para m\'as  detalles sobres  los tensores y  operaciones sobres estos,  se puede
referirse por ejemplo a~\cite[Cap.~4]{KosMan97}.
