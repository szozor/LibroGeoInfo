...
%
$\Mat_d(\Kset) \equiv \Kset^d$ & Espacio de vector \ $M = \protect\begin{bmatrix} M_1\\
...
\Mat_{d'_1,\ldots,d'_K}(\Kset)$ \  el producto  externo \ $A  \optimes B$ \  es el
...
\hline
%
$\Ort_d(\Rset)$ & Conjunto de matrices ortogonales (groupo
...
\hline
%
$\Sti_{d,d'}(\Kset)$, $d' \le d, \: \Kset = \Rset$ o $\Cset$ & Variedad de Stiefel~\cite{Sti35, Ste51, Jam76}:\vspace{1mm}\newline $\Sti_{d,d'}(\Kset) = \left\{ Q \in \Mat_{d,d'}(\Kset)
\tq Q^\dag Q = I \right\}$.\vspace{1mm}\newline Se notar\'a que
$\Sti_{d,d}(\Kset) = \Unit_d(\Kset)$ y $\Sti_{d,1}(\Rset) = \Sset_d$ (y la
esf\'era unitaria compleja en el caso complejo).\\[2.5mm] \hline
%
