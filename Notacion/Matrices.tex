...
$u \ne v$, $\sigma_{(u,v)}(i) = v \uno_{\{u\}}(i) + u \uno_{\{v\}}(i) + i
(1-\uno_{\{u,v\}}(i))$, se usa m\'as simplemente \ $\tau_{(u,v)} \equiv
...
\vdots & \ddots & \ddots &   0    \\[-.25mm]
   ...
   \protect\end{bmatrix}$\newline
%
Por convenci\'on, $I_0 \igualc [ \: ]$ matriz ``vacia'' 
%= \Diag(\uno)$
%\vspace{1mm}\newline (se saca el \'indice cuando no hay ambiguedad)
\\[2.5mm]
\hline
%
$\uno$ & Vector de componentes iguales a 1, \ $\uno = \protect\begin{bmatrix} 1\\
\vdots \\ 1 \protect\end{bmatrix}$.\vspace{1mm}\\[2.5mm]
\hline
%
$\uno_i$ & Vector de componentes $j$-\'esima iguales a $\uno_{\{i\}}(j)$\\[2.5mm]
\hline
%
...
\Sigma = \protect\begin{bmatrix} \uno_{\sigma(1)} & \cdots &
\uno_{\sigma(d)} \end{bmatrix}^t\protect = \sum_{i=1}^d \uno_i \uno_{\sigma(i)}^t,
...
%       \uno_i   \right\}_{i=1}^k$}}    \SZ{de   \   $Rset_+^k$,   i.e.,}\newline
...
%$\Diag(V) = \sum_{i=1}^d \left( \uno_i \uno_i^t \right) \otimes V_i =
%\protect\begin{bmatrix}
%  V_1  &   0    & \cdots &    0   \\
%   0   & \ddots & \ddots & \vdots \\
%\vdots & \ddots & \ddots &    0   \\
...
%\ $\Mat_{d^2,d^2}(\Kset)$ \ de bloques \ $J_{i,j} = \uno_j \uno_i^t \in
%\Mat_{d,d}(\Kset)$ \ poniendo la componente (linea) \ $i$-\'esima de un vector
%(matriz) \ en la componente (linea) \ $j$-\'esima,\vspace{1mm}\newline
%%
%$ \displaystyle J  = \sum_{i,j=1}^d \left( \uno_i \uno_j^t  \right) \otimes \left(
%  \uno_j \uno_i^t \right) =
%\protect\begin{bmatrix}
%\uno_1 \uno_1^t & \uno_2 \uno_1^t & \cdots & \uno_d \uno_1^t\\
%    \vdots    &    \vdots     & \cdots &     \vdots   \\
%\uno_1 \uno_d^t & \uno_2 \uno_d^t & \cdots & \uno_d \uno_d^t
%\protect\end{bmatrix}$.\vspace{1mm}\\[2.5mm]
%\hline
%%
%$K$ & Matriz de no-comutaci\'on~\footref{Foot:MP:ComutacionMatriz} de \
%$\Mat_{d^2,d^2}(\Kset)$ \ de bloques \ $K_{i,j} = \uno_i \uno_j^t \in \Mat_{d,d}(\Kset)$
%\ poniendo la componente (linea) \ $j$-\'esima de un vector (matriz) \ en la
%posici\'on (linea) \ $i$-\'esima,\vspace{1mm}\newline
%%
%$ \displaystyle K  = \sum_{i,j=1}^d \left( \uno_i \uno_j^t  \right) \otimes \left(
%  \uno_i \uno_j^t \right)
%\protect\begin{bmatrix}
%\uno_1 \uno_1^t & \uno_1 \uno_2^t & \cdots & \uno_1 \uno_d^t\\
%   \vdots     &    \vdots    & \cdots &     \vdots   \\
%\uno_d \uno_1^t & \uno_d \uno_2^t & \cdots & \uno_d \uno_d^t
%\protect\end{bmatrix}$.\vspace{1mm}\\[2.5mm]
%\hline
%
...
\protect\begin{bmatrix} I_{j-1} & \uno_i & 0\\ 0 & 0 &
