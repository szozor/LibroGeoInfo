...
\item   Si  la   medida  \   $P_{\widetilde{Z}}$  \   admite  una   derivada  de
  ...
  de  Wirtinger~\cite{Rem91}~\footnote{Hay que entender  $z_i =  x_i +  \imath \
    ...
    \imath \, \frac{\partial}{\partial z_i} - \imath \, \frac{\partial}{\partial
      z_i^*}$.
    % Eso  es  conocido como  respectivamente  $\Rset$-derivativa and  conjugada
    % $\Rset$-derivada~\cite{Kre07}, y e
    ...
    % o $\Cset\Rset$-calculo~\cite{Kre07}.
    De hecho,  fue introducido antes , por  lo menos desde H.  Poincar\'e en los
    a\~nos 1899~\cite{Poi99}.\label{Foot:MP:Wirtinger}
    % K. Kreutz-Delgado, "The complex gradient operator and the CR-calculus,"
    % Lecture Notes Supplement [online], 2007, http://dsp.ucsd.edu/~kreutz/
    % The  Complex Gradient Operator  and the  CR Calculus.
    % Kreutz-Delgado, K.,
    % Arxiv Report, ArXiv:0906.4835v1, June 2009
}
  %
  \[
...
simetria particular).  En este marco  general, $X$ viviendo sobre  $\X \subset
\Mat_{d,d'}(\Rset)$:
%
\begin{itemize}
...
En  el contexto  sim\'etrico,  \ie el  espacio  de llegada  es  \ $\X  \subset
\Sim_d(\Rset)$ (ej.   el cono $\Pos_d^+(\Rset)$), aparece  que por lo  m\'as la matriz
...
$\omega \in  \Sim_d(\Rset)$ para  respectar las simetrias  del problema y  no tener
ninguna degenerencia  de esa misma (ver por  ejemplo~\cite{PedRic91, And03}).  A
veces, es a\'un  dificil o imposible calcular en  todo \ $\Mat_{d,d}(\Rset)$. Eso
tiene consecuencias:
%
\begin{itemize}
