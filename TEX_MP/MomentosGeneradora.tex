...
\end{definicion}
%
La secunda formulaci\'on se proba  sencillamente, empezando por $X = \un_A$ para
unos $A$. Entonces $P_X =  (1-P(A)) \delta_{\{0\}} + P(A) \delta_{\{1\}}$. Luego
$\displaystyle \int_\Omega \un_A(\omega) dP(\omega) = P(A) = (1-P(A)) \times 0 +
P(A)  \times  1   =  \int_\Rset  x  dP_X(x)$.   Se  cierra   la  prueba  con  el
...
En el caso  de vectores aleatorios continuo, obtenemos  la expresi\'on siguiente
...
ejemplo pagina~\ref{Ej:MP:ProbaMixta}, $X = V  \, \un_{U < \frac12} + \un_{U \ge
...
+  \frac12 \delta_{\{1\}}(x)$.   A  continuaci\'on, \  $\displaystyle \Esp[X]  =
\frac12  \int_\Rset dP_V(x)  + \frac12  \int_\Rset d\delta_{\{1\}}(x)  = \frac12
...
probar directamente  saliendo de~\footnote{Hay que  tener precauciones separando
  la series  de una diferencia de  terminos. En el caso  $X$ continuo admitiendo
  una densidad, el resultado se obtiene tambi\'en haciendo una integraci\'on por
  partes,  pero  hay que  mirar  bien el  comportamiento  de  $t (1-F_X(t))$  al
  infinito.} $P(X = n) = P(X>n)- P(X>n-1)$.
...
\modif{Volviendo al ejemplo de la pagina~\pageref{Ej:MP:ProbaMixta}, tratando de
  la media de \ $X = V \, \un_{U < \frac12} + \un_{U \ge \frac12}$ \ con \ $U$ \
  y \ $V$ \ variables independientes  de distribuci\'on uniformas sobre $(0 \, ;
  \, 1)$,  se calcula gracia  a la  linealidad y a  la indepencia, \  $\Esp[X] =
  \Esp[V] \Esp[\un_{U <  \frac12}] + \Esp[\un_{U \ge \frac12}]  = \frac12 \times
  \frac12  + \frac12 =  \frac34$ \  como lo  hemos obtenido  usando $P_X$  en la
  pagina~\pageref{Ej:MP:ProbaMixta}.}
