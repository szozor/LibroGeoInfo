...
\item Caen en  la familia exponencial la leyes: de  Bernoulli, binomial cuando \
  ...
  $n$ \  es fijo,  geometrica, de poisson,  gausiana~\footnote{En este  caso, se
    puede ver el par\'ametro como \  $\left( m , \Sigma^{-1} \right)$ \ en lugar
    del vector formado  de los \ $m_i$ \ y  \ $\left( \Sigma^{-1} \right)_{i,j},
    \:  i \le  j$ \  y la  estad\'istica suficiente  como \  $\left( x  ,  x x^t
    \right)$ \ siendo $x^t \Sigma^{-1} x = \Tr\left( \Sigma^{-1} x x^t \right)$,
    en lugar del vector formado  de los \ $x_i$ \ y \ $x_i x_j,  \: i \le j$ (de
    la   simetr\'ia).   El   orden  es   $d  +   \frac{d   (d+1)}{2}$.},  gamma,
  Wishart~\footnote{De  nuevo,  en  este   caso  se  puede  ver  el  par\'ametro
    formalmente  como \  $\left(  \nu ,  V^{-1}  \right)$ \  y la  estad\'istica
    suficiente como \  $\left( \log |x| ,  x \right)$. El orden es  $1 + \frac{d
      (d+1)}{2}$.}, beta, de Dirichlet
%
% resp. eta = log(p/(1-p)), log(p/(1-p)), log p, log p = [log p_1 ... \log p_k], log(1-p), log lambda
%       (m,Sigma^{-1}), [a b], (nu,V^{-1}), [a b], a = [a_1 ... a_k]
% resp. S = x, x, x, x = [x_1 ... x_k], x, x
%       (x,x x^t), [log x -x], (log |x| , x), [log x  log(1-x)], [log x_1 ... log(x_k)]
%
...
\begin{ejemplo}
  ...
  m\'as  exactamente  de  Edwards-Anderson~\cite{EdwAnd75,  LanLif80,  MezMon09,
    ...
    homenaje  a los trabajos  de P.  Curie~\cite{Cur95} y  P. Weiss~\cite{Wei96,
      Wei07}  quien  trabajaron   sobre  los  materiales  ferromagneticos.}   se
  presenta  de  la misma  manera,  con la  energ\'ia  $\displaystyle  \E(x) =  -
  \frac{1}{d}  \sum_{(i,j)   \mbox{\tiny  pares}}  x_i  x_j   +  B  \sum_{i=1}^d
  x_i$~\cite{MezMon09, Mer10, Mer18}.
  ...
  energ\'ia libre~\footnote{Se  usa la letra \ $S$  \ que la primera  que se uso
    historicamente. Obviamente  no corresponde a la estadica  sufficiente que es
    ac\'a $\E$.},
  %
  \[
...
\SZ{A ver~\cite{Bol96, Bol98, Jay65,
  Mer10, Mer18, Gib02, CovTho06,
      Rio07, Max67}}
