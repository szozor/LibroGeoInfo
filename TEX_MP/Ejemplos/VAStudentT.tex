...
%\uno_j^t \right) + \left( \uno_i % \uno_j^t \right) \otimes \left( \uno_i \uno_j^t
%\right) + \left( \uno_i \uno_j^t % \right) \otimes \left( \uno_j \uno_i^t \right) \!
%\Big)$
}\newline si \ $\nu >
...
Nota: nuevamente se puede escribir $X \, \iguald \, \Sigma^{\frac12} S + m$ \ con
...
% Bie53, p.   173
%
% geom\`etre connu mentionn\'e par Fermat, sans doute Antoine de Lalouv\`ere, un
% Jesuite de Toulouse
%
% Agnesi, ver tambien Tomo I, esempio III, p. 361-363, fig. 129
%                     Tomo II, Libro III, Capop I 26., p. 638-640, fig. 4
%
...
Poi24}~\footnote{M\'as probablemenre, esta distribuci\'on es debido a M. Poisson
que la estudi\'o  en 1824~\cite{Poi24}. M\'as all\'a, la forma  de esta curva es
conocida  desde  1659  o  a\'un antes  por  P.   de  Fermat~\cite{Fer1679:44p55}
o~\cite{TanHen96:216}, estudiada  luego por  G.  Grandi~\cite{Gra1703}  en 1703,
I. Newton~\cite{New1704} (llamada de  sexag\'esima terceras\'eptima especie, ver
p.~155   de  la   secunda  parte,   y   figura~67.),  el   seguidor  de   Newton
J. Stirling~\cite{Sti1717, Sti1797} (llamada  de sexag\'esima s\'eptima especie,
p.~116) y M.   G. Agnesi~\cite{Agn1748:1:380,Agn1748:2:561} en 1748.  De esa, es
conocida  tambi\'en como  ``brujula de  Agnesi'' por  error de  traducci\'on del
italiano de ``versiera'' (del verbo lat\'in girar) al ingl\'es por J.  Colson.}.
Es      un      caso       particular      tambi\'en      de      distribuci\'on
$\alpha$-estables~\cite{SamTaq94}.  En  particular, una combinaci\'on  lineal de
variables de  Cauchy independientes queda de  Cauchy. Pero, no viola  el teorema
del  l\'imite  central  del hecho  de  que  una  variable  de Cauchy  no  admite
covarianza.
...
escribiendo \ $A X + b \iguald  \sqrt{\frac{\nu}{V}} A \Sigma^{\frac12} G + A m +
b$.
...
lema~\ref{Lem:MP:StabilidadLinealStudentT} escribiendo \ $A X + b \iguald \frac{A
...
\begin{proof}
  ...
  %
  \[
  \overline{\widetilde{X}} \equiv \overline{X} - m = \frac{1}{n} \, \widetilde{X} \uno
  \]
  %
  con \ $\uno \in  \Rset^n$ \ vector de componentes iguales a \  $1$ \ y vimos en
  la prueba del corolario~\ref{Cor:MP:WishartEstimacion} que
  %
  \[
  \overline{\Sigma} = \frac{1}{n-1} \widetilde{X} \left( I - \frac{\uno \uno^t}{n}
  \right) \widetilde{X}^t
  \]
  %
  $A = I - \frac{\uno \uno^t}{n} \in  \Pos_n(\Rset)$ \ es idemponenta de rango 1, con
  $A  \uno = 0$,  as\'i que  por diagonalizaci\'on~\cite{HorJoh13,  Bha97, Bha07}
  tenemos
  %
  \[
  A = P \begin{bmatrix} I_{n-1} & 0\\ 0 & 0 \end{bmatrix} P^t \qquad \mbox{con} \qquad
  P = \begin{bmatrix} B & \frac{1}{\sqrt{n}} \uno \end{bmatrix}
  \]
  %
  $P P^t = P P^t = I$ \ y
  %
