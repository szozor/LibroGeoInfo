...
caso matriz-variada $X$  definido sobre $\Mat_{d,d'}(\Rset)$; se denota  \ $X \,
\sim \, \N(m,\Sigma,\Sigma')$ \ donde \ $m \in \Mat_{d,d'}(\Rset), \: \Sigma \in
\Pos_d^+(\Rset), \:  \Sigma' \in \Pos_{d'}^+(\Rset)$  y la densidad es  dada por
$\displaystyle  p_X(x)  = \frac{1}{(2  \pi)^{\frac{d  d'}{2}}  \, \left|  \Sigma
  \right|^{\frac{d'}{2}}  \left| \Sigma'  \right|^{\frac{d}{2}}}  \: e^{-\frac12
  \Tr \left(  (x-m)^t \Sigma^{-1} (x-m) \Sigma'^{-1} \right)}$.   Se refiera por
ejemplo a~\cite{GupNag99,  KotNad04, Daw81}.  En particular,  se generalizan los
teoremas~\ref{Teo:MP:StabilidadGaussiana}, \ref{Teo:MP:IndependenciaGaussiana} y
\ref{Teo:MP:CLT}.
