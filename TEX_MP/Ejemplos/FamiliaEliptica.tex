...
%
  ...
  \item  El  teorema~\ref{Teo:MP:MomentosCumulantesEliptica}  da, con  el  mismo
    ...
    \Nset \setminus \{ (0,0) \}$ \ tenemos \ $\zeta_{i_1 , \ldots , i_l ; i'_1 ,
      \ldots , i'_n}[Z] = \widetilde{\alpha}_k(\varphi_X) \T_{i_1 , \ldots , i_l
      ; i'_1 , \ldots , i'_n}(\Sigma)$ \  y \ $\kappa_{i_1 , \ldots , i_l ; i'_1
      , \ldots , i'_n}[Z] =  \widetilde{\alpha}_k(\psi_X) \T_{i_1 , \ldots , i_l
      ; i'_1 , \ldots , i'_n}(\Sigma)$ \ con \ $\widetilde{\alpha}_k(f) = (-4)^k
    f^{(k)}(0)$ \ y \  $ \displaystyle \T_{i_1 , \ldots , i_l  ; i'_1 , \ldots ,
      i'_n}(\Sigma)  =  \sum_{(\pi,\pi') \in  \Pi_{2  l,2}  \times \Pi_{2  l,2}}
    \prod_{(r,s)  \in \pi  \cup \pi'}  \Sigma_{i_r,i_s}$, \  los  otros momentos
    centrales  y  cumulantes  de  orden  mayor   o  igual  a  2  siendo  0  (ver
    tambi\'en~\cite{Kri76}).
%      corrolario~\ref{Cor:MP:MediaCovarianzaEliptica}
  %
...
\begin{lema}
  ...
  %
  \item\label{Enum:MP:EDM_Simetrica}  $\ED_\simed \cap \left( \ED_\izq \cup \ED_\der \right) = \emptyset$
...
%
\begin{itemize}
\item Sea $X$ de densidad de probabilidad $p_X(x) \propto e^{- \| x^t x \|_q}$ con $q \ne 2$. Del hecho de que $\| A B\|_q \ne \| B A \|_q \ne \|A\|_q \|B\|_q$, claramente $X \in \ED_\izq$ pero $X \ne \ED_\der$ y tampoco...uy \SZ{Buscar un que es izquierda,  pero no bilateral y no columna: $\ED_\izq
    \setminus \left( \ED_\bil \cap \ED_\col \right) \neq \emptyset$.}
% Veremos m\'as adelante en el ejemplo~\ref{Ej:MP} que $X  \sim
%    \U\left( \Sti_{d,d'}(\Rset)  \right)$ uniforme sobre la  variedad de Stiefel
%    (ver notaciones y~\cite[p.~67-70]{Mui82}) partenece a $\ED_\izq$ pero no 
%
...
\item $X \in \ED_\simed$ si y solamente si
  ...
  los casos \  $\bullet = \izq, \, \col, \, \vcol,  \simed$ \ respectivamente, y
  en el caso bilateral, \ $X \sim \ED_\bil(m,\Sigma,\Sigma',\varphi_X)$.
%
\item Igualmente,  como en  el contexto vectorial,  cuando existe,  tambi\'en la
  ...
  p_X(x)  = \left|  \Sigma \right|^{-\frac{d-1}{2}}  \,  d_X\left( \lambda\left(
      \Sigma^{-1} (x-m)  \right)\right) \qquad \mbox{con}  \qquad d_X: \Rset_+^d
  \mapsto \Rset_+
  \]
  \end{itemize}
  %
  ...
  \,  \vcol, \simed$  \  respectivamente, y  en  el caso  bilateral,  \ $X  \sim
  \ED_\bil(m,\Sigma,\Sigma',d_X)$.
  %
%
\item La  extensi\'on del teorema~\ref{Teo:MP:MomentosCumulantesEliptica}  y del
  corrolario~\ref{Lem:MP:MediaCovarianzaEliptica}  no   es  una  tarea  sencilla
  porque $\varphi_X$ no queda una  funci\'on escalar en general. Sin embargo, se
  puede   concentrarnos  en   los  momentos   de  orden   1  y   2,   los  m\'as
  usados~\cite{Mal61, Daw81,  Tyl82, GruRoc90, BilBre99, GupNag99}.  En los item
  que  siguen, notaremos  $Y$ l  matriz  a simetr\'ia  esf\'erica de  generadora
  caracteristica \ $\varphi_X$.
  %
  \begin{itemize}
  \item  Para $Y$  a sim\'etria  esf\'erica izquierda,  de $Y  \egald O  Y$ para
    cualquier matriz ortogonal, se obtiene tambi\'en  $- Y \egald Y$, de lo cual
    se deduce $\Esp[Y]  = 0$. Lo mismo occure con el  context derecho, as\'i que
    para  cualquier  subcaso (ver  figura~\ref{Fig:MP:EDMatrix}):  para $X  \sim
    \ED_\bullet(m,\Sigma,\varphi_X), \quad  \bullet =  \izq, \col, \vcol$,  \ $X
    \sim \ED_\bullet(m,\Sigma',\varphi_X), \quad \bullet  = \der, \lin, \vlin$ \
    o \ $X \sim \ED_\bil(m,\Sigma,\Sigma',\varphi_X)$, $$\Esp[X] = m$$
  %
  \item En el  caso $\ED_\simed$ esf\'erico, $Y  \egald O Y O^t$ \  se obtiene \
    $\Esp[Y]  =  O \Esp[Y]  O^t$  \ para  cualquier  matriz  ortogonal. De  esta
    identidad, viene que si \ $v$ \ es un autovector de \ $\Esp[Y]$ \ asociado a
    un autovalor $\mu_1$, cualquier \ $O v$  \ va a ser autovector asociado a la
    misma autovalor. De \ $\{ O v  \tq O \in \Ort_d(\Rset) \} = \Rset^d$ \ queda
    claro para cualquier  \ $v \in \Rset^d$, \quad $\Esp[Y] v  = \mu_1 v$, dando
    que existe un \ $\mu_1$ \ tal que \ $\Esp[Y] = \mu_1 I$.  Tommando la traza,
    se   obtiene  $\mu_1   =  \frac{1}{d}   \Esp[\Tr(Y)]  =   \frac{1}{d}  \un^t
    \Esp[\lambda(Y)]$.      En     conclusi\'on,      para     \     $X     \sim
    \ED_\simed(m,\Sigma,\varphi_X)$,  $$  \Esp[X]  =  m +  \mu_1  \Sigma  \qquad
    \mbox{con}  \qquad  \mu_1  =  \frac{1}{d} \un^t  \Esp\left[  \lambda  \left(
        \Sigma^{-1} (X-m) \right) \right] $$ \SZ{Ver para obtener el coefficient
      via $\nabla_\lambda \varphi_X(0)$ si es verdadero.}
  %
\item \SZ{Covarianza caso bil...}
  %
\item En el caso esf\'erico  \ $Y \sim \ED_\simed(0,I,\varphi_Y)$, escribimos la
  covarianza  bajo la  forma  $\displaystyle \sum_{i,j,k,l=1}^d  \T_{i,j,k,l}[Y]
  \un_i \otimes  \un_j \otimes \un_k \otimes  \un_l$ \ con  \ $\T_{i,j,k,l}[Y] =
  \Esp\left[  (Y-\mu_1  I)_{i,j}  (Y-\mu_1  I)_{k,l}\right]$.   Luego,  usaremos
  varias  veces para  cualquier  matriz $A  \in  \Mat_{d,d}(\Rset)$ la  relation
  (i) $$  \T_{i,i,k,l}[ A  Y A^t] =  \sum_{r,s,u,v=1}^d A_{i,r}  A_{j,s} A_{k,u}
  A_{l,v} \ \T_{r,s,u,v}[Y].  $$ Ahora, si se considera las matrices ortogonales
  \ $O_\varepsilon  = \Diag(\varepsilon)$ \ con  \ $\varepsilon \in \{-1  \; + 1
  \}^d$ \ de $Y \egald O Y  O^t$ \ se obtiene sencillamente \ $\T_{i,j,k,l}[Y] =
  \varepsilon_i  \varepsilon_j   \varepsilon_k  \varepsilon_l  \T_{i,j,k,l}[Y]$.
  Jugando sobre los signos, queda claro que $\T_{i,j,k,l}[Y] \ne 0$ solamente si
%
  \begin{itemize}
  \item \ $i = j = k = l$, \ o
  \item \ $i = j \ne k = l$, \ o
  ...
  \item \ $i = l \ne j = k$.
  \end{itemize}
  % Eso simplifica  el tensor covarianza en la  forma $\displaystyle \sum_{i \ne
  %   j}^d \Big(  \T_{i,i,j,j}[Y] \, \un_i  \otimes \un_i \otimes  \un_j \otimes
  % \un_j + \T_{i,j,i,j}[Y] \, \un_i \otimes \un_j \otimes \un_i \otimes \un_j +
  % \T_{i,j,j,i}[Y] \,  \un_i \otimes \un_j \otimes \un_j  \otimes \un_i \Big) +
  % \sum_{i=1}^d  \T_{i,i,i,i}[Y] \, \un_i  \otimes \un_i \otimes  \un_i \otimes
  % \un_i$.
  A continuaci\'on,  si consideramos \ $O$  \ matriz que permuta  dos lineas, es
  obvio que  $\forall \: i \ne j \in  \{ 1 \;  \ldots \; d\}$,
%
  \begin{itemize}
  \item \ $\T_{i,i,i,i}[Y] = \sigma_0^2$ \ constante positiva (siento un momento
    de orden 2),
  \item \ $\T_{i,i,j,j}[Y] = \mu_2$ \ constante
  \item \ $\T_{i,j,i,j}[Y] = \sigma_1^2$ \ constante positiva (siento un momento
    de orden 2),
  \item \ $\T_{i,j,j,i}[Y] = \mu'_2$ \ constante.
  \end{itemize}
  % as\'i  que se puede re-escribir  la covarianza bajo  la forma $\displaystyle
  % \sum_{i,j}^d \Big( \mu_2 \,  \un_i \otimes \un_i \otimes \un_j \otimes \un_j
  % \, + \,  \sigma_1^2 \, \un_i \otimes \un_j \otimes \un_i  \otimes \un_j \, +
  %  \, \mu'_2  \,  \un_i \otimes  \un_j  \otimes \un_j  \otimes  \un_i \Big)  +
  % \sum_{i=1}^d \left(  \sigma_0^2 - \mu_2 - \sigma_1^2  - \mu'_2 \right) \un_i
  % \otimes \un_i \otimes \un_i \otimes \un_i$.  A continuaci\'on,
%
  % = \sum_{r,s,u,v=1}^d O_{i,r} O_{i,s} O_{j,u} O_{j,v} T_{r,s,u,v}
  Adem\'as, aplicando la relaci\'on (i)  para cualquier matriz ortogonal \ $O$,
  de $Y  \egald O  Y O^t$,  se obtiene para  $j \ne  i$, $\displaystyle  \mu_2 =
  \T_{i,i,j,j}[Y]  =  \mu_2 \sum_{r,u=1}^d  O_{i,r}  O_{i,r}  O_{j,u} O_{j,u}  +
  \sigma_1^2 \sum_{r,  u=1}^d O_{i,r} O_{i,u} O_{j,r} O_{j,u}  + \mu'_2 \sum_{r,
    u=1}^d  O_{i,r}  O_{i,u} O_{j,u}  O_{j,r}  +  \left(  \sigma_0^2 -  \mu_2  -
    \sigma_1^2 - \mu'_2 \right) \sum_{r=1}^d O_{i,r}^2 O_{j,r}^2$, lo que es, de
  la ortogonalidad de $O$, \ $\displaystyle  \mu_2 = \mu_2 + \left( \sigma_0^2 -
    \mu_2 -  \sigma_1^2 - \mu'_2  \right) \sum_{r=1}^d O_{i,r}^2  O_{j,r}^2$, es
  decir  $\sigma_0^2 = \mu_2 +  \sigma_1^2 +  \mu'_2 =  0$. Se  puede  sacar un
  p\'arametro  m\'as,  u\'as,  de   la  simetr\'ia  $(Y-\mu  I)_{i,j}  =  (Y-\mu
  I)_{j,y}$,  dando \ $\T_{i,j,i,j}[Y]  = \T_{i,j,j,i}[Y]$,  es decir  $\mu'_2 =
  \sigma_1^2$, \ie
  %
  \begin{itemize}
  \item \ $\mu'_2 = \sigma_1^2$ \ y
  \item  \ $\sigma_0^2  = \mu_2  + 2  \sigma_1^2$; \  pasando,  necesariamente \
    $\mu_2 + 2 \sigma_1^2 \ge 0$.
  \end{itemize}
  %
   En  conclusi\'on, $\forall \: i \ne j \in \{ 1 \; \ldots \; d \}$,
  %
  \begin{itemize}
  \item \ $\T_{i,i,i,i}[Y] = \mu_2 + 2 \sigma_1^2$
  \item \ $\T_{i,i,j,j}[Y]  = \mu_2$,
  \item \ $\T_{i,j,i,j}[Y] = \T_{i,j,j,i}[Y] = \sigma_1^2$,
  \item \ si no, $\T_{i,j,k,l}[Y] = 0$.
  \end{itemize}
  %
  Ahora, usando de nuevo la relaci\'on (i), con $A$ tal que $A A^t = \Sigma$, se
  obtiene  que  $\T_{i,j,k,l}[A  Y  A^t]  = \mu_2  \Sigma_{i,j}  \Sigma_{k,l}  +
  \sigma_1^2  \big(   \Sigma_{i,k}  \Sigma_{j,l}  +   \Sigma_{i,l}  \Sigma_{j,k}
  \big)$. Notamos que
  %
  \begin{itemize}
  \item   \   $\Sigma_{i,j}  \Sigma_{k,l}   =   \big(   \Sigma  \otimes   \Sigma
    \big)_{i,j,k,l}$\vspace{1.5mm}
  %
  \item \  $\Sigma_{i,l} \Sigma_{j,k} = \Sigma_{k,j} \Sigma_{i,l}  = \big( \un_i
    \un_k^t  \Sigma   \big)_{i,j}  \big(  \un_k   \un_i^t  \Sigma  \big)_{k,l}$,
    conduciendo  a\vspace{1mm}\newline $\Big( \big(  \un_i \un_k^t  \Sigma \big)
    \otimes  \big( \un_k  \un_i^t  \Sigma \big)  \Big)_{u,j,v,l} =  \Sigma_{i,l}
    \Sigma_{j,k} \un_{(i,k)}((u,v))$.
  %
  \item \ $\Sigma_{i,k} \Sigma_{j,l}  = \Sigma_{i,k} \Sigma_{l,j} = \big( \Sigma
    \, \un_k  \un_l^t \, \Sigma  \big)_{i,j}$, conduciendo a\vspace{1mm}\newline
    $\Big( \big(  \Sigma \, \un_k  \un_l^t \, \Sigma  \big) \! \otimes  \! \big(
    \un_k   \un_l^t   \big)    \Big)_{i,j,u,v}   =   \Sigma_{i,k}   \Sigma_{j,l}
    \un_{(k,l)}((u,v))$.
  \end{itemize}
  %
  En conclusi\'on, con cambio de  indices adecuados, se obtiene la covarianza de
  \ $X$ \ bajo la forma:
  %
  \[
  \Cov[X] = \mu_2  \, \Sigma \otimes \Sigma +  \sigma_1^2 \sum_{k,l} \Big( \big(
  \un_k \un_l^t  \, \Sigma \big) \otimes  \big( \un_l \un_k^t \,  \Sigma \big) +
  \big(  \Sigma \, \un_k  \un_l^t \,  \Sigma \big)  \otimes \big(  \un_k \un_l^t
  \big) \Big)
  \]
  % : de  $\displaystyle \sum_{i=1}^d \un_i \otimes  \un_i = I$, se  nota que la
  % covarianza tiene la estructura \ $\displaystyle \Cov[Y] = \mu_2 \, I \otimes
  % I  + \sigma_1^2  \sum_{i,j=1}^d  \left( \un_i  \otimes  \un_j \otimes  \un_i
  %   \otimes  \un_j   +  \un_i  \otimes  \un_j  \otimes   \un_j  \otimes  \un_i
  % \right)$. Ahora, usando de nuevo la relaci\'on (i), con $A$ tal que $A A^t =
  % \Sigma$,  se obtiene  que \  $\T_{i,j,k,l}[ A  Y A^t]  =  \mu_2 \Sigma_{i,j}
  % \Sigma_{k,l}  + \sigma_1^2  \big( \Sigma_{i,k}  \Sigma_{j,l}  + \Sigma_{i,l}
  % \Sigma_{j,k} \big)$.
  %
  Finalmente, se  notar\'a que  \ $\Var\left[  \Tr(Y) \right] =  \left( d^2  - d
  \right)  \mu_2 +  d  \left( \mu_2  +  2 \sigma_1^2  \right)$.  Similarmente  \
  $\Tr\left( \Esp\left[ (Y - \Esp[Y] )  (Y - \Esp[Y])^t \right] \right) = \left(
    d^2 -  d \right) \sigma_1^2  + d  \left( \mu_2 +  \sigma_1^2 \right) =  d \,
  \mu_2 + d^2 \sigma_1^2$.  Se notara  que \ $\Tr(Y) = \un^t \lambda(Y)$ \ dando
  $\Var[\Tr(Y)]  =  \un^t  \Cov[\lambda(Y)]  \un$ y por consecuencia
  %
  \[
  d^2 \, \mu_2 + 2 \, d \, \sigma_1^2 = \un^t \Cov\left[ \lambda(Y) \right] \un
  \]
  %
  Pasando,  de  la  positividad   de  la  covarianza,  necesariamente  $\mu_2  +
  \frac{2}{d} \,  \sigma_1^2 \ge 0$  \ (lo que  es m\'as restrictivo que  lo que
  obtuvimos unos pasos atr\'as).

  Similarmente, $  \Tr\left( \Esp\left[  (Y - \Esp[Y])  (Y -  \Esp[Y])^t \right]
  \right) = \Tr\left( \Esp[Y Y^t]  \right) - \Tr\left( \Esp[Y] \Esp[Y]^t \right)
  = \Esp\left[  \un^t \lambda\left( Y  Y^t \right) \right] -  \frac{1}{d} \left(
    \un^t  \Esp[   \lambda(Y)  ]  \right)^2   =  \Tr\left(  \Esp\left[\lambda(Y)
      \lambda(Y)^t \right] \right) -  \frac{1}{d} \un^t \Esp[ \lambda(Y) ] \Esp[
  \lambda(Y) ]^t \un = \Tr\left( \Cov\left[\lambda(Y) \right] + \Esp[\lambda(Y)]
    \Esp[ \lambda(Y)]^t \right) -  \frac{1}{d} \Tr\left(\Esp[ \lambda(Y) ] \Esp[
    \lambda(Y) ]^t  \un \un^t  \right) = \Tr\left(  \Cov\left[\lambda(Y) \right]
  \right) + \Esp[ \lambda(Y) ]^t \left(  I - \frac{1}{d} \un \un^t \right) \Esp[
  \lambda(Y) ]$ \ (donde  la secunda igualdad viene de la forma  de la media, la
  tercera de la simetria  de $Y$, la cuarta de $u^t v =  \Tr(v u^t)$ y la quinta
  de  $\Tr(A  B)  = \Tr(B  A)$.  En  conclusi\'on
  %
  \[
  d  \, \mu_2  + d^2  \, \sigma_1^2  = \Tr\left(  \Cov\left[  \lambda(Y) \right]
  \right) + \Esp[ \lambda(Y) ]^t \left(  I - \frac{1}{d} \un \un^t \right) \Esp[
  \lambda(Y) ]
  \]
  %
  y tabi\'en $\mu_2 + d \, \sigma_1^2  \ge 0$ \ (lo que es menos restrictivo que
  el v\'inculo anterior cuando $d \ge 2$).  Se notar\'a tambi\'en que \ $\Pi = I
  - \frac{1}{d} \un \un^t$ \ es una matriz de proyecci\'on sobre el ortogonal de
  $\un$. \SZ{Ver para obtener el coefficient via $\Hess_\lambda \varphi_X(0)$ si
    es verdadero.}
\end{itemize}
%
  \item El  Teorema~\ref{Teo:MP:TranformacionAfinEliptica} de conservaci\'on del
    ...
    matricial tambi\'en. En lo que  sigue, denotamos \ $A \in \Mat_{n,d}(\Rset),
    \: n \le d$ \ de rango lleno, $B \in \Mat_{d',n'}(\Rset), \: n' \le d'$ \ de
    rango lleno, $C \in \Mat_{n,n'}(\Rset)$.
  %
  \begin{itemize}
  \item Denotamos  por \ $\bullet$ \  los casos \ $(\izq),  (\col)$ y $(\vcol)$.
    Entonces\newline   $X   \,   \sim  \,   \ED_\bullet(m,\Sigma,\Phi_X)   \quad
    \Rightarrow \quad A X B + C \: \sim  \: \ED_\bullet(A m B + C , A \Sigma A^t
    , \varphi_{X B})$\newline  En particular, si $X$ es  a simetr\'ia esf\'erica
    de  uno de  estos  tipos, multiplicar  por $B$  a  su derecha  no cambia  la
    esfericidad.
  %
  \item $X \, \sim  \, \ED_\bil(m,\Sigma,\Sigma',\Phi_X) \quad \Rightarrow \quad
    A X B  + C \: \sim \:  \ED_\bil(A m B + C  , A \Sigma A^t , B^t  \Sigma' B ,
    \varphi_X)$.
  %
  \item $X \, \sim  \, \ED_\simed(m,\Sigma,\Phi_X) \quad \Rightarrow \quad
    A X A^t  + c \: \sim \:  \ED_\simed(A m A^t + C  , A \Sigma A^t,
    \varphi_X)$ donde $C = C^t$.
  %
%  \item \SZ{$X \, \sim \,  \ED_\simed(m,\Sigma,\Phi_X) \quad \Rightarrow \quad A X B
%    + C \: \sim \: \ED_\bil(A m B + C , A \Sigma A^t , B B^t , \varphi_X)$.}
  \end{itemize}
  %
  En particular,  si $X$  es a simetr\'ia  el\'iptica de tipos  $(\izq), (\col),
  (\vcol)$, cualquier componente es  un vector aleatorio a simetr\'ia el\'iptica
  del mismo tipo.
%
\item \SZ{Teorema~\ref{Teo:MP:ProyeccionComponentesEliptica} Proyeccion Componentes Eliptica en:}
% Sea \ $Z$, vector
...
\item El teorema  de mezcla uniforme~\ref{Teo:MP:MezclaUniforme} tambi\'en tiene
  ...
  \item $X \sim \ED_\col(m,\Sigma,\varphi_X)  \:\: \Leftrightarrow \:\: X \egald
    \Sigma^{\frac12}   \,  U_\col   \,R_\col  +   m$  \:\:   con   \:\:  $U_\col
    = \begin{bmatrix}  U_{\col,1} & \cdots & U_{\col,d'}  \end{bmatrix}$ tal que
    los $U_{\col,j} \sim \U\left( \Sset_d  \right)$ uniforme sobre la esfera son
  ...
  \item $X \sim \ED_\bil(m,\Sigma,\Sigma',\varphi_X) \:\: \Leftrightarrow \:\: X
    ...
    \Sigma'^{\frac12}   +    m$   \:\:   con   \:\:    $U_\izq   \sim   \U\left(
      \Sti_{d,d'}(\Rset)   \right)$   \:\:   y   \:\:  $V_\der   \sim   \U\left(
      \Sti_{d',d'}(\Rset)   \right)$,    independientes   e   independiente   de
    $\Lambda_\bil \ge  0$ matriz aleatoria diagonal  positiva.  Pasando, aparece
    que $\Lambda_\bil = \Diag(\lambda\left(  (X^t X)^{\frac12} \right))$, \ie de
    diagonal   el  vector  de   las  autovalores   de  $(X^t   X)^\frac12$  (ver
    tambi\'en~\cite[Rep.~2]{Daw78}).
%
  \item \SZ{ME TENGO QUE FIJAR BIEN... a\'un que sea seguro $X  \sim \ED_\simed(m,\Sigma,\varphi_X) \:\:  \Leftrightarrow \:\: X
      \egald \Sigma^{\frac12} \, U \, \Lambda_\simed \, U^t \ \Sigma^{\frac12} +
      m$ \:\: con  \:\: $U \sim \U\left( \Sti_{d,d}(\Rset)  \right)$ ($d' = d$),
      independiente  de   $\Lambda_\simed  \ge  0$   matriz  aleatoria  diagonal
      positiva.}
    % Se   lo   puede   probar   escribiendo  $Y   =   \Sigma^{-\frac12}   (X-m)
    % \Sigma^{-\frac12}$ bajo  la forma $Y  = \left( Q  \Delta^{\frac12} \right)
    % \left( Q \Delta^{\frac12} \right)^t$ y aplicando la escritura estochastica
    % del caso $(\izq)$ con $R
    Pasando, aparece que $\Lambda_\simed  = \Diag(\lambda\left( X \right))$, \ie
    de diagonal el vector de las autovalores de $X$.
      % (ver tambi\'en~\cite[Rep.~2]{Daw78}).
  \end{itemize}
  %
...
%
%\item \SZ{ El equivalente del lema~\ref{Lem:MP:AlphaConR} tratando de la media y covarianza  en el
%    caso       matricial da: para $X \sim \ED_\simed(m,\Sigma,\varphi_X) $
%
%         $\widetilde{\alpha}_k(\varphi_X)          =
%    \frac{\Gamma(d)}{\Gamma(d+k)} \Esp\left[ \left( (Z-m)^\dag \Sigma^{-1} (Z-m)
%      \right)^k \right]$.
%}
%
\item  Se  monstr\'o  que,  en  el  caso  $d'  =  1$  la  densidad  es  definida
  ...
  \item Si $X \sim \ED_\izq(m,\Sigma,\varphi_X)$, la lei de $X$ es completamente
    ...
    (ver~\cite{HorJoh13, Bha97}), esa define  completamente la lei de $X$. M\'as
  ...
  \item Si $X \sim \ED_\col(m,\Sigma,\varphi_X)$, la lei de $X$ es completamente
    definida  por  la  lei  del  vector $R_\col  =  \left(  \diag\left(  (X-m)^t
  ...
  \item  Si  $X \sim  \ED_\bil(m,\Sigma,\Sigma',\varphi_X)$  la  lei  de $X$  es
    completamente definida  por la lei  de $\lambda_\bil =  \left( \lambda\left(
         (X-m)^t  \Sigma^{-1} (X-m) \Sigma'^{-1} \right)
    \right)^{\odot  \frac12}$  donde,   \SZ{sin  perdida  de  generalidad,}  las
    autovalores  son tomadas  de valores  decrecientes  \SZ{(rearreglado; sufice
      poner la permutaci\'on  dentro del generador de densidad)}.  Si existe, la
   ...
   %
    ...
    (ver~\cite[p.~146]{FanChe84}~\footnote{Notar  que los  autores  omitieron el
      cuadrado    en    el    argumento    de   $d_X$.    De    hecho,    citant
      a~\cite[Lem.~13.3.1]{And03},   pero   este   lema   da  la   densidad   de
      $\Sigma'^{-\frac12} (X-m)^t \Sigma^{-1} (X-m) \Sigma'^{-\frac12}$. De esto
      viene el cuadrado,  argumento en $p_X$ y hace falta  combinar el lemma con
      el teorema~13.3.1 de~\cite{And03}.}).
  %
  \item   Si  $X  \sim   \ED_\simed(m,\Sigma,\varphi_X)$  la   lei  de   $X$  es
    completamente  definida  por  la  lei  de  $\lambda_\simed  =  \lambda\left(
      \Sigma^{-1}  (X-m) \right)$  donde, \SZ{sin  perdida de  generalidad,} las
    autovalores  son tomadas  de valores  decrecientes  \SZ{(rearreglado; sufice
      poner la permutaci\'on  dentro del generador de densidad)}.  Si existe, la
    densidad de probabilidad es dada por
   %
    ...
    d_X\left( \lambda  \right), \quad  \lambda \in \{  \mu \in  \Rset_+^{d'} \tq
    \mu_1 \ge \cdots \ge \mu_{d'} \}
    \]
    %
    (ver~\cite[Def.~2.5.1]{AndGui10} para el determinente de la transformaci\'on
    pasando de $X$ a los autovalores, o~\cite[Teo.~13.3.1]{And03}).
  \end{itemize}
  %
%
%  \item Teorema~\ref{Teo:MP:SchoenbergCaracteristica} en: \ $Z \sim \CED\left( m
...
%    \Leftrightarrow \:\: \varphi_Z  \in \PDSI$, \ con \ $A  > 0$ \ independiente
%    de \ $G \sim \CN(0,I)$;
%  % Distributions of the compound and scale mixture of vector and spherical matrix variate elliptical distributions  de Jose A.Diaz-Garcia Ramon Gutierrez-Jaimez JMVA 2003: 10.1016/j.jmva.2010.08.008
%
%
%  \item  Teorema~\ref{Teo:MP:SchoenbergDensidad} en:  \ $Z  \sim \CED\left(  m ,
%      \Sigma  , d_Z  \right)  \egald \,  A \,  \Sigma^{\frac12}  \, G  + m  \:\:
...
\SZ{PONER ESO?
% Sea $X = \begin{bmatrix} X_1 & \cdots & X_{d'} \end{bmatrix} \sim
%  \ED_\bullet(m ,  \Sigma , \varphi_X)$. Entonces  $X_i \sim \ED(m_i  , \Sigma ,
%  \varphi_{X_i})$  con $\varphi_{X_i}$  definido  a partir  de $\varphi_X$  para
%  $\omega =  \begin{bmatrix} \omega_1 & \cdots &  \omega_{d'} \end{bmatrix}$ tal
%  que $\omega_k = w \un_{\{i\}}(k)$. Adem\'as, p
Para $d' > 1$, \ $\left\{ X_i \tq
    X \sim \ED_\vcol(m , \Sigma ,  \varphi_X) \right\} \varsubsetneq \left\{ X_i \tq X
    \sim \ED_\col(m ,  \Sigma , \varphi_X) \right\} = \left\{  X_i \tq X \sim
    \ED_\izq(m , \Sigma , \varphi_X)  \right\}$ (para $d' = 1$ son obviamente
  todos iguales).}
...
\SZ{Que mas? La lei de $\lambda_\bil$ no tiene una forma que conozco...,; ver GSM...}
\end{ejemplo}
...
%
  ...
  e^{- \frac{d}{4}  \, \un^t  \lambda^{\odot 2}}$ \  (ver tambi\'en~\cite{Meh89,
    AndGui10}).
