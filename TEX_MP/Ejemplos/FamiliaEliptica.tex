...
%
  ...
  de misma ley de dimensi\'on m\'as grande), \ie $\omega \mapsto \varphi_X\left(
...
\cite[Teo.~1]{Kan94}).
%
\begin{teorema}[Sch{\oe}berg'38]
  Sea \ $X \sim  \ED\left( m , \Sigma , \varphi_X \right)$.  Entonces $X$ es una
  mezcla de  gausiana si y  solamente si $X$  es consistente, lo que  se escribe
  tambi\'en, con $Y = \sigma^{-\frac12} (X-m)$,
  %
  \[
  Y \, \egald \, A \, G \quad \Leftrightarrow \quad \Phi_Y \in \PD
  \]
  %
...
%
\begin{proof}
  La directa es inmediata de
  %
  \[
  \Phi_Y(\omega)  =  \Esp\left[  \Esp\left[  \Phi_G( A  \omega)  \left.  \right|
      A\right] \right]
  \]
  %
  conjuntamente a $\Phi_G \in \PD$  (eso es inmediato tambi\'en de $\varphi_G(w)
  = e^{-\frac{w}{2}}$).

  La reciproca es  m\'as dificil a probar. Para los  detalles, dejamos el lector
  a~\cite{Sch38}. Los  elementos de  prueba son los  siguiente. De  la escritura
  ...
  %
  \[
  \varphi_X\left(  w^2  \right)   =   \int_{\Rset_+}  2^{\frac{d}{2}-1}  \Gamma\left(  \frac{d}{2}
  \right) \left(  w   r  \sqrt{d}  \right)^{1-\frac{d}{2}}
  ...
  intercambiar limite e integral.  Dio una prueba Sch{\oe}nberg en~\cite{Sch38},
  y  luego  propuso  un  ``m\'as  moderna''  Steerneman  y  van  Perlo-ten-Kleij
  en~\cite{SteVan05}. Basicamente se reconoce en $2^{\frac{d}{2}-1} \Gamma\left(
    \frac{d}{2}   \right)   \left(    w   r   \sqrt{d}   \right)^{1-\frac{d}{2}}
  J_{\frac{d}{2}-1}\left( w r \sqrt{d} \right)$ la funci\'on caracter\'istica de
  $q_d(x)  \propto \left(  1  - \frac{x^2}{d}  \right)_+^{\frac{d-3}{2}}$ \  que
  tiende a  la gausiana (ver e.g. secci\'on~\ref{Sssec:MP:StudentR}).  Se usa el
  lema  de  Scheff\'e  o  de  Riez  (ver~\cite{Rie28,  Sch47,  Nov72,  Kus10}  o
  \cite{AthLah06, Bog07:v1, Bil12}) para probar la convergencia de la integral.
% (es similar al uso del teorema de convergencia dominada).
\end{proof}
%
Se encuentra una prueba alternativa tambi\'en en~\cite{FanKot90, Kin72}.
