...
\begin{definicion}[Vector esfericamente invariante]
  ...
  ortogonal (o de rotaci\'on)~\footnote{Recordarse que \ $O$ \ es ortogonal o de
    rotaci\'on si \ $O O^t = O^t O = I$.} \ $O \in \O_d(\Rset)$,
  %
  \[
  O  X  \: \egald  \: X
  \]
...
\begin{definicion}[Vector a simetr\'ia el\'iptica]
  ...
  P_d^+(\Rset)$ \ tal que para cualquier matriz ortogonal \ $O \in \O_d(\Rset)$,
  %
  \[
  ...
  $\Sigma$  \  y  \  $Q  \in  \O_d(\Rset)$  \  la  matriz  de  los  autovectores
...
\begin{proof}
  ...
  ortogonal \ $O \in \O_d(\Rset)$ \ y cualquier \ $\omega \in \Rset^d$
  %
  \[
  ...
  forma dada, para  cualquier matriz ortogonal \ $O  \in \O_d(\Rset)$ \ $\Phi_{O
...
\begin{proof}
  ...
  \]
  %
  Siendo $R_Y \in P_d^+(\Rset)$ \ se diagonaliza con $Q \in \O_d(\Rset)$ bajo la
  ...
  $R_Y$ \ y \ $Q \in \O_d(\Rset)$ \ los autovectores. Sean ahora
  %
  \[
...
\begin{proof}
  ...
  cualquier matriz ortogonal \ $O \in \O_d(\Rset)$ \ y cualquier \ $x$
  %
  \[
...
\begin{definicion}[Vector complejo a simetr\'ia el\'iptica]\label{Def:MP:ElipticoComplejo}
  ...
  P_d^+(\Cset)$ \ tal que para cualquier matriz unitaria \ $V \in \U_d(\Cset)$,
  %
  \[
  ...
  \  $\Sigma$  \ y  \  $Q  \in \U_d(\Cset)$  \  la  matriz  de los  autovectores
  correspondientes~\cite{Bha97,   Bha07,  HorJoh13},  \   $\Sigma  =   Q  \Delta
...
\begin{proof}
  ...
  conjuntamente a \ $e^{\imath \theta} I \in \U_d(\Cset)$.
\end{proof}
...
%
\item $M \in \U_d(\Cset) \quad \Leftrightarrow \quad \overline{M} \in \O_{2d}(\Rset)$.
\end{itemize}
%
%\[
...
\begin{proof}
  ...
  \overline{Q}^t$ \ con \ $\overline{\Delta}  = \begin{bmatrix} \Delta & 0\\ 0 &
    \Delta  \end{bmatrix}  \in P_{2d}^+(\Rset)$  \  diagonal, $\overline{Q}  \in
  \O_{2d}(\Rset)$.      La     prueba    se     cierra     saliendo    de     la
...
covarianza \ $\Sigma_WX$ \  es \ $\displaystyle \widehat{\Sigma}_X = \frac{1}{n}
\sum_{i=1}^n X_i X_i^t$,  matriz de $S_d(\Rset)$ (y a\'un  de $P_d(\Rset)$, y de
$P_d^+(\Rset)$ casi siempre si $n \le  d$). Ahora, si los $X_i$ son a sim\'etria
elipt\'ica, $W$ va  a tener simetr\'ia por cambio de  escala y rotaciones. M\'as
precisamente, si  se supone $X_i$  esf\'erica, para cualquier rotaci\'on  $O$ se
tiene   $O   X_i  \egald   X_i$,   dando   $O   \widehat{\Sigma}_X  O^t   \egald
\widehat{\Sigma}_X$.  Esta  definici\'on de  una matriz a  simetr\'ia esf\'erica
aparece por unas de las primeras veces en~\cite{Mal61}.Ahora, si uno quierre una
matriz  de covarianza  $\Sigma_{X,Y}$  con $X$  vector  $d$-dimensional e  $Y_i$
vector  $d'$-dimensional  a  partir  de  copias  independientes,  $\displaystyle
\widehat{\Sigma}_{X,Y}  =   \frac{1}{n}  \sum_{i=1}^n  X_i   Y_i^t$,  matriz  de
$\M_{d,d'}(\Rset)$. Se observa que cuando  los $X_i$ e $Y_i$ son esfericas, para
cualquier   par   de   rotaciones   $O$   de   $\M_{d,d}(\Rset)$   y   $O'$   de
$\M_{d',d'}(\Rset)$     se     tiene     $\widehat{\Sigma}_{X,Y}    \egald     O
...
\begin{definicion}[Matriz a simetr\'ia esf\'erica]
  Sea  $X$ matriz  aleatoria definida  sobre  $\X =  \M_{d,d'}(\Rset)$.  $X$  es
  dicha:
  %
  \begin{enumerate}%[label={\normalsize (\roman*)}]
  \item[(i)] a simetr\'ia esf\'erica a la  derecha si para cualquier matriz \ $O'
    \in \O_{d'}(\Rset)$ \ se tiene
    %
    \[
    X Q \: \egald \: X;
    \]
  ...
  \item[(ii)] A simetr\'ia esf\'erica a  la izquierda si para cualquier matriz \
    $O \in \O_d(\Rset)$ \ ortogonal se tiene
    %
    \[
    O X \: \egald \: X;
    \]
  ...
  \item[(iii)] A  simetr\'ia esf\'erica si para  cualquier par de  matrices \ $O
    \in \O_(\Rset)$ \ y \ $Q \in \O_{d'}(\Rset)$ \ se tiene
    %
    \[
    O X Q \: \egald \: X;
    \]
  %
  \item[(iv)]  cuando $d'  = d$  y $\X  = S_d(\Rset)$,  a  simetr\'ia esf\'erica
    fuerte si para cualquier matriz \ $O \in \O_d(\Rset)$ \ se tiene
    %
    \[
    O X O^t \: \egald \: X
    \]
...
%
\begin{definicion}[Vector a simetr\'ia el\'iptica]
  \SZ{RETOMAR A LA LUZ DE LA DEFINICION PRECEDIENTE
...
Se denotar\'a respectivamente \ $X \sim \DED(m,S,\varphi_X)$ \ en el caso (i), \ $X \sim \IED(m,\Sigma,\varphi_X)$ \ en el caso (ii), \ $X \sim \ED(m,\Sigma,S,\varphi_X)$ \ en el caso (iii), \ $X \sim \FED(m,\Sigma,\varphi_X)$ \ en el caso (iv).}
\end{definicion}
...
%
\[
\DED \, \cap \, \IED \: = \: \ED \quad \mbox{y, para } \: d'=d, \quad \ED \subset \FED
\]


\SZ{ADELANTAR covarianzas empiricas media cero, y sacando la media empirica en tal clase}
