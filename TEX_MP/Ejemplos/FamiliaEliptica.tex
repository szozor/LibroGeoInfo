...
Si el caract\'er el\'iptico se conserva por transformaci\'on lineal, no es el caso en general por adici\'on:
%
\begin{teorema}
...
\begin{proof}
  ...
  \]
  %
  Tomando $v =  \un_i$ se ve que  $R$ debe ser diagonal, $R  = \diag(r)$ \SZ{Ver
    bien si no  puede occurir un ``concurso de  circonstancia'' anulando cada lado
    de la  igualidad}.  A  continuaci\'on, para  cualquier $i =1  , \ldots  , d$
  tenemos
  %
  \[
  \varphi_X'\left( v^t v \right) \, \varphi_Y\left( \omega^t \Lambda_Y v \right)
...
o~\cite{Tyl82, GruRoc90,  GupVar94, GupVar95, FanLi99,  CarGon16}.  Tipicamente,
si  uno sale de  \ $X_1,  \ldots, X_n$  \ vectores  aleatorios $d$-dimensionales
independientes,  de misma ley,  y suponemos  la media  cero para  simplificar el
ejemplo, un  estimador natural de la matriz  de covarianza \ $\Sigma_WX$  \ es \
$\displaystyle \widehat{\Sigma}_X =  \frac{1}{n} \sum_{i=1}^n X_i X_i^t$, matriz
de $S_d(\Rset)$ (y a\'un de $P_d(\Rset)$, y de $P_d^+(\Rset)$ casi siempre si $n
\le  d$). Ahora,  si  los $X_i$  son a  sim\'etria  elipt\'ica, $W$  va a  tener
simetr\'ia por cambio  de escala y rotaciones. M\'as  precisamente, si se supone
$X_i$ esf\'erica,  para cualquier  rotaci\'on $O$ se  tiene $O X_i  \egald X_i$,
dando $O  \widehat{\Sigma}_X O^t \egald \widehat{\Sigma}_X$.  Es precisamente lo
que va a dar la noci\'on de matriz a simetr\'ia esf\'erica, y luego el\'iptica.

\begin{definicion}[Matriz esfericamente invariante]
