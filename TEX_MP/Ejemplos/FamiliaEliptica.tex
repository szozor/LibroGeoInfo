...
\item Como  en el  contexto vectorial, la  funci\'on caracter\'istica  tiene una
  ...
  %
\item \SZ{A VER... $X \in \ED_\simed$  si y solamente si 
  %
  \[
  \Phi_X(\omega) = e^{\imath \, \Tr\left( \omega^t m \right)} \, \varphi_x\left(
...
%
  \item \SZ{ME TENGO QUE FIJAR $X  \sim \ED_\simed(m,\Sigma,\varphi_X) \:\:  \Leftrightarrow \:\: X
      ...
      positiva.   Pasando, aparece que  $\Lambda_\simed =  \Diag(\lambda\left( X
      \right))$, \ie de diagonal el vector de las autovalores de $X$.
...
\item  Se  monstr\'o  que,  en  el  caso  $d'  =  1$  la  densidad  es  definida
  ...
  \item Si $X \sim \ED_\col(m,\Sigma,\varphi_X)$, la lei de $X$ es completamente
    ...
    p_{R_\col}(r)  = \frac{2  \pi^{\frac{d d'}{2}}}{\Gamma\left(  \frac{d d'}{2}
      \right)}  \: \prod_{i=1}^{d'}  |\Diag(r)|^{d-1} \,  d_X\left(  r^{\odot 2}
    \right), \quad r \in \Rset_+^{d'}
    \]
    %
  ...
  \item  Si  $X \sim  \ED_\bil(m,\Sigma,\Sigma',\varphi_X)$  la  lei  de $X$  es
    completamente definida  por la lei  de $\lambda_\bil =  \left( \lambda\left(
        \Sigma'^{-\frac12} (X-m)^t  \Sigma^{-1} (X-m) \Sigma'^{-\frac12} \right)
    \right)^{\odot \frac12}$ donde , sin perdida de generalidad, las autovalores
    son  tomadas   de  valores   decrecientes  (rearreglado;  sufice   poner  la
    permutaci\'on dentro del  generador de densidad). Si existe,  la densidad de
    probabilidad es dada por
   %
    \[
    \SZ{p_{\lambda_\bil}(\lambda)      =      \frac{\pi^{\frac{d'      (d+d')}{2}}}{
      \Gamma_{d'}\left(   \frac{d'}{2}  \right)   \Gamma_{d'}\left(  \frac{d}{2}
      \right)} \: \left| \Diag(\lambda) \right|^{\frac{d-d'-1}{2}} \left( \prod_{i < j}
      (\lambda_i-\lambda_j)  \right)  d_X\left(  \lambda^{\odot 2}
    \right), \quad \lambda \in \Rset_+^{d'}}%, \: \Lambda = \diag(\lambda)}
    \]
    %
    %donde  el  cuadrada en  $\lambda^2$  se  entiende  componente a  componente.
    (ver~\cite[p.~146]{FanChe84} o~\cite[Teo.~13.3.1]{And03}).
  %
  \item \SZ{Que pasa en el caso $(\simed)$?}
  \end{itemize}
  %
%
%  \item Teorema~\ref{Teo:MP:SchoenbergCaracteristica} en: \ $Z \sim \CED\left( m
...
%
  ...
  variada,       $\displaystyle        p_X(x)       =       \frac{\Gamma_d\left(
  ...
  secci\'on~\ref{Sssec:MP:StudentT}),  se   ve  inmediatamente  que   la  matriz
  aleatoria   partenece  a   $\ED_\bil$,  con   $d_X\left(  \lambda   \right)  =
  \displaystyle  \frac{\Gamma_d\left(  \frac{\nu+d+d'-1}{2}\right)}{\pi^{\frac{d
        d'}{2}}   \Gamma_d\left(  \frac{\nu+d-1}{2}\right)}   \:   \left|  I   +
    \frac{\Diag(\lambda)}{\nu}  \right|^{- \frac{\nu+d+d'-1}{2}}$ .   Se muestra
  que     no      satisface     al     la      extensi\'on     matricial     del
  corollario~\ref{Cor:MP:MezclaUniforme}, punto  (ii), as\'i  que no esta  ni en
  $\ED_\col$,  ni en  $\ED_\lin$  y entonces  tampoco  ni en  $\ED_\vcol$ ni  en
  $\ED_\vlin$        (ver~\cite[Ej.~1]{FanChe84}       o~\cite[ejemplo       del
  ec.~(2.3)]{Kar81}).
...
\begin{ejemplo}[Wishart]\label{Ej:MP:WishartED}
  De  la  forma   de  la  densidad  de  probabilidad   $\displaystyle  p_X(x)  =
  \frac{|x|^{\frac{\nu-d-1}{2}}     \,      e^{-\frac12     \Tr\left(     V^{-1}
        x\right)}}{2^{\frac{d \nu}{2}}  \, |V|^{\frac{\nu}{2}} \, \Gamma_d\left(
      \frac{\nu}{2}  \right)}$   \  definido  sobre   \  $\Pos_d^+(\Rset)$  (ver
  secci\'on~\ref{Sssec:MP:Wishart}),     de     $\Tr\left(    V^{-\frac12}     x
    V^{-\frac12}\right) = \Tr\left( V^{-1} x\right)$ y de $|V^{-1} x| = |V|^{-1}
  |x|$  se  ve inmediatamente  que  la  matriz  aleatoria es  de  distribuci\'on
  $\ED_\simed\left( 0 , V , d_X \right)$  pero no es ni en $\ED_\bil$ y entonces
  ni  en  $\ED_\col$,  ni  en  $\ED_\lin$  y  por  consecuencia  ni  tampoco  en
  $\ED_\vcol$  o  en  $\ED_\vlin$.   El   generador  de  densidad  es  dado  por
  $\displaystyle      d_X(\lambda)       =      \frac{\left|      \Diag(\lambda)
    \right|^{\frac{\nu-d-1}{2}}   \,   e^{-\frac12  \un^t   \lambda}}{2^{\frac{d
        \nu}{2}}  \Gamma_d\left(  \frac{\nu}{2}   \right)}$  \SZ{Que  mas?  GSM,
    rayo...}
\end{ejemplo}
