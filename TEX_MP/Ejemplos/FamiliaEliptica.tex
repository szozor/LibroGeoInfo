...
\begin{proof}
  ...
  $\mu$~\cite{Hau21:I, Hau21:II}, visto como transformada de Laplace-Stieljes en
  \ $t = n$.  Se define la funci\'on \ $\displaystyle f(r) = \int_{\Rset_+} e^{-
    r  u} d\mu\left(  e^{-u}  \right)$ \  que coincide  con  \ $d_X$  \ sobre  \
  $\Nset$.   Se  proba   finalmente  que  \  $d_X(r)  -   f(r)$  \  se  prolonga
...
%
\begin{lema}\label{Lem:MP:AIndeêndiented}
...
\begin{proof}
  ...
  \M_{d,n}(\Rset)$  de rango  lleno con  $d \le  n$, \  $\widetilde{X}  \egald A
  \widetilde{\Sigma}^{\frac12}  \widetilde{G}  $  \  con \  $\widetilde{G}  \sim
  \N(0,I)$  \  $n$-dimensional,  tenemos  $X   =  M  \widetilde{X}  \egald  A  M
  \widetilde{\Sigma}^{\frac12}     \widetilde{G}    \egald     A     \left(    M
    \widetilde{\Sigma}  M^t \right)^{\frac12}  G$  \ con  \ $\widetilde{G}  \sim
  \N(0,I)$ \ $d$-dimensional (ver teorema~\ref{Teo:MP:StabilidadGaussiana}). Una
  mezcla de gausiana queda mezcla  de proyecci\'on, con la misma generadora $A$,
  y al  rev\'es pyede  ser vista  como proyecci\'on de  una mezcla  de dimension
  m\'as grande (no va a cambiar la generadora)~\footnote{Cuidense de que no anda
    si salimos a partir de una mezcla de base uniforme: como lo vamos a ver, las
    proyecciones no son uniforme m\'as si $d < n$, lo que se intuite por razones
    de dominio imagen (bolas por proyecci\'on de la esfera).}.
\end{proof}

\SZ{Adelantar: ley de  $A$ via Laplace, Fourier a ver,  Mellin.  Volver sobre la
  independencia de $A$ con $d$.}
...
%
  ...
  Ya        v\'imos        en       la        secci\'on~\ref{Sssec:MP:StudentT},
  ...
  tambi\'en~\cite{FanKot90, KotNad04}). \SZ{Se puede re-obtener este resultado buscando directamente $p_A$ por transformaci\'on (Laplace, Fourier) inversa de $d_X$ o $\varphi_X$, lema~\ref{toto} y~\cite[Cap.~5, Tab.~A.5.1, Ec.~14]{Pou10} o~\cite[Cap.~2, Tab.~2.5]{Pou10}.}
%
...
\label{Teo:MP:MezclaStidentRCaracteristica}
%
...
\label{Teo:MP:MezclaStidentRDensidad}
%
...
\begin{proof}
 ...
 \int_{\Rset_+} b^{-d-2k} \left( 1 - \frac{r}{(2 n + d) b^2} \right)_+^{\! n - k
  ...
  integrable sobre \  $(\alpha \; +\infty)$ \ y \  (ii) \ $\displaystyle \lim_{t
    \to +\infty} t^k f^{(k)}(t) = 0$. Luego, sea \ $P_n$  \ medida finita tal que
  %
  \[
...
Se notar\'a tmbi\'en que para buscar la  ley de \ $B$, sufice ver la Student-$r$
...
\]
%
Adem\'as, se  puede imaginar tambi\'en buscar  la ley de una  mezcla de gausiana
pasando por la  de mezcla de Student-$r$, tomando el  l\'imite cuando $d$ tiende
al infinito de la ley de $B$.
...
mismos   argumentos   que   llevaron   al   estudio   de   vectores   aleatorios
complejos.
\SZ{
%  Ahora, lo que
%juega el rol de  esfera va a ser la esfera compleja  $d$-dimensional \ $\{ u \in
%\Cset^d \tq u^\dag u = 1 \}$. 
Ver~\cite{Bau} para tener m\'as detailles.}
...
%
  ...
  \item Teorema~\ref{Teo:MP:ProyeccionComponentesEliptica} en: Sea \ $Z$, vector
    aleatorio  complejo $d$-dimensonal  de componentes  $Z_i$. Entonces  $Z \sim
    \CED(m,\Sigma,\varphi_Z)  \quad  \Leftrightarrow  \quad  \forall  \:  a  \in
    \Cset^d,   \quad   a^\dag  (Z   -   m)   \egald  \sqrt{\frac{a^\dag   \Sigma
        a}{\Sigma_{i,i}}} (Z_i - m_i)$;
  %
  \item Teorema~\ref{Teo:MP:MaxwellHershell} en:  \ $Z \sim \CED(m,I,\varphi_Z)$
    ...
    $\alpha > 0$;
  %
  \item \SZ{Teorema~\ref{Teo:MP:MezclaUniforme} en:};
  %
  %
  \item \SZ{Corolario~\ref{Cor:MP:MezclaUniforme} en:};
  %
  \item \SZ{Lema para alpha momentos \ref{Lem:MP:AlphaConR}};
  %
  \item \SZ{Teorema~\ref{Teo:MP:DensidadRayo} en: \ $Z \sim \CED(0,I,d_Z)$ \ y \
      $R  \egald \|Z\|$  \ admite  una  densidad que  se escribe  $\displaystyle
      p_R(r)  =  \frac{2  \pi^d}{\Gamma(d)}   \,  r^{2  d-1}  \,  d_X\left(  r^2
      \right)$};
  %
  \item   Teorema~\ref{Teo:MP:TransformadaDeHankel}   en:   para   \   $Z   \sim
    \CED(m,\Sigma,\varphi_Z)   \equiv  \CED(m,\Sigma,d_Z)$  \   las  generadoras
    caracter\'isticas y de  densidad son relacionadas por\newline $\displaystyle
  ...
  \item  Teorema~\ref{Teo:MP:SchoenbergDensidad} en:  \ $Z  \sim \CED\left(  m ,
    ...
    \sim \CN(0,I)$;
  %
  \item Lema~\ref{Lem:MP:AIndeêndiented} en: \ $\varphi_Z \in \PDSI$ (o $d_Z \in
  ...
  %
  \item \SZ{Mezcla de Student con carac~\ref{Teo:MP:MezclaStidentRCaracteristica}};
  %
  \item \SZ{Mezcla de Student con densidad~\ref{Teo:MP:MezclaStidentRDensidad}}
  \end{itemize}
%
\item \SZ{A ver unif compleja, gausiana compleja, extension student-r}
\end{itemize}
...
\item Caso complejo, con matriz unitaria; 

\item Ver BilBre def 13: extension matricial
