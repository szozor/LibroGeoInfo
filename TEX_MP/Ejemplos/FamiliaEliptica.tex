...
%
  ...
  %
  \[
  O  \,   \Delta^{-\frac12}  \,  Q^t  \left(   X  -  m  \right)   \:  \iguald  \:
  ...
  $m$ \ es llamado {\it par\'ametro de posici\'on} y \ la matriz \ $\Sigma$ \ es
  llamada {\it matriz caracter\'istica}.
\end{definicion}
%
...
$\Delta^{\frac12}$  \   los  factores  de   estiramientos,  $X  \iguald  m   +  Q
...
%
  ...
  llamado {\it  generadora caracter\'istica}.  Tomando  el logaritmo, obviamente
  ...
  escalar.        La        llamaremos       {\it       secunda       generadora
...
\begin{proof}
  ...
  funci\'on caracter\'istica, $Y \iguald O Y$.
...
\begin{corolario}
  ...
  %
  \[
  Q \Delta^{\frac12} Y + m \: \iguald \:  \Sigma^{\frac12} Y + m \: \iguald \: L Y
...
%
  ...
  ($\sigma_{(u,v)}(i)   =   v   \uno_{\{u\}}(i)    +   u   \uno_{\{v\}}(i)   +   i
  (1-\uno_{\{u,v\}}(i))$, ver notaciones).}
  %
  \[
  \kappa_X  = \frac{\varphi_X''(0)}{4  \,  \big( \varphi_X(0)  \big)^2} \Big(  I
  \otimes I + \tau_{(2,3)}\big[ I  \otimes I \big] + \tau_{(2,4)}\big[ I \otimes
  I \big] \Big)
  %\sum_{i,j=1}^d \Big( \left( \uno_i \uno_i^t \right) \otimes \left( \uno_j \uno_j^t
  %\right) + \left( \uno_i \uno_j^t  \right) \otimes \left( \uno_i \uno_j^t \right) +
  %\left( \uno_i \uno_j^t \right) \otimes \left( \uno_j \uno_i^t \right) \Big)
  \]
  %
\end{corolario}
\begin{proof}
...
%
  ...
  \in \Rset^d, \quad a^t (X - m) \iguald \sqrt{\frac{a^t \Sigma a}{\Sigma_{i,i}}}
  (X_i - m_i)
  \]
\end{teorema}
%
\begin{proof}
  ...
  es     a    simetr\'ia    el\'iptica,     es    decir     $(X_i-m_i)    \iguald
  -(X_i-m_i)$). Reciprocamente, si para cualquier  \ $a \in \Rset^d$ \ tenemos \
  $a^t (X  - m)  \iguald \sqrt{\frac{a^t \Sigma  a}{\Sigma_{i,i}}} (X_i  - m_i)$,
  necesariamente
  %
  \[
...
\begin{proof}
  ...
  \]
  %
  Para cualquier  $1 \le i  \le d$, tomando  $v = w  \uno_i$ con $w  \in \Rset_0$
  ...
  \left( \varphi_X'\left( w^2 \right) \, \varphi_Y\left( w^2 \lambda_i \right) +
    ...
    \right) \right) \uno_i = \varphi'\left( w^2 R_{i,i} \right) R \uno_i
  \]
  %
...
%
  ...
  %
  \[
  X \iguald R \, U \qquad \mbox{con} \qquad U \sim \U(\Sset_d), \quad R > 0 \quad
  \mbox{independientes}
  \]
  %
...
\begin{proof}
  ...
  de un vector aleatorio y la funci\'on caracter\'istica. De \ $X \iguald R \, U$
  \ y \ $\| U \| = 1$ \ viene  \ $R \iguald \| X \|$. \SZ{Me tengo que fijar bien
...
\begin{corolario}\label{Cor:MP:MezclaUniforme}
  ...
  \]
  %
  tales  que  \ $\|  X  \|  \iguald  R$ \  y  \  $\frac{X}{\|X\|} \iguald  U  \sim
  \U(\Sset_d)$ \ son independientes.
\end{corolario}
%
\begin{proof}
  Se  aplica  el  teorema~\ref{Teo:MP:IgualdadDistribucionFuncionVA} a  \  $f(x)
  = \begin{bmatrix} \|x\|\\ \frac{x}{\|x\|} \end{bmatrix}$ \ con \ $X \iguald Y =
  R \ U$.
\end{proof}.

De la escritura  $X \iguald R \,  U$, se puede ver $X  \sim \ED(0,I,\varphi_X)$ \
como mu\~necas rusas: a cada escala o  capa \ $R = r$ tenemos una distribuci\'on
uniforma sobre la esfera de este radio  $r$. Por eso, $X$ es tambi\'en dicho {\it
  mezcla de  escala} de  una {\it base}  \ uniforme  y \ $R$  \ es  llamada {\it
  variable generadora} con respecto a  esta base. En esta situaci\'on, llamaremos
tambi\'en a la variable \ $R$ \ {\it radio}.
...
el  momento  del ``radio''  $R$.   Adem\'as,  se puede  dar  una  otra forma  del
coeficiente $\alpha_k$:
%
...
%
  ...
  {\it generadora de densidad}. Reciprocamente, si la densidad tiene esta forma,
...
\ son  en el ``radio''  \ $R \iguald  \sqrt{(X-m)^t \Sigma^{-1} (X-m)}$. En  lo que
sigue,  sin   perdida  de  generalidad,   se  puede  concentrarse  en   $X  \sim
\ED(0,I,d_X)$.

Primero, se puede escribir la ley de $R$ a partir de $d_X$:
%
\begin{teorema}[Densidad del radio]
\label{Teo:MP:DensidadRadio}
%
  Sea \ $X \sim \ED(0,I,d_X)$ \ y \ $R \iguald \|X\|$. \ $R$ admite tambi\'en una
...
%
  ...
  {\it transformada  de Hankel}  de orden \  $\frac{d}{2}-1$~\cite{Sch38, Sch69,
    Sch71,  Lor54,  Pou99,  Pou10}.    A  veces,  la  transformaci\'on  dando  \
  $\varphi_X\left( w^2  \right)$ \ a  partir de \  $d_X\left( r^2 \right)$  \ es
  llamada {\it transformada de Hankel modificada} de orden \ $\frac{d}{2}-1$.

  M\'as generalmente, \ $\varphi_X$ \ es relacionada a la medida de probabilidad
  \   $P_R$  \  del   radio  por   transformaci\'on  dicha   de  Hankel-Stieltjes
...
\begin{proof}
  ...
  escritura \ $X \iguald R \, U$ \ tenemos
  %
  \begin{eqnarray*}
  ...
  teorema~\ref{Teo:MP:DensidadRadio} se obtiene
  %
  \[
...
%
\[
\theta_d \igualc 0, \quad \prod_{k=1}^{0} \igualc 1
\]

Los  par\'ametros  de  las  coordenadas  hiperesf\'ericas  (radio,  angulos)  son
...
En el  caso de vector  a simetr\'ia  esf\'erica, aparece que  el radio $R$  y los
...
\begin{teorema}
  ...
  \sin\theta_i \big)^{d-i-1}  \, \uno_{[0 \;  \pi)}(\theta_i), \quad 1 \le  i \le
  d-2 , \qquad p_{\Theta_{d-1}}(\theta_{d-1}) =  \frac{1}{2 \pi} \, \uno_{[0 \; 2
    \pi)}(\theta_{d-1})
  \]
  %
\end{teorema}
...
\begin{proof}
  ...
  f_{R,\Theta_1,\ldots,\Theta_{d-1}}(r,\theta_1,\ldots,\theta_{d-1}) = d_X\left(
    r^2    \right)     r^{d-1}    \prod_{j=1}^{d-2}    \left(     \left(    \sin
      \theta_i\right)^{d-j-1} \uno_{[0 \; \pi)}(\theta_i) \right) \, \uno_{[0 \; 2
    \pi)}(\theta_{d-1})
  \]
  %
  ...
  necesariamente el del radio).
\end{proof}
%
Obviamente,  se  recupera la  ley  de  \ $R  \iguald  \|X\|$  \ que  encontramos.
...
%
  ...
  el\'iptica  de  generadora de  densidad  $d_X$.   Este  vector es  dicho  {\it
...
llamados {\it mezcla de escala de  base gaussiana}, o simplemente {\it mezcla de
...
mezcla de gaussiana si y solamente si $X$ es consistente, es decir
  %
  \[
  X \,  \iguald \,  A \, \Sigma^{\frac12}  \, G  + m \quad  \Leftrightarrow \quad
  \varphi_X \in \PDSI
  \]
  %
...
orden~\footnote{Una funci\'on  \ $f: I \subset  \Rset \mapsto \Rset$  \ es dicha
  {\it absolutamente monotona} si es continua, diferenciable a todos los ordenes
  en el interior de $I$, y todas sus derivadas son positivas, $ \forall \: k \in
  \Nset, \quad f^{(k)} \ge 0$.  $f$ \ es dicha {\it completamente monotona} si \
...
%
  ...
  %
  \[
  X \, \iguald \, A \, \Sigma^{\frac12}  \, G + m \quad \Leftrightarrow \quad d_X
  \in \CM
  \]
  %
  con \ $A > 0$ \ independiente de \ $G \sim \N(0,I)$.
...
\begin{lema}\label{Lem:MP:AIndependienteD}
  ...
  escritura  \ $X  \iguald  A \,  \Sigma^{\frac12} G  +  m$ \  con  \ $A  > 0$  \
...
\begin{proof}
  ...
  \Mat_{d,n}(\Rset)$ de rango lleno con $d  \le n$, \ $X \iguald A \Sigma^{\frac12}
  G $ \ con  \ $G \sim \N(0,I)$ \ $n$-dimensional, tenemos $X' =  M X \iguald A M
  \Sigma^{\frac12} G \iguald A \left( M  \Sigma M^t \right)^{\frac12} G'$ \ con \
...
sencillamente  la  ley  del radio  \  $R$,  se  puede tambi\'en  caracterizar  la
...
inversas)~\footnote{Se   puede  tambi\'en  pensar   a  la   transformaci\'on  de
  ...
  reconoce en \  $g_X(r) = r^{d-1} d_X\left( r^2 \right)$ \  (ley del radio, bajo
  un factor) lo  que se llama convoluci\'on af\'in, entre \  $g_G(r)$ \ (ley del
  radio  de la  gaussiana)  \  y la  medida  de \  $A$,  $\displaystyle g_X(r)  =
...
%
  ...
  Se obtiene de \ $d_X$ \ la ley de radio
  %
  \[
  ...
  Student-$t$   como   mezcla   de   escala   gaussiana  donde   \   $A   \iguald
...
%
  ...
  Se obtiene as\'i la ley del radio bajo la forma
  %
  \[
...
%
  ...
  d_{X'}\left(     r^2      \right)     =     \frac{\Gamma\left(     \frac{d}{2}
    ...
    r^2 \right)^{\frac{d-2-d'}{2}} \uno_{(0 \; 1)}(r)
  \]
  %
...
%
  ...
  %
  \[
  \varphi_X \in  \PDSI_n, \:  n > d  \quad \Leftrightarrow  \quad X \iguald  B \,
...
\begin{proof}
  ...
  escribir \ $Y \iguald  R \, U$ \ con \ $U  \sim \U(\Sset_n)$, y reciprocamente.
...
\ED(m,\Sigma,\varphi_X) \iguald B \Sigma^{\frac12} S_{n-2} + m$, $d$-dimensional:
%
\[
\mbox{Construimos     }     \:\:     Y     \sim     \ED(0,I,\varphi_X)     \quad
(d+2)\mbox{-dimensional}  \qquad   \mbox{y}  \qquad  Y   \iguald  R  \,   U  \:\:
\Leftrightarrow \:\: B \iguald R
\]
...
%
  ...
  %
  \[
  d_X \in  \CM_n, \:  n \in \Nset_0  \quad \Leftrightarrow  \quad X \iguald  B \,
...
%
  ...
  %
  \[
  V  \,  \Delta^{-\frac12}  \,  Q^\dag  \left(  X  -  m  \right)  \:  \iguald  \:
...
%
\begin{proof}
  La prueba es  inmediata de \ $Z-m \iguald  Q \, \Delta^{\frac12} \, Y$  \ con \
  $Y$ \  a simetr\'ia esf\'erica. Se  cierra la prueba de,  \ $e^{\imath \theta}
  (Z-m) \iguald Q \, \Delta^{\frac12} \,  \big( e^{\imath \theta} I \big) \, Y$ \
...
\]
%
Llamaremos   {\it    forma   real}   ambas   inmersiones    y   las   escrituras
...
%      corolario~\ref{Cor:MP:MediaCovarianzaEliptica}
  ...
  \item Teorema~\ref{Teo:MP:ProyeccionComponentesEliptica} en: Sea \ $Z$, vector
    ...
    \forall \:  a \in  \Cset^d, \quad a^\dag  (Z - m)  \iguald \sqrt{\frac{a^\dag
  ...
  \item  Teorema~\ref{Teo:MP:MezclaUniforme} en:  \ $Z  \sim \CED(0,I,\varphi_Z)
    \:\: \Leftrightarrow  \:\: Z \iguald R \,  U$ \:\: con \:\:  $U \sim \U\left(
      \SCset_d \right)$  \ uniforme sobre  la esfera $d$-dimensional  compleja y
    m\'as generalmente, para \ $Z \sim \ED(m,\Sigma,\varphi_Z)$, \quad $Y \iguald
    \Sigma^{\frac12} R \, U + m$.
  %
  \item  Corolario~\ref{Cor:MP:MezclaUniforme} en: $Z  \sim \CED(0,I,\varphi_Z)$
    da \ $\| Z \| \iguald R$ \ y \ $\frac{Z}{\|Z\|} \iguald U \sim \U(\SCset_d)$ \
    son independientes;
  %
  \item Con  el mismo enfoque,  el lema~\ref{Lem:MP:AlphaConR} se expresa  en el
    ...
    \frac{\Gamma(d)}{\Gamma(d+k)} \Esp\left[ \left( (Z-m)^\dag \Sigma^{-1} (Z-m)
      \right)^k \right]$.
  %
  \item Teorema~\ref{Teo:MP:DensidadRadio} en: \ $Z  \sim \CED(0,I,d_Z)$ \ y \ $R
    \iguald \|Z\|$ \  admite una densidad que se  escribe $\displaystyle p_R(r) =
  ...
  \item Lema~\ref{Lem:MP:AIndependienteD}  en: \  $\varphi_Z \in \PDSI$  (o $d_Z
    ...
    \CED(m,\Sigma,\varphi_Z)$ \  $d$-dimensional, en la escritura \  $X \iguald A
  ...
  \item   Teorema~\ref{Teo:MP:MezclaStudentRCaracteristica}   en:   \  $Z   \sim
    ...
    \quad X  \iguald B  \, \Sigma^{\frac12} \,  S_{n-2} + m$  \ con  \ $B >  0$ \
  ...
  \item    Teorema~\ref{Teo:MP:MezclaStudentRDensidad}    en:    \    $Z    \sim
    ...
    \quad Z \iguald B \, \Sigma^{\frac12} \, S_{2 n +  2 d - 2} + m$ \ con \ $B >
...
se  tiene   $O  X_i  \iguald   X_i$,  dando  $O  \widehat{\Sigma}_X   O^t  \iguald
...
\iguald  O  \widehat{\Sigma}_{X,Y}  \iguald  \widehat{\Sigma}_{X,Y}  O'  \iguald  O
...
\begin{definicion}[Matriz a simetr\'ia esf\'erica]
  ...
  definidas  sobre  $\Mat_{1,d'}(\Rset)$.   $X$   es  dicha  {\it  a  simetr\'ia
    esf\'erica}
  %
  \begin{tabular}
  {>{}p{.015\textwidth}
   ...
   >{}p{.43\textwidth}}
  %
  $(\izq)$ & {\it a la izquierda} si \ $\forall \: O \in \Ort_d(\Rset)$, \[ O X
  \: \iguald \: X \] & &
  %
  $(\der)$ & {\it a la derecha} si \ $\forall \: O' \in \Ort_{d'}(\Rset)$, \[ X
  O' \: \iguald \: X \]\\
  %
  $(\col)$ & {\it por columnas} si \ $\forall \: O_1 , \ldots , O_{d'} \in
  \Ort_d(\Rset) $, \[ \begin{bmatrix} O_1 X_1 & \cdots & O_{d'}
  X_{d'} \end{bmatrix} \: \iguald \: \begin{bmatrix} X_1 & \cdots &
  X_{d'} \end{bmatrix} \] & &
  %
  $(\lin)$ & {\it por lineas} si \ $\forall \: O'_1 , \ldots , O'_d \in
  \Ort_{d'}(\Rset) $, \[ \begin{bmatrix} X'_1 O'_1 \\ \vdots \\ X'_d
  O'_d \end{bmatrix} \: \iguald \: \begin{bmatrix} X'_1 \\ \vdots \\
  X'_d \end{bmatrix} \]\\
  %
  $(\vcol)$ & {\it por vectorizaci\'on columna} si \ $\forall \: O
  \in \Ort_{d d'}(\Rset) $, \[ O \begin{bmatrix} X_1 \\ \vdots \\
  X_{d'} \end{bmatrix} \: \iguald \: \begin{bmatrix} X_1 \\ \vdots \\
  X_{d'} \end{bmatrix} \] & &
  %
  $(\vlin)$ & {\it por vectorizaci\'on linea} si \ $\forall \: O' \in \Ort_{d
  d'}(\Rset) $, \[ \begin{bmatrix} X'_1 & \cdots & X'_d \end{bmatrix} O' \: \iguald
  \: \begin{bmatrix} X'_1 & \cdots & X'_d \end{bmatrix} \]
  %
  \end{tabular}
%
\centerline{$(\bil)  \:$  {\it bilateral}~\footnote{En  la  literatura, se  dice
  ...
  \Ort_d(\Rset), O' \in \Ort_{d'}(\Rset), \quad\qquad O X O' \: \iguald \: X$}

\

\centerline{$(\simed)  \:$  {\it  sim\'etrica}~\footnote{En  la  literatura,  se
  ...
  y \ $\forall \: O \in \Ort_d(\Rset), \quad\qquad O X O^t \: \iguald \: X$}
...
Nota: para $d' = d$, tomando $O' =  O^t$ con $O = \uno_i \uno_j^t + \uno_j \uno_i^t
+ \sum_{k \ne i,j} \uno_k \uno_k^t$,  se observa, por inducci\'on, que $Y^t \iguald
...
%
  ...
  dicha  {\it  a  simetr\'ia  el\'iptica  en   torno  a  una  matriz  $m  =  \in
...
%
  ...
  \Ort_d(\Rset),  \quad  O   Y  \iguald  Y$,  \  es  decir   $\forall  \:  O  \in
  \Ort_d(\Rset),  \quad  Y^t  O^t  \iguald   Y^t$,  \ie  $X^t  \in  \ED_\der$,  y
  ...
  %
  \[
  O \begin{bmatrix}  Y_1\\ \vdots\\ Y_{d'}  \end{bmatrix} \iguald \begin{bmatrix}
  ...
  \iguald   \begin{bmatrix}  Y_1   \\  \vdots   \\  Y_{d'}   \end{bmatrix}  \quad
  \Leftrightarrow   \quad    \begin{bmatrix}   O_1   Y_1&    \cdots   &   O_{d'}
    Y_{d'}    \end{bmatrix}   \iguald    \begin{bmatrix}   Y_1    &    \cdots   &
    Y_{d'} \end{bmatrix} = Y
  \]
  %
  lo que prueba  que tambi\'en \ $X \in \ED_\col$.  Luego,  si $X \in \ED_\col$,
  entonces  \  $\forall  \:  O_1  ,  \ldots  ,  O_{d'}  \in  \Ort_d(\Rset)$,  de
  $\begin{bmatrix} O_1 X_1  & \cdots & O_{d'} X_{d'}  \end{bmatrix} \iguald Y$, y
  en particular para cualquiera $O \in \Ort_d(\Rset)$ se obtiene
  %
  \[
  O Y = \begin{bmatrix} O Y_1 & \cdots & O Y_{d'} \end{bmatrix} \iguald Y
  \]
  %
  ...
  \: O \in \Ort_d(\Rset), O' \in \Ort_{d'}(\Rset),  \quad O Y O' \iguald Y$, y en
  ...
  O Y \iguald Y$ \ y  a continuaci\'on $\forall \: O' \in \Ort_{d'}(\Rset), \quad
  O Y O' \iguald Y O' \iguald Y$: necesariamente $X \in \ED_\bil$.
  ...
  \, \begin{bmatrix}
    Y_1\\ \vdots\\ Y_{d'} \end{bmatrix} \: \iguald \: \begin{bmatrix} Y_1\\ \vdots\\
    Y_{d'} \end{bmatrix}
  \]
  %
  ...
  O'$   as\'i,  $O  Y   O'  \iguald   Y$:  tambi\'en   $X  \in   \ED_\bil$.   Del
  ...
  cantidad que comparten estos dos vectores, \ie $\uno^t \lambda(\omega^t \omega)
  = \Tr( \omega^t \omega) = \uno^t \diag(\omega^t \omega)$, \ie $\Phi_Y(\omega) =
  ...
  \iguald Y$, y en particular para $O' = O^t$. Pero eso no implica que $X$ sea en
  ...
  \Sim_d(\Rset)$  as\'i  que $O  Y  O' \niguald  Y$:  una  matriz definida  sobre
...
\item  Sea \ $S$  \ vector  aleatorio de  distribuci\'on esf\'erica,  y \  $T$ \
  ...
  T^t$. Inmediatamente, de  \ $O S \iguald  S$ \ para cualquiera matriz  \ $O \in
...
%  columnas $\omega_j \in \Rset^d, \: j = 1, \ldots , d'$.
  ...
  $\Tr\left( \omega^t \Sigma \omega  \right) = \uno^t \diag\left( \omega^t \Sigma
...
\item  Nota:  si  $X  \in  \ED_\col \cap  \ED_\lin  \varsubsetneq  \ED_\bil$  la
  ...
  \right) = \uno^t \lambda\left( \omega^t \, \Sigma \, \omega \, \Sigma' \right)$
  se escribe
  %
  \[
  \Phi_X(\omega) = e^{\imath \, \Tr\left( \omega^t m \right)} \, \varphi_X\left(
  ...
  %
  %Pasando, $\Tr\left(  \omega^t \, \Sigma \,  \omega \, \Sigma'  \right) = \uno^t
...
\item $X \in \ED_\simed$ si y solamente si
  ...
  %  Y Q^t \iguald Q$.  Al final,  para cualquier matriz de permutaci\'on \ $\Pi$,
...
\item $X \in \ED_\vcol$ \ si y solamente si
  ...
  =  \uno^t  \diag\left(  (x-m)^t  \Sigma^{-1}  (x-m)  \right)$  \  enfatisa  las
...
\item Obviamente, tambi\'en con la densidad si \ $X \in \ED_\vcol \cap \ED_\vlin
  ...
  caso $(\col)$  y por transposici\'on),  \ie de $\Tr\left(  (x-m)^t \Sigma^{-1}
    (x-m)  \Sigma'  \right)  =  \uno^t \lambda\left(  (x-m)^t  \Sigma^{-1}  (x-m)
...
\item La  extensi\'on del teorema~\ref{Teo:MP:MomentosCumulantesEliptica}  y del
  ...
  \:  \diag(\omega^t \omega),  \:  \Tr(\omega^t \omega)  = \uno^t  \diag(\omega^t
  \omega)  = \uno^t  \lambda(\omega^t  \omega)$  \ seg\'un  el  caso estudiado  \
  ...
  \item Para  $X \sim \ED_{\izq,\col,\vcol}(m,\Sigma,\varphi_X)$  \ o \  $X \sim
    ...
    \]
    %
    De hecho, siendo \ $Y$ \ a sim\'etria esf\'erica izquierda, de \ $Y \iguald O
    Y$ \ para cualquiera matriz ortogonal, se obtiene tambi\'en \ $- Y \iguald Y$,
  ...
  \item Para \ $X \sim \ED_\simed(m,\Sigma,\varphi_X)$,
    ...
    \, \frac{\partial  \varphi_X}{\partial \lambda_1}(0) =  \frac{1}{d} \, \uno^t
    \Esp\left[ \lambda(Y) \right]
    \]
    %
    Una    pruba    dada    por   ejemplo    en~\cite[\S~13.2]{BilBre99}    (ver
    tambi\'en~\cite{Mal61,  Tyl82}) sale  de \  $Y  \iguald O  Y O^t$  \ dando  \
    ...
    Eso  conduce a  \  $\mu_1  = \frac{1}{d}  \Esp[\Tr(Y)]  = \frac{1}{d}  \uno^t
    ...
    iguales:     $\nabla_\lambda      \varphi_X     (0)     =     \frac{\partial
      \varphi_X(0)}{\partial \lambda_1} \: \uno$.  Ahora, notando $\lambda \equiv
    ...
    \frac{\partial    \lambda}{\partial   \omega_{i,j}}(0)    =   \frac{\partial
      \varphi_X}{\partial     \lambda_1}(0)      \:     \uno^t     \frac{\partial
      \lambda}{\partial \omega_{i,j}}(0) = \frac{\partial \varphi_X(0)}{\partial
      \lambda_1}  \: \frac{\partial  \uno^t  \lambda}{\partial \omega_{i,j}}(0)$.
    Eso    da,    de   $\uno^t    \lambda    =   \Tr(\omega)$,    $\frac{\partial
      \varphi_X}{\partial \omega_{i,j}}(0)  = \frac{\partial \varphi_X}{\partial
      \lambda_1}(0) \uno_{\{i\}}(j)$, es decir que, de la expresi\'on de la media
  ...
  \Mat_{d',d'}(\Rset)
  % \sum_{j,l=1}^{d'}  \alpha_{j,l}(\varphi_X) \sum_{i=1}^d \uno_i  \otimes \uno_j
  % \otimes \uno_i \otimes  \uno_l \qquad \mbox{con} \qquad \alpha_{j,}(\varphi_X)
  ...
  % \sum_{j=1}^{d'} \alpha_j(\varphi_X) \sum_{i=1}^d \uno_i \otimes \uno_j \otimes
  %  \uno_i  \otimes  \uno_j  \qquad  \mbox{con} \qquad  \alpha_j(\varphi_X)  =  -
  ...
  % \sum_{i=1}^d \sum_{j=1}^{d'} \uno_i \otimes \uno_j
  % \otimes \uno_i \otimes \uno_j
  \]
  %
  ...
  % \sum_{i=1}^d \sum_{j=1}^{d'} \uno_i \otimes \uno_j \otimes \uno_i \otimes \uno_j
  \]
  %
  ...
  \frac{\partial  \varphi_X(0)}{\partial  \lambda_1}  \:  \uno$.   Con  el  mismo
  ...
  = \frac{\partial^2 \varphi_X}{\partial \lambda_1 \partial \lambda_2}(0) \, \uno
  \uno^t   +  \left(   \frac{\partial^2  \varphi_X}{\partial   \lambda_1^2}(0)  -
  ...
  \frac{\partial   \varphi_X}{\partial   \lambda_1}(0)  \frac{\partial^2   \uno^t
    ...
    \omega_{k,l}}(0)$.  Ahora, de \ $\uno^t  \lambda = \Tr(\omega^t \omega)$ \ se
  nota que el  primer t\'ermino vale \ $2  \, \frac{\partial \varphi_X}{\partial
    \lambda_1}(0)  \, \uno_{\{i\}}(k)  \uno_{\{j\}}(l)$ \  y que  el  secundo vale
...
\item Para \ $X \sim \ED_\simed(m,\Sigma,\varphi_X)$ la covarianza es dada por:
  ...
  % \sum_{i,j} \Big(  \uno_i \otimes  \uno_j \otimes \uno_i  \otimes \uno_j  + \uno_i
  % \otimes \uno_j  \otimes \uno_j \otimes \uno_i  \Big) \Cov[X] =  \mu_2 \, \Sigma
  % \otimes \Sigma + \sigma_1^2 \sum_{k,l} \Big( \big(
  % \uno_k \uno_l^t \, \Sigma \big) \otimes \big( \uno_l \uno_k^t \, \Sigma \big) +
  % \big( \Sigma \, \uno_k \uno_l^t \, \Sigma \big) \otimes \big( \uno_k \uno_l^t
  % \big) \Big)
  \]
  %
  ...
  %
  \[
  d^2  \, \mu_2(\varphi_X)  + 2  \,  d \,  \sigma_1^2(\varphi_X) \:  = \:  \uno^t
  \Cov\left[ \lambda(Y) \right] \uno
  \]
  %
  y
  %
  ...
  d  \,  \mu_2(\varphi_X) +  d^2  \,  \sigma_1^2(\varphi_X)  \: =  \:  \Tr\left(
    ...
    \frac{1}{d} \uno \uno^t \right) \Esp[ \lambda(Y) ]
  \]
  %
  Pasando, se notar\'a que  \ $\Pi = I - \frac{1}{d} \uno  \uno^t$ \ es una matriz
  de  proyecci\'on  sobre  el  ortogonal  de  $\uno$.

  La pruba,  dada por ejemplo  en~\cite[\S~13.2]{BilBre99}, sale de nuevo  de la
  relaci\'on \ $Y  \iguald O Y O^t$ \ para cualquieras  matrices ortogonales. Si se
  ...
  Y^t$).  Adem\'as,  para cualquiera  matriz ortogonal  \ $O$, de  $Y \iguald  O Y
  ...
  %\Sigma_{k,j} \Sigma_{i,l} = \big( \uno_i \uno_k^t \Sigma \big)_{i,j} \big( \uno_k
  %\uno_i^t \Sigma \big)_{k,l}$, conduciendo a \ $\Big( \big( \uno_i \uno_k^t \Sigma
  %\big) \otimes \big( \uno_k  \uno_i^t \Sigma \big) \Big)_{u,j,v,l} = \Sigma_{i,l}
  %\Sigma_{j,k} \uno_{(i,k)}((u,v))$, \  $\Sigma_{i,k} \Sigma_{j,l} = \Sigma_{i,k}
  %\Sigma_{l,j}  =  \big(  \Sigma   \,  \uno_k  \uno_l^t  \,  \Sigma  \big)_{i,j}$,
  %conduciendo a $\Big( \big( \Sigma \,  \uno_k \uno_l^t \, \Sigma \big) \! \otimes
  %\!   \big( \uno_k  \uno_l^t  \big) \Big)_{i,j,u,v}  = \Sigma_{i,k}  \Sigma_{j,l}
  %\uno_{(k,l)}((u,v))$. Con cambio de indices adecuados, se obtiene la covarianza
  ...
  que  \   $\Tr(Y)  =   \uno^t  \lambda(Y)$)  \   dando  $\Var[\Tr(Y)]   =  \uno^t
  \Cov[\lambda(Y)] \uno$, lo que da  la primera relaci\'on v\'inculando \ $\mu_2$
  ...
  \Tr\left( \Esp\left[ (Y - \Esp[Y]) (Y - \Esp[Y])^t \right] \right) = \Tr\left(
    \Esp[Y Y^t] \right) - \Tr\left( \Esp[Y] \Esp[Y]^t \right) = \Esp\left[ \uno^t
    \lambda\left(  Y  Y^t  \right)  \right]  - \frac{1}{d}  \left(  \uno^t  \Esp[
    \lambda(Y) ] \right)^2 = \Tr\left( \Esp\left[\lambda(Y) \lambda(Y)^t \right]
  \right)  - \frac{1}{d} \uno^t  \Esp[ \lambda(Y)  ] \Esp[  \lambda(Y) ]^t  \uno =
  \Tr\left( \Cov\left[\lambda(Y) \right]  + \Esp[\lambda(Y)] \Esp[ \lambda(Y)]^t
  \right)  - \frac{1}{d} \Tr\left(\Esp[  \lambda(Y) ]  \Esp[ \lambda(Y)  ]^t \uno
    \uno^t  \right)  = \Tr\left(  \Cov\left[\lambda(Y)  \right]  \right) +  \Esp[
  \lambda(Y) ]^t \left( I - \frac{1}{d}  \uno \uno^t \right) \Esp[ \lambda(Y) ]$ \
  ...
  la  media). El  secundo t\'ermino  vale  $\frac{\partial^2 \varphi_X}{\partial
    \lambda_1 \partial \lambda_2}(0) \uno_{\{i\}}(j) \uno_{\{k\}}(l)$ (ver calculo
  ...
  la   simetr\'ia   de  \   $\omega$,   \  $\frac{\partial   \lambda^t}{\partial
    ...
    \omega_{i,j}} =  \left( 2  - \uno_{\{i\}}(j) \right)  \, \omega_{i,j}$,  y, a
  ...
  \frac{\partial   \lambda}{\partial  \omega_{k,l}}   =  \frac12   \left(   2  -
    \uno_{\{i\}}(j)   \right)   \left(   2   -  \uno_{\{k\}}(l)   \right)   \left(
    \uno_{\{i\}}(k)  \uno_{\{j\}}(l)  +  \uno_{\{i\}}(l)  \uno_{\{j\}}(k)  \right)$.
  ...
  \otimes  Y]  =   -  \frac{\partial^2  \varphi_X}{\partial  \lambda_1  \partial
    \lambda_2}(0) \sum_{i,j=1}^d \uno_i \otimes \uno_i \otimes \uno_j \otimes \uno_j
  +  \frac12  \left(  \frac{\partial^2  \varphi_X}{\partial  \lambda_1  \partial
      \lambda_2}(0)   -  \frac{\partial^2   \varphi_X}{\partial  \lambda_1^2}(0)
  \right) \sum_{i,j=1}^d \left( \uno_i  \otimes \uno_j \otimes \uno_i \otimes \uno_j
    + \uno_i  \otimes \uno_j \otimes  \uno_j \otimes \uno_i \right)$,  conduciendo a
...
%
  ...
  \item Denotamos  por \ $\bullet$ \  los casos \ $(\izq),  (\col)$ y $(\vcol)$.
    ...
    cambia la  esfericidad. Adem\'as,  para \  $A' = \uno_i$  \ ($n'  = 1$)  \ se
    obtiene  que\newline  $X \,  \sim  \, \ED_\bullet(m,\Sigma,\varphi_X)  \quad
    \Rightarrow \quad X_i \: \sim \: \ED(m_i , \Sigma , \varphi_{X \uno_i})$ 
  %
...
\item  El  teorema~\ref{Teo:MP:ProyeccionComponentesEliptica}  de  proyeccion  y
  ...
  \quad  \forall \:  a \in  \Rset^d, \quad  a^t (X  - m)  \iguald \sqrt{\frac{a^t
...
\item El teorema  de mezcla uniforme~\ref{Teo:MP:MezclaUniforme} tambi\'en tiene
  ...
  %
  \begin{itemize}
  \item $X \sim \ED_\izq(m,\Sigma,\varphi_X)  \:\: \Leftrightarrow \:\: X \iguald
  ...
  %
  \item $X \sim \ED_\col(m,\Sigma,\varphi_X)  \:\: \Leftrightarrow \:\: X \iguald
    ...
    % prueba.
  %
  \item $X \sim \ED_\vcol(m,\Sigma,\varphi_X) \:\: \Leftrightarrow \:\: X \iguald
  ...
  \item $X \sim \ED_\bil(m,\Sigma,\Sigma',\varphi_X) \:\: \Leftrightarrow \:\: X
    \iguald    \Sigma^{\frac12}   \,   U_\izq    \,   \Lambda_\bil    \,   V_\der
...
%
  \item  $X  \sim  \ED_\simed(m,\Sigma,\varphi_X)  \:\: \Leftrightarrow  \:\:  X
    \iguald \Sigma^{\frac12} \,  U \, \Lambda_\simed \, U^t  \ \Sigma^{\frac12} +
    ...
    1$ no cambia ni la distribuci\'on  ($U \iguald U S$), ni la descomposici\'on.
    ...
    $\Delta$ son independientes. A continuaci\'on, de $X = U \Delta U^t \iguald O
...
\item  Se  monstr\'o  que,  en  el  caso  $d'  =  1$  la  densidad  es  definida
  completamente    por   lo    del   radio,    con   densidad    dada    por   el
  teorema~\ref{Teo:MP:DensidadRadio}. Se  lo puedo  probar directamente y  ver el
  ...
  \item Si $X \sim \ED_\izq(m,\Sigma,\varphi_X)$, la ley de $X$ es completamente
    ...
    \iguald  (X-m)^t \Sigma^{-1}  (X-m)$ \  es  definida de  manera \'unica.   Si
...
%
% \SZ{La densidad  del  radio, que  no  explicitamos ac\'a  (ver
...
%
  ...
  $d_X(r)   \propto  e^{-\frac{r}{2}}$  (ver   tambi\'en~\cite{Daw81,  FanKot90,
    GupNag99}).   Como  en el  caso  escalar, el  cuadrado  del  radio tiene  una
  ...
  este enfoque el generador  de densidad \ $d_X(\lambda) \propto e^{-\frac{\uno^t
      \lambda}{2}}$, lo que es coherente  con el punto de vista anterior, siendo
  $\uno^t \lambda(y y^t) = \| y \|_F^2 \equiv r^2$.
