...
distribuci\'on uniforme sobre la esfera \ $\Sset_d$, \ $U \sim \U(\Sset_d)$~\SZ{\cite{toto, titi, FanKot01}:}
%
...
%
  ...
  parametrizar \ $s \in \Sset_d^{\pm}$ \  bajo la forma \ $s = \begin{bmatrix} b
    & \pm \sqrt{1-\|b\|^2} \end{bmatrix}^t, \quad b \in \Bset_{d-1}$, lo que da,
  siguiendo~\cite[ec.~4.644]{GraRyz15} (cambio de variables) y notando $\mu_L$ \
...
%
  ...
  %
  \[
  \zeta_{i_1,\ldots,i_{2  k  +1}}[X] =  \kappa_{i_1,\ldots,i_{2  k  +1}}[X] =  0,
  \qquad k \ge 1
  \]
  %
  ...
  %
  \[
  \zeta_{i_1,\ldots,i_{2   k}}[X]   =   \alpha_k\left(  \varphi_X   \right)   \,
  c_{i_1,\ldots,i_{2 k}}(\Sigma) \qquad  y \qquad \kappa_{i_1,\ldots,i_{2 k}}[X]
  ...
  %
  \[
  c_{i_1,\ldots,i_{2k}}(\Sigma) = \sum_{\pi \in  \Pi_{2 k , 2}} \prod_{(l,n) \in
    \pi} \Sigma_{i_l,i_n} \qquad \mbox{y} \qquad \alpha_k(f) = (-2)^k f^{(k)}(0)
  \]
  %
...
el  tensor de  los momentos  tiene  una structura  dada bajo  un factor  escalar
dependiendo   de  la   ley.   Lo   que  es   menos  obvio,   de   la  relaci\'on
...
$\Sset_d$. Este resultado es  debido a Sch{\oe}nberg~\cite{Sch38, FanKot01} y se
enuncia como sigue:
%
...
De esta escritura, se puede ir un paso m\'as all\'a~\cite[Teo~2.3]{FanKot}:
%
\begin{corolario}
  Sea  \ $X$ \  $d$-dimensional a  simetria esferica.  Entonces, este  vector se
  escribe estocasticamente como
...
%
\begin{proof}
  Seg\'un~\cite{FanKot01, Zol86}, si  dos vectores aleatorios \ $A$ \  y \ $B$ \
...
De esta  escritura, queda ahora claro  que cada tensor de  momentos centrales de
orden  par  tienen  una estrctura  fija:  de  \  $X  =  R \Sigma  U$  \  tenemos
$\zeta_{i_1,\ldots,i_{2k}}[X]     =    \Esp\left[     R^{2     k}    \right]
\zeta_{i_1,\ldots,i_{2k}}[\Sigma  U]$  (ver  ej.~\cite[teo.~2.8]{FanKot01}  para
$\Sigma  \propto I$). Adem\'as,  se puede  dar una  otra forma  del coefficiente
$\alpha_k$:
%
\begin{lema}
...
\begin{proof}
  ...
  \]
  %
  A continuaci\'on, para \ $X \sim \ED(m,\Sigma,\varphi_X)$,  de $X-m = R \ \Sigma \ U$ \ ($R$ es escalar) y de la formula del tensor de momentos
  centrales se obtiene
  %
  \[
  \zeta_{i_1,\ldots,i_{2    k}}[X]     =    \Esp\left[    R^{2     k}    \right]
  \zeta_{i_1,\ldots,i_{2   k}}[\Sigma   U]   =   \Esp\left[  R^{2   k}   \right]
  \alpha_k(\varphi_U) c_{i_1,\ldots,i_{2 k}}(\Sigma)
  \]
  %
  es decir
  %
...
%
  ...
  J_{\frac{d}{2}-1}( r  w) \, dr\qquad  \mbox{y} \qquad d_X\left( r^2  \right) =
  \left(   2   \pi   \right)^{-\frac{d}{2}}   r^{1-\frac{d}{2}}   \int_{\Rset_+}
  w^{\frac{d}{2}} \varphi_X\left( w^2 \right) \, J_{\frac{d}{2}-1}( r w) \, dw
  \]
  %
  Esta    transformaci\'on    es   conocida    como    {\em   transformada    de
    Hankel}~\cite{Lor54, Pou10}.
\end{teorema}
%
\begin{proof}
...
\SZ{
\begin{itemize}
\item mzcla consistente? varphi ind dimension?

\item Volver a los cumulantes $2 k$: se puede decir m\'as?

\item Coordenadas hyperesfericas

\item GSM y consistante


\item  Hablar de  nuevo de  la Gausiana con el $r$, de Student-t con $r$ y como  GSM~\cite{toto, titi,  FanKot90,
