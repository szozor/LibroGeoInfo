...
%
  ...
  \item  El  teorema~\ref{Teo:MP:MomentosCumulantesEliptica}  da, con  el  mismo
    ...
    tambi\'en~\cite{Kri76}).
%      corrolario~\ref{Lem:MP:MediaCovarianzaEliptica}
  %
...
\subsubseccion{Caso matriz variada en unas palabras}
...
\centerline{$(\simed)  \:$  {\em  sim\'etrica}~\footnote{En  la  literatura,  se
    ...
    ej. en~\cite{Tyl82, BilBre99}.} si  \ $d = d', \: \X =  \Sim_d(\Rset)$ \ y \
  $\forall \: O \in \Ort_d(\Rset), \quad\qquad O X O^t \: \egald \: X$}
...
\begin{lema}
  ...
  %
  \item\label{Enum:MP:EDM_Simetrica}  \SZ{A VER BIEN, las rotaciones arruiniando  la simetr\'ia; creo que la interseccion es de hecho vacia; ver entonces lo que dije sobre la beta que es simed!} $\{  X  \in   \ED_\bil  \tq  X  =  X^t  \}
...
\item     La    ley     Student-$t$     matriz    variada     (ver    fin     de
  ...
  $\ED_\vlin$ (lo  mismo occure  para la ley  beta matriz-variada, vista  fin de
  secci\'on~\ref{Sssec:MP:Beta}):  $\ED_\bil  \setminus  \left(  \ED_\vcol  \cup
    \ED_\vlin \right) \neq \emptyset$.
%
...
\item  \SZ{ Momentos  y  cumulantes~\ref{Teo:MP:MomentosCumulantesEliptica}, y
      corrolario~\ref{Lem:MP:MediaCovarianzaEliptica}}
% Momentos y cumulantes... ver por lo menos promedio y covarianza
%
\item  El Teorema~\ref{Teo:MP:TranformacionAfinEliptica}  de  conservaci\'on del
  ...
  %
  \begin{itemize}
  \item  Denotamos   por  \  $\bullet$  \   los  casos  \   $(\izq),  (\col)$  y
    $(\vcol)$.  Entonces  $X   \,  \sim  \,  \ED_\bullet(m,\Sigma,\Phi_X)  \quad
    \Rightarrow \quad  A X B  + C \: \sim  \: \ED_\bullet(A m  B + C ,  A \Sigma
    A^t , \varphi_{X B})$.  En  particular, si $X$ es a simetr\'ia esf\'erica
  ...
  \item $X \, \sim  \, \ED_\bil(m,\Sigma,\Sigma',\Phi_X) \quad \Rightarrow \quad
    A X B  + C \: \sim \:  \ED_\bil(A m B + C  , A \Sigma A^t , B  \Sigma' B^t ,
    \varphi_X)$.
  %
  \item \SZ{$X \, \sim  \, \ED_\simed(m,\Sigma,\Phi_X) \quad \Rightarrow \quad
    A X A  + C \: \sim \:  \ED_\bil(A m A + C  , A \Sigma A^t,
  ...
  \end{itemize}
  %
  En  particular, si  $X$ es  a  simetr\'ia el\'iptica  de uno  de estos  tipos,
  cualquier componente es un vector aleatorio a simetr\'ia el\'iptica.
%
...
%
  ...
  variada, vista
% $\displaystyle  p_X(x) =  \frac{1}{(2 \pi)^{\frac{d d'}{2}}  \, \left|
%      \Sigma  \right|^{\frac{d'}{2}}  \left|  \Sigma' \right|^{\frac{d}{2}}}  \:
%  e^{-\frac12 \Tr \left( (x-m)^t \Sigma^{-1} (x-m) \Sigma'^{-1} \right)}$ \ (ver
  fin  de  secci\'on~\ref{Sssec:MP:Gaussiana}  (primer  item),  de  $\Tr  \left(
    (x-m)^t  \Sigma^{-1}   (x-m)  \Sigma'^{-1}   \right)  =  \Tr   \left(  (x-m)
...
\begin{ejemplo}[Ennsemble Gaussiano Ortogonal]\label{Ej:MP:GOE}
%
  ...
  matriz   aleatoria  partenece   a  $\ED_\simed$   (ver  tambi\'en~\cite{Meh89,
    AndGui10}) pero  no es ni en $\ED_\bil$  y entonces ni en  $\ED_\col$, ni en
  $\ED_\lin$ y por  consecuencia ni tampoco en $\ED_\vcol$  o en $\ED_\vlin$. El
  generador  de densidad  se escribe  en  este caso  $d_X(\lambda) \propto  e^{-
    \frac{d}{4} \, \un^t \lambda^{\odot 2}}$.
  %Como en el caso escalar, el  cuadrado del rayo tiene una distribuci\'on Gamma,
  %$R_\vcol^2 \sim \G\left( \frac{d d'}{2} , \frac12 \right)$. \SZ{Que mas?}
\end{ejemplo}


\begin{ejemplo}[Wishart]\label{Ej:MP:WishartED}
  De la  forma de  la densidad de  probabilidad $\displaystyle p_X(x)  = \propto
  \left|  V^{-1} x \right|^{\frac{\nu-d-1}{2}}  \, e^{-\frac12  \Tr\left( V^{-1}
      x\right)}$     \    definido     sobre     \    $\Pos_d^+(\Rset)$     (ver
  secci\'on~\ref{Sssec:MP:Wishart}),   se  ve   inmediatamente  que   la  matriz
  aleatoria es de distribuci\'on $\ED_\simed\left( 0  , V , d_X \right)$ pero no
  es  ni en  $\ED_\bil$ y  entonces ni  en $\ED_\col$,  ni en  $\ED_\lin$  y por
  consecuencia  ni tampoco  en $\ED_\vcol$  o en  $\ED_\vlin$.  El  generador de
  densidad es dado por $\displaystyle d_X(\lambda) \propto \left| \Diag(\lambda)
  \right|^{\frac{\nu-d-1}{2}}  \,  e^{-\frac12  \un^t \lambda}$.   \SZ{Que  mas?
    GSM, rayo...}
\end{ejemplo}
