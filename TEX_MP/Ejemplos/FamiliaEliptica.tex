...
esferica se conserva tomando $O$ aleatoria independiente de $X$:
%
\begin{lema}\label{Lem:MP:SIAleatoria}

  Sea   \   $X$  \   vector   aleatorio   $d$-dimensional   real  a   simetr\'ia
  esf\'erica.  Entonces, para  cualquier  matriza aleatoria  $0$ definida  sobre
  $O(\Omega) = \Ort_d(\Rset)$,
  %
  \[
  O  X  \: \egald  \: X
...
\begin{proof}
  ...
  traves  de   la  esperanza  condicional,  $\Phi_{O   X}(\omega)  =  \Esp\left[
    \left.     \Phi_{O    X}(\omega)     \right|    O\right]     =    \Esp\left[
    \left.  \Phi_X(\omega)  \right|  O\right]$   \  de  la  definici\'on  de  la
  invarianza esf\'erica.  La prueba  se cierra de  al independencia entre  $X$ y
  $O$.
\end{proof}

En todo lo que  sigue, consideraremos matrices ortogonales deterministicas, pero
se las podr\'as  re-emplazar por matrices ortogonales aleatoria  sin que cambien
las deiniciones, los teoremas, lemas o corolarios.
...
\item La  extensi\'on del teorema~\ref{Teo:MP:MomentosCumulantesEliptica}  y del
  ...
  \item Para \ $X \sim \ED_\simed(m,\Sigma,\varphi_X)$,
    ...
    un autovalor  $\mu_1$, cualquier \ $O v$  \ va a ser  autovector asociado al
    mismo autovalor.  De \ $\{ O v \tq O \in \Ort_d(\Rset) \} = \Rset^d$ \ queda
    claro para cualquier  \ $v \in \Rset^d$, \quad $\Esp[Y] v  = \mu_1 v$, dando
    que existe un \  $\mu_1$ \ tal que \ $\Esp[Y] = \mu_1  I$. Se puede hacer un
    paso m\'as  para identificar el  factor \ $\mu_1$  \ tomando la  traza.  Eso
    conduce  a   \  $\mu_1  =  \frac{1}{d}  \Esp[\Tr(Y)]   =  \frac{1}{d}  \un^t
    \Esp[\lambda(Y)]$.\newline De hecho, m\'as directamente, primero se nota que
    para  cualquier  matriz  de   permutaci\'on,  \  $\varphi_X(\pi  \lambda)  =
    ...
    (0)$  \ para  cualquier  permutaci\'on,  es decir  que  las componentes  son
...
\item El teorema  de mezcla uniforme~\ref{Teo:MP:MezclaUniforme} tambi\'en tiene
  ...
  \item $X \sim \ED_\izq(m,\Sigma,\varphi_X)  \:\: \Leftrightarrow \:\: X \egald
    ...
    $R_\izq^t R_\izq$ es  \'unica. Sin embargo, si $P(|X^t X| =  0) = 0$, existe
    una \'unica  matriz aleatoria triangular inferior (o  una \'unica triangular
    superior)    de    diagonal   no    negativa    que    satisfaga   a    esta
...
%
    ...
    independiente de $R_\vcol \ge 0$ escalar.
  %
  \item $X \sim \ED_\bil(m,\Sigma,\Sigma',\varphi_X) \:\: \Leftrightarrow \:\: X
    ...
    ejemplo.
%
  \item  \SZ{ME  TENGO  QUE  FIJAR   BIEN...   a\'un  que  sea  seguro  $X  \sim
      \ED_\simed(m,\Sigma,\varphi_X)   \:\:   \Leftrightarrow   \:\:  X   \egald
      \Sigma^{\frac12} \,  U \,  \Lambda_\simed \, U^t  \ \Sigma^{\frac12}  + m$
      \:\:  con  \:\:  $U  \sim  \U\left( \Ort_d(\Rset)  \right)$  ($d'  =  d$),
      ...
      descomposici\'on no es \'unica, salvo si imponemos que en $\Lambda_\simed$
      los autovalors sean de valor en orden decreciente por ejemplo.~\cite[Teo.~17.3, fin de \S~17.3.1, Teo.~17.6]{TemCal13}}
...
\item  Se  monstr\'o  que,  en  el  caso  $d'  =  1$  la  densidad  es  definida
  ...
  \item   Si  $X  \sim   \ED_\simed(m,\Sigma,\varphi_X)$  la   ley  de   $X$  es
   ...
   %
    ...
    \]
    %
    (ver~\cite[\S~3.1]{Met04} o~\cite[\S~9.3]{Car83} o~\cite[\S~2.4.2]{DeiGio09} para  el  determinente  de  la  jacobiana  de  la
    transformaci\'on  pasando   de  $X$   a  los  autovalores   y  autovectores,
    o~\cite[Teo.~13.3.1]{And03}).
