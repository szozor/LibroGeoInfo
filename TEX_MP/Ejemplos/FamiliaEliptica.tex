...
%
\begin{itemize}
\item \SZ{Buscar un que es izquierda,  pero no bilateral y no columna: $\ED_\izq
...
%
\item La ley  de Wishart (ver fin de  secci\'on~\ref{Sssec:MP:Wishart}) y la ley
  Student-$t$  matriz  variada  (ver fin  de  secci\'on~\ref{Sssec:MP:StudentT})
  partenecen a $\ED_\bil$  pero no son ni en $\ED_\vcol$  ni en $\ED_\vlin$ (ver
  ejemplos~\ref{Ej:MP:StudentTMatriz}  y~\ref{Ej:MP:WishartED}  m\'as adelante):
  $\ED_\bil \setminus \left( \ED_\vcol \cup \ED_\vlin \right) \neq \emptyset$.
%
\item $X$ gausiana matriz-variada (ver fin de secci\'on~\ref{Sssec:MP:Gaussiana}
  y   ejemplo~\ref{Ej:MP:GaussianaED}   m\'as   adelante)  partenece   a   ambas
...
\item Igualmente,  como en  el contexto vectorial,  cuando existe,  tambi\'en la
  ...
  \right|^{-\frac{d}{2}}  \, d_X\left(  \lambda\left( (x-m)^t  \Sigma^{-1} (x-m)
      \Sigma'^{-1}     \right)\right)    \qquad    \mbox{con}     \qquad    d_X:
  \Rset_+^{\min(d,d')} \mapsto \Rset_+
  \]
  %
  \end{itemize}
  %
  ...
  \,   \vcol$  \  respectivamente,   y  en   el  caso   bilateral,  \   $X  \sim
...
\item El teorema  de mezcla uniforme~\ref{Teo:MP:MezclaUniforme} tambi\'en tiene
  sus  contra-partes matriciales~\cite[Teorema~1]{FanChe84},  dependiendo  de la
  clase: Se supone ac\'a que $d' \le d$ y luego
  %
  \begin{itemize}
  ...
  \item $X \sim \ED_\vcol(m,\Sigma,\varphi_X) \:\: \Leftrightarrow \:\: X \egald
    ...
    $R_\col \ge 0$ escalar.
  %
  \item  \SZ{$X \sim  \ED_\bil(m,\Sigma,\Sigma',\varphi_X)  \:\: \Leftrightarrow
      \:\: X  \egald \Sigma^{\frac12} ???  \Sigma'^{\frac12} + m$ \:\:  con \:\:
      ???.}
  \end{itemize}
  %
...
\item  Se  monstr\'o  que,  en  el  caso  $d'  =  1$  la  densidad  es  definida
  ...
  \item Si $X \sim \ED_\izq(m,\Sigma,\varphi_X)$, la lei de $X$ es completamente
    ...
    \]
  %
    (ver tambi\'en~\cite[Teo.~7.9.2]{And03}).
  %
  \item Si $X \sim \ED_\col(m,\Sigma,\varphi_X)$, la lei de $X$ es completamente
    definida  por  la  lei  del  vector $R_\col  =  \begin{bmatrix}  (X_1-m_1)^t
      \Sigma^{-1}    (X_1-m_1)    &    \cdots   (X_{d'}-m_{d'})^t    \Sigma^{-1}
  ...
  \item   Si  $X  \sim   \ED_\vcol(m,\Sigma,\varphi_X)$,  la   lei  de   $X$  es
    completamente  definida   por  la  lei  de  $R_\vcol   =  \Tr\left(  (X-m)^t
      \Sigma^{-1}  (X-m)   \right)  =   \left\|  \Sigma^{-  \,   \frac12}  (X-m)
    \right\|_F$. Si existe la densidad, tiene la forma
    %
    \[
    p_{R_\vcol}(r) = \frac{2  \pi^{\frac{d d'}{2}}}{\Gamma\left( \frac{d d'}{2} \right)}
    \, r^{d d'-1} \, d_X\left( r^2 \right), \quad r \in \Rset_+
    \]
  %
  ...
  \end{itemize}
  %
\SZ{Se puede tener la forma a partir de $d_X$ en los casos $\izq$ y $\bil$?}
%
%  \item Teorema~\ref{Teo:MP:SchoenbergCaracteristica} en: \ $Z \sim \CED\left( m
%      , \Sigma , \varphi_Z \right) \egald \, A \, \Sigma^{\frac12} \, G + m \:\:
%    \Leftrightarrow \:\: \varphi_Z  \in \PDSI$, \ con \ $A  > 0$ \ independiente
...
%
% Distributions of the compound and scale mixture of vector and spherical matrix variate elliptical distributions January 2011Journal of Multivariate Analysis 102(1):143-152 DOI: 10.1016/j.jmva.2010.08.008 José A. Díaz-GarcíaJosé A. Díaz-GarcíaRamón Gutiérrez Jáimez
%
%  \item  Teorema~\ref{Teo:MP:SchoenbergDensidad} en:  \ $Z  \sim \CED\left(  m ,
%      \Sigma  , d_Z  \right)  \egald \,  A \,  \Sigma^{\frac12}  \, G  + m  \:\:
%    \Leftrightarrow \:\: d_Z \in  \CM$, \ con \ $A > 0$  \ independiente de \ $G
...
%
  ...
  secci\'on~\ref{Sssec:MP:Gaussiana}), de $\Tr  \left( (x-m)^t \Sigma^{-1} (x-m)
    \Sigma'^{-1}  \right) =  \Tr \left(  (x-m) \Sigma'^{-1}  (x-m)^t \Sigma^{-1}
  \right)$ se ve  inmediatamente que la matriz aleatoria  partenece a $\ED_\vcol
  \cap \ED_\vlin$  (ver tambi\'en~\cite{Daw81, FanKot99, FanNad04}).  Como en el
  caso escalar, el cuadrado del  rayo tiene una distribuci\'on Gamma, $R_\vcol^2
  \sim \G\left( \frac{d d'}{2} , \frac12 \right)$. \SZ{Que mas?}
\end{ejemplo}
...
%
  ...
  aleatoria  partenece  a  $\ED_\bil$.   Se  muestra  que  no  satisface  al  la
  extensi\'on matricial  del corollario~\ref{Cor:MP:MezclaUniforme}, punto (ii),
  as\'i que no esta ni en $\ED_\col$,  ni en $\ED_\lin$ y entonces tampoco ni en
  $\ED_\vcol$ ni en  $\ED_\vlin$ (ver~\cite[Ej.~1]{FanChe84} o~\cite[ejemplo del
  ec.~(2.3)]{Kar81}).  \SZ{La densidad  del  rayo, que  no  explicitamos ac\'a  (ver
  f\'ormula   de   calculo   dado   anteriormente),   tiene   una   forma   beta
  matricial genealizada~\cite[Cor.~9.3.12.2]{GupNag99}.}  \SZ{Que mas? GSM...}
\end{ejemplo}
...
\begin{ejemplo}[Wishart]\label{Ej:MP:WishartED}
  ...
  aleatoria partenece a $\ED_\simed$ pero no es ni en \SZ{$\ED_\bil$ y entonces ni en $\ED_\col$, ni en $\ED_\lin$
  y por consecuencia ni tampoco en $\ED_\vcol$ o en $\ED_\vlin$.  Que mas? GSM, rayo...}
\end{ejemplo}
