...
%  \end{enumerate}
  ...
  \item Para \ $X \sim \ED_\simed(m,\Sigma,\varphi_X)$,
    ...
    \]
    %
    % \SZ{Ver para obtener el coeficient
...
\item Para \ $X \sim \ED_\simed(m,\Sigma,\varphi_X)$ la covarianza de \ $X$ \ se
  ...
  para los coeficientes $\mu_2$ y $\sigma_1^2$.
\end{itemize}
%
...
\item  El  teorema~\ref{Teo:MP:ProyeccionComponentesEliptica}  de  proyeccion  y
  ...
  \quad  \forall \:  a \in  \Rset^d, \quad  a^t (X  - m)  \egald \sqrt{\frac{a^t
      \Sigma a}{\Sigma_{i,i}}} (X'_i - m'_i)$.  \SZ{A ver: para la reciproca, la
    matiz del  caso matricial  es que  $a$ no es  una matriz,  \ie no  tiene las
    domensiones de $X$, contrariamente a lo que pasaba en el caso vectorial.}
%  %
...
%
\item     \SZ{El     caso    de     mezcla     de     escala    de     Gausiana,
    ...
    literatura tiene algunos resultados.}
%  \item Teorema~\ref{Teo:MP:SchoenbergCaracteristica} en: \ $Z \sim \CED\left( m
%      , \Sigma , \varphi_Z \right) \egald \, A \, \Sigma^{\frac12} \, G + m \:\:
%    \Leftrightarrow \:\: \varphi_Z  \in \PDSI$, \ con \ $A  > 0$ \ independiente
%    de \ $G \sim \CN(0,I)$;
...
%
  ...
  % : $\|X\|_F^2$ es  nada m\'as que la suma de las  autovalores de $X X^t$ (ver
