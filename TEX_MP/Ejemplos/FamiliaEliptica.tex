...
\begin{lema}
  ...
  \item\label{Enum:MP:EDM_Transpuesta} $X \in \ED_\izq \:\: \Leftrightarrow \:\:
    ...
    \ED_\vlin$
  %
  \item\label{Enum:MP:EDM_Inclusiones}   $\ED_\vcol  \varsubsetneq   \ED_\col  \varsubsetneq
    \ED_\izq$ \qquad y \qquad $\ED_\vlin \varsubsetneq \ED_\lin \varsubsetneq \ED_\der$
  %
...
\item La ley  de Wishart (ver fin de  secci\'on~\ref{Sssec:MP:Wishart}) y la ley
  ...
  ejemplos~\ref{Ej:MP:WishartED}    y~\ref{Ej:MP:WishartED}   m\'as   adelante):
...
\item $X$ gausiana matriz-variada (ver fin de secci\'on~\ref{Sssec:MP:Gaussiana}
  y  ejemplo(ver~\ref{Ej:MP:GaussianaED}  m\'as  adelante))  partenece  a  ambas
...
\item Como  en el  contexto vectorial, la  funci\'on caracter\'istica  tiene una
  ...
  \varsubsetneq \ED_\bil$ la  funci\'on va a ser la suma de  las autovalores (ver caso
  $(\col)$ y por transposici\'on), \ie
  %
  \[
  \Phi_X(\omega) = e^{\imath \, \Tr\left( \omega^t m \right)} \, \varphi_x\left(
...
\item Igualmente,  como en  el contexto vectorial,  cuando existe,  tambi\'en la
  ...
  \right|^{-\frac{d}{2}} \, d_x\left( \lambda\left( (x-m)^t \Sigma (x-m) \Sigma'
    \right)\right)  \qquad \mbox{con}  \qquad d_X:  \Rset_+^{\min(d,d')} \mapsto
  \Rset_+
  \]
  %
  \end{itemize}
...
\item  El Teorema~\ref{Teo:MP:TranformacionAfinEliptica}  de  conservaci\'on del
  ...
  \item  Denotamos   por  \  $\bullet$  \   los  casos  \   $(\izq),  (\col)$  y
    ...
    de  un de  estos  tipos,  multiplicar por  $B$  a su  derecha  no cambia  la
    esfericidad.
  %
...
\item El teorema  de mezcla uniforme~\ref{Teo:MP:MezclaUniforme} tambi\'en tiene
  ...
  \item $X \sim \ED_\izq(m,\Sigma,\varphi_X)  \:\: \Leftrightarrow \:\: X \egald
    ...
    independiente de $R_\izq$, matriz aleatoria de $\Mat_{d',d'}(\Rset)$.
  %
  ...
  \item $X \sim \ED_\vcol(m,\Sigma,\varphi_X) \:\: \Leftrightarrow \:\: X \egald
    ...
    $R_\col \ge 0$ escalar.
  %
  \item \SZ{$X \sim \ED_\bil(m,\Sigma,\Sigma',\varphi_X) \:\: \Leftrightarrow \:\: X \egald
    \Sigma^{\frac12}   ??? \Sigma'^{\frac12}   +    m$   \:\:   con   \:\:   ???.}
  \end{itemize}
  %
...
\SZ{PONER ESO? Sea $X = \begin{bmatrix} X_1 & \cdots & X_{d'} \end{bmatrix} \sim
  ...
  que $\omega_k = w \un_{\{i\}}(k)$. Adem\'as, para $d' > 1$, \ $\left\{ X_i \tq
    X \sim \ED_\vcol(m , \Sigma ,  \varphi_X) \right\} \varsubsetneq \left\{ X_i \tq X
    \sim \ED_\bullet(m ,  \Sigma , \varphi_X) \right\} = \left\{  X_i \tq X \sim
    \ED_\bullet(m , \Sigma , \varphi_X)  \right\}$ (para $d' = 1$ son obviamente
  todos iguales).}
...
%
  ...
  variada, secci\'on~\ref{Sssec:MP:StudentT}, se ve inmediatamente que la matriz
  aleatoria  partenece a  $\ED_\col \cap  \ED_\lin$  (ver tambi\'en~\cite{Daw81,
    FanKot99, FanNad04}).

Se   muestra    que   no   satisface    al   la   extensi\'on    matricial   del
corollario~\ref{Cor:MP:MezclaUniforme}, punto (ii).
\end{ejemplo}

\begin{ejemplo}[Student-$t$ matriz variada]\label{Ej:MP:StudentTMatriz}
%
De  la forma  de la  densidad de probabilidad  en el  contexto Student-$t$
...
De  la forma  de la  densidad de probabilidad  en el  contexto Student-$t$
matriz variada,  secci\'on~\ref{Sssec:MP:Wishart}, se ve  inmediatamente que la
matriz  aleatoria partenece  a $\ED_\bil$  pero no  es ni  en $\ED_\col$,  ni en
$\ED_\lin$  y  entonces  tampoco  ni   en  $\ED_\vcol$  ni  en  $\ED_vlin$.
\end{ejemplo}
}
