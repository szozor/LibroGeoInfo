...
orden~\footnote{Una funci\'on  \ $f: I \subset  \Rset \mapsto \Rset$  \ es dicha
  ...
  $f(-x)$ \ es absolutamente monotona~\cite{Ber29}.}:
%
\[
\CM_n = \left\{ f: \Rset_+ \mapsto \Rset_+ \: \mbox{ continuas, diferenciables hasta
    el orden $n$ } \tq \forall \, k =  0 , \ldots , n, \:\: (-1)^k f^{(k)} \ge 0
\right\}
\]
%
y
%
\[
\CM = \bigcap_{n=0}^{+\infty}  \CM_n = \left\{ f: \Rset_+  \mapsto \Rset_+ \: \mbox{
    continuas, infinitemente  diferenciables }  \tq \forall \,  k \in  \Nset, \:\:
  (-1)^k f^{(k)} \ge 0 \right\}
\]
%
...
\SZ{Adelantar:  ley  de  $A$  via  Laplace,  Fourier  a  ver.  Volver  sobre  la
...
%
  ...
  escala de gausiana. \SZ{En calculos... Recordar $X/\sqrt{...}$}
\end{ejemplo}
...
%
\begin{teorema}[Mezcla de escala de base Student-r]
%
  ...
  %
  \[
  \varphi_X \in \PDSI_n, \: n > d \quad \Leftrightarrow X \egald B \, \Sigma^{\frac12} \, S_{m-2} + m
  \]
  %
...
%Sin perdida de generalidad, consideramos $m = 0, \: \Sigma = I$.
  ...
  \ED(0,I,\varphi_X)$ \ de  generadora caracter\'istica $\varphi_X$. Escribiendo
  $Y \egald  R U$ \  con \ $U  \sim \U(\Sset_n)$, y reciprocamente.   Ahora, del
  ejemplo~\ref{Ej:MP:UniformeCnt}, la proyecci\'on $d$-dimensional de \ $U$ \ es
  Student-r  con  $n-2$  grados  de  libertad, y  reciprocamente  se  puede  ver
  cualquier  Student-r con  $n-2$ grados  de  libertad como  proyecci\'on de  un
  uniforme  m\'as  grande.  Adem\'as,   $U$  siendo  independiente  de  $R$,  la
  proyecci\'on est tambi\'en independiente de $R$. Eso cierra la prueba.

  Alternativamente, escribiendo  la generadora  caract\'eristica a partir  de la
  mezcla uniforme $n$-dimensional como en el teorema~\ref{Teo:MP:SchoenbergCaracteristica}, en
  %
  \[
...
\SZ{Adelantar: mezcla de Student-r: Keilson KeiSte74 \S~3.3, Williamson Wil56 \S~2 para $d=1$.}

Nota: obviamenre, si denotamos \  $\mathfrac{S}_{n,d}$ \ el conjunto de vectores
aleatorios  $d$-dimensjonal,  mezcla  de  Student-r  con \  $n-2$  \  grados  de
libertad, $\mathfrac{S}_{d+1,d} \supset \mathfrac{S}_{d+2,d} \supset \cdots$ \ y
...
\item GSM (y consistante) %
  ...
  GSM~\cite{Yao94}. Formal~\cite{Kan94, Yao73, Ver64, Pic70, Kel71, KeiSte74, Tei60, AndMal74}, Schoenber~\cite{Sch38, SteVan05}, Laplace transfo~\cite{Fel71, Wil56, Wid46}, Appli~\cite{PorStr03, BomBea08, Sel08, ShiSel07, ZozVig10, TisNic04, TodTab07}
