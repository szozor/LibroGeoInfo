...
%
\begin{itemize}
\item Sea  \ $S$  \ vector aleatorio  de distriibuci\'on  esf\'erica, y \  $T$ \
  vector  aleatorio \  $d'$-dimensional independiente  de \  $S$. Sea  \ $X  = S
  T^t$. Inmediatamente,  de \ $O S  \egald S$ \  para cualquier matriz \  $O \in
...
\item La  extensi\'on del teorema~\ref{Teo:MP:MomentosCumulantesEliptica}  y del
  ...
  \item Para  $X \sim \ED_{\izq,\col,\vcol}(m,\Sigma,\varphi_X)$  \ o \  $X \sim
    ...
    Y$ \ para cualquier matriz ortogonal, se obtiene tambi\'en \ $- Y \egald Y$,
  ...
  \item Para \ $X \sim \ED_\simed(m,\Sigma,\varphi_X)$,
    %
    \[
    \Esp[X]  = m  + \mu_1  \Sigma,  \qquad \mu_1  = -  \imath \,  \frac{\partial
      \varphi_X}{\partial  \lambda_1}(0)  =   \frac{1}{d}  \,  \un^t  \Esp\left[
      \lambda(Y) \right]
    \]
    %
  ...
  %
  \item Tomando la forma de $\Phi_X$ en cada caso, y de la formula relaciando la
    funci\'on  caracter\'istica y  los momentos,  se obtiene  las formas  de las
    covarianzas de $Y$. Adem\'as, a  partir de estas formas, los terminos usando
  ...
  %
  \[
  \Cov[X] =  \tau_{(2,3)} \big[  \Sigma \otimes \alpha(\varphi_X)  \big], \qquad
  ...
  %
  \[
  \Cov[X]  =  \tau_{(2,3)} \big[  \Sigma  \otimes \Diag\left(  \alpha(\varphi_X)
  \right)  \big],   \qquad  \alpha(\varphi_X)   =  -  \nabla_r   \varphi_X(0)  =
  ...
  %
  \[
  \Cov[X] = \alpha(\varphi_X) \, \tau_{(2,3)} \big[ \Sigma \otimes I_{d'} \big],
  \qquad \alpha(\varphi_X)  = - \varphi_X'(0)  = \frac{1}{d d'} \Esp\left[  \| Y
    \|_F^2 \right]
  ...
  %
  \[
  \Cov[X]  =  \alpha(\varphi_X) \,  \tau_{(2,3)}  \big[  \Sigma \otimes  \Sigma'
  \big], \qquad  \alpha(\varphi_X) =  - 2 \,  \frac{\partial \varphi_X}{\partial
  ...
  prueba  de la  forma de  la covarianza  es  un poco  m\'as sutil  en la  clase
  $\ED_\bil$. Primero, se nota que  para cualquier matriz de permutaci\'on, como
  lo  vimos tratando  del  caso  $(\simed)$ \  $\nabla_\lambda  \varphi_X (0)  =
  \frac{\partial  \varphi_X(0)}{\partial  \lambda_1}  \:  \un$.   Con  el  mismo
  enfoque, se nota tambi\'en que para cualquier matriz de permutaci\'on $\pi$, \
  $\pi      \Hess_\lambda      \varphi_X(0)      \pi^t      =      \Hess_\lambda
...
\item Para \ $X \sim \ED_\simed(m,\Sigma,\varphi_X)$ la covarianza es dada por:
  %
  \[
  \Cov[X] = \mu_2 \, \Sigma  \otimes \Sigma + \sigma_1^2 \Big( \tau_{(2,3)}\big[
  \Sigma \otimes  \Sigma \big] +  \tau_{(2,4)}\big[ \Sigma \otimes  \Sigma \big]
  \Big)
