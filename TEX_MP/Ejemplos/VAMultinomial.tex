...
\sum_{i=1}^{k-1} x_i \le n \right\}$.\label{Foot:MP:MultinomialDominio}} & $\X = \Part{n}{k}$
...
Su    masa     de    probabilidad     es    representadas    en     la    figura
...
\leyenda{Ilustraci\'on de una distribuci\'on  de probabilidad multinomial para \
  ...
  dibujada solamente la distribuci\'on  sobre $\widetilde{\X}$, siendo esta nula
  afuera  de $\widetilde{\X}$.   Los  par\'ametros  son \  $n  = 5$  \  y  \ $p  =
