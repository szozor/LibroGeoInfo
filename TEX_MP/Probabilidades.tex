...
ocurrencia  $\frac{k}{K}$ del  atributo\modif{~\cite{Ber1713,  Ber1713:2, Dem56}
  o~\cite[\& Ref.]{Bra76, Hal90, ShaVov06}.}
...
teor\'ia  de conjuntos \modif{(ej.~\cite{Spi76,  Bre88, ManWol95,  Sie75, Sie76,
    Bor98, Bor09}):}
...
\  (ver  ej.~\cite{Jef48,  Jef73,   Hal50,  Fel71,  Bre88,  ManWol95,  ManWol95,
  IbaPar97, LehCas98, AthLah06, Coh13}).

\begin{figure}[h!]
\begin{center} \input{TIKZ_MP/Ensembles} \end{center}
%
\leyenda{\modif{Ilustraci\'on   de  la  operaciones   de  uni\'on   $A  \cup   B$  (a),
...
\modif{Formalmente,   se  define   de  manera   abstracta  un   espacio  medible
  $(\Omega,\A)$ de  la manera siguiente~\cite{Hal50,  Bre88, IbaPar97, AthLah06,
    Coh13} (ver  tambi\'en~\cite[\& Ref.]{BarNov78, Bor98,  Sie18, Sie75, Sie76}
  para notas hist\'oricas):
%
...
los siguientes \modif{(ej.~\cite{Spi76, Kol56, ShaVov06, Pla05}):}
...
\item si $A$ es subconjunto de $B$, \modif{lo que escribiremos $A \subset B$, es
    ...
    rev\'es)},     entonces    \    $P(A)     \leq    P(B)$;  \modif{   Es    ilustrado
  figure~\ref{fig:MP:CompletoSub};}
\end{itemize}
...
%
\leyenda{\modif{Ilustraci\'on  de  conjunto completo  de  eventos posibles  excluyentes
...
{\it ley de composici\'on} o \modif {\it formula de inclusi\'on-exclusi\'on} es
%
\[
...
\modif{Se  prueba  tambi\'en  de  que  si $\{  A_i  \}_{i=1}^{+\infty}$  es  una
  secuencia creciente  de eventos, \ie $\forall  \, i \ge 1,  \quad A_i \subset
  A_{i+1}$, entonces
%
\[
...
\begin{definicion}[Probabilidad condicional]
  \modif{  \underline{Por definici\'on}, l}a  {\it probabilidad  condicional} de
  $A$ dado  $B$ es  la raz\'on entre  la probabilidad  del evento conjunto  y la
  probabilidad de que se d\'e $B$ (cuando \'este es un evento no nulo):
  %
  \[
...
probabilidad~\footnote{ \modif{  se puede definir un espacio  de probabilidad $(
  ...
  veces en la literatura se  la denota $P_B(A)$}. \modif{Diversas situaciones de
  probabilidades   condicionales  son   ilustradas  en   la   figura  siguiente,
  Fig.~\ref{fig:MP:Condicional}.

\begin{figure}[h!]
\begin{center} \input{TIKZ_MP/Condicional} \end{center}
%
\leyenda{\modif{Ilustraci\'on  de  la probabilidad  condicional  con  $A$ interior  del
...
\item {\it f\'ormula de Bayes}: \ si $\{ B_j \}$ es un conjunto completo
  ...
  \]
  %
\modif{(ver~\cite{Bre88, Bay63, Bar58}).}
\end{itemize}
...
\modif{ Terminamos  esta secci\'on  por la noci\'on  de independencia  entre dos
  ...
  ``informaci\'on'' sobre el otro~\cite{Bre88, ManWol95, Hau01, Bor09}:
%
