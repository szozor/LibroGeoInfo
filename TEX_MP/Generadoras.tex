...
\begin{proof}
  ...
  \ $\displaystyle G_X(z) =  \sum_{\modif{x} \in \Nset^d} \left( \prod_{i=1}^d z_i^{\modif{x_i}}
  \right)  P(X   =  \modif{x})$  \   con  \  $\modif{x}   =  \begin{bmatrix}  \modif{x_1}  &   \cdots  &
    \modif{x_d}  \end{bmatrix}^t$.  A  continuaci\'on,  se nota  que  la serie  converge
...
%
\modif{
\begin{teorema}[Propiedades principales de la funci\'on caracter\'istica]\label{Teo:MP:PropCarac}
%
\begin{enumerate}
...
\begin{teorema}
  ...
  $\Phi_X$.   Sea  \ $A  =  \optimes_{i=1}^d (a_i  \;  b_i)  \in \B(\Rset^d)$  y
  $\partial   A   =   \optimes_{i=1}^d    [a_i   \;   b_i]   \setminus   A$   su
  borde. Entonces~\footnote{Se  prolonga la  funci\'on \ $\frac{e^{-  \imath a_j
        \omega_j}  -  e^{-  \imath  b_j  \omega_j}}{\imath  \omega_j}$  \  en  \
    $\omega_j  = 0$ \  por su  l\'imite \  $\displaystyle \lim_{\omega_j  \to 0}
    \frac{e^{-  \imath  a_j  \omega_j}   -  e^{-  \imath  b_j  \omega_j}}{\imath
      \omega_j} = b_j - a_j$.},
  %
  \[
  \lim_{w \to +\infty} \frac{1}{(2 \pi)^d}  \int_{[-w \; w]^d} \left( \prod_{j =
      1}^d \frac{e^{-  \imath a_j \omega_j} - e^{-  \imath b_j \omega_j}}{\imath
      \omega_j} \right) \Phi_X(\omega) \, d\omega = \int_{\Rset^d} \prod_{j=1}^d
  \left( \un_{\left( a_j \; b_j  \right)}(x_j) + \frac12 \un_{\left\{ a_j \; b_j
  ...
  \[
  \lim_{w \to +\infty} \frac{1}{(2 \pi)^d}  \int_{[-w \; w]^d} \left( \prod_{j =
      1}^d  \frac{e^{- \imath  a_j  \omega_j}  - e^{-  \imath  b_j \omega_j}}{\imath  \omega_j}
  \right) \Phi_X(\omega) \, d\omega = P_X(A).
  \]
\end{teorema}
%
Nota: el l\'imite $\displaystyle\lim_{T \to +\infty} \int_{[-T \; T]^d}$ se nota
a  veces  $\displaystyle \operatorname{vp}  \int_{\Rset^d}$,  integral {\it  en
  valor principal}.
%
\begin{proof}
  ...
  %
  \[
  \int_{[-w \;  w]^d} \left( \prod_{j =  1}^d \frac{e^{- \imath  a_j \omega_j} -
      e^{- \imath  b_j \omega_j}}{\imath \omega_j} \right)  \Phi_X(\omega) \,
  d\omega  = \int_{[-w \;  w]^d} \left(  \prod_{j =  1}^d \frac{e^{-  \imath a_j
        \omega_j}  -  e^{- \imath  b_j  \omega_j}}{\imath  \omega_j} \right)  \,
  \int_{\Rset^d} e^{\imath \omega^t x} \, dP_X(x) \, d\omega
  \]
  %
  Ahora, notando que  $\left| \frac{e^{- \imath a_j \omega_j}  - e^{- \imath b_j
        \omega_j}}{\imath \omega_j}  \, e^{\imath \omega^t x} \right|  \le b_j -
  a_j$   \   uniformamente   acotado.    Se   puede   evocar   el   teorema   de
  Fubini~\ref{Teo:MP:Fubini}  para  intercambiar   las  integrales,  as\'i  que,
  notando que $e^{\imath \omega^t x} = \prod_{j=1}^d e^{\imath \omega_j x_j}$,
  %
  \[
  \int_{[-w \;  w]^d} \left( \prod_{j =  1}^d \frac{e^{- \imath  a_j \omega_j} -
      e^{- \imath  b_j \omega_j}}{\imath \omega_j} \right)  \Phi_X(\omega) \,
  d\omega  = \int_{\Rset^d} \left(  \prod_{j=1}^d \int_{-w}^w  \frac{e^{- \imath
        \omega_j  (a_j -  x_j)}  -  e^{- \imath  \omega_j  (b_j -  x_j)}}{\imath
      \omega_j} \, d\omega_j \right) \, dP_X(x)
  \]
  %
  Se nota que
  %
  \begin{eqnarray*}
  \int_{-w}^w \frac{e^{- \imath \omega_j (a_j - x_j)} - e^{- \imath \omega_j
  (b_j - x_j)}}{\imath \omega_j} \, d\omega_j & = & - \int_{-w}^w \frac{e^{+
  \imath \omega_j (a_j - x_j)} - e^{- \imath \omega_j (b_j - x_j)}}{\imath
  \omega_j} \, d\omega_j\\[2mm]
  %
  & = & \int_{-w}^w \frac{\sin(\omega_j (b_j - x_j)) - \sin(\omega_j (a_j -
  ...
  es  decir,  de  $\displaystyle  \lim_{T \to  +\infty}  \int_{-T}^T  \frac{\sin
    \omega}{\omega} d\omega = \pi$~\SZ{\cite{toto}}, se obtiene
  ...
  teorema     de     convergiencia    dominada~\ref{Teo:MP:ConvergenciaDominada}
  permitiendo   permutar   integral  y   l\'imite,   y   de  $\sign(b_j-x_i)   -
  \sign(a_j-x_i) = 2 \, \un_{\left(  a_j \; b_j \right)}(x_j) + \un_{\left\{ a_j
      \; b_j \right\}}(x_j)$.
\end{proof}
 }

\SZ{caso con densidad y se cuadrado integrable; uno-uno casi siempre}
