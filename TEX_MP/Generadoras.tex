...
%
  Para cualquier \  $k = \begin{bmatrix} k_1 & \cdots  & k_d \end{bmatrix}^t \in \modif{\Part{K,d}}$
%  \Nset^d$ \ con \ $K =  \sum_{i=1}^d k_i$, 
derivando $M_X$ se proba que, cuando
  existen
  %
  \[
...
1889~\cite{Thi89} y aparecen  bajo esta denominaci\'on en un  papel de R. Fisher
\& J.   Wishart 4 decadas despu\'es~\cite{FisWis32}.  Estos  momentos aparecen a
trav\'es del logaritmo de la funci\'on  generadora de momentos.  Como lo vamos a
ver  en  la  seci\'on~\ref{Ssec:MP:FamiliaExponencial}  tratando de  la  familia
exponencial, aparece  que el  logarotmo de la  generadora de momentos  tiene una
significaci\'on f\'isica,  permitiendo de calcular  energias libre o  interna en
f\'isica estadistica a trav\'es de lo que conocido como funci\'on de partici\'on
(ver~\cite{Gib01,  Gib02,  Fis29}). Se  refiera  tambi\'en a~\cite{Luc70,  titi,
...
\begin{definicion}[Cumulantes]
  ...
  %
  \[
  C_X(u)     =    \sum_{K=1}^{+\infty}     \,     \sum_{k    \in     \Part{K,d}}
  \kappa_{k_1,\ldots,k_d} \, \frac{u_1^{k_1} \ldots u_d^{k_d}}{K!}
  \]
  %
  donde  el  tensor \  $\kappa_K$  \  de  order \  $K$  \  y de  componentes  $k
  \in \Part{K,d}$ llamados {\em cumulentes}
  %
  \[
  \kappa_{k_1,\ldots,k_d} \, = \, \left.\frac{\partial^K C_X}{\partial u_1^{k_1}
      \ldots \partial u_d^{k_d}} \right|_{u = 0}
  \]
  %
...
%
\item $\kappa_2 = \Hess C_X(0) = \Sigma_X = \zeta_2$ \ covarianza de $X$;
%
\item $\kappa_3 = \zeta_3$ \ momento centrado de orden 3;
%
\item Pero, $\forall K > 3, \quad \kappa_K \ne \zeta_K$.
\end{itemize}

Sin embargo, en  el caso escalar \  $d=1$ \ de \ $m_k  = \left. \frac{\partial^k
    M_X}{\partial     u^k}    \right|_{u=0}    =     \left.     \frac{\partial^k
    \exp(C_X)}{\partial   u^k}   \right|_{u=0}$   \   o   $\kappa_k   =   \left.
...
\begin{lema}[Relaci\'on momentos-cumulantes]
  Sea  \ $X$ \  variable aleatorias  escalares. Cuando  existen, los  momentos y
  cumulantes son relacionados por las formulas
%
\[\left\{\begin{array}{lll}
m_k & = & \displaystyle \sum_{n=1}^k B_{k,n}( \kappa_1 , \ldots , \kappa_{n-k+1}) \\[2mm]
%
\kappa_k & = & \displaystyle \sum_{n=1}^k (-1)^n (n-1)! B_{k,n}( m_1 , \ldots , m_{n-k+1}) \\[2mm]
\end{array}\right.\]
%
Notando que \ $C_{X-m_X}(u) = -u m_X + C_X(u)$ \ se obtiene inmediatamente
%
\[\left\{\begin{array}{lll}
\zeta_k & = & \displaystyle \sum_{n=1}^k B_{k,n}( 0 , \kappa_2 , \ldots , \kappa_{n-k+1}) \\[2mm]
%
\kappa_k & = & \displaystyle \sum_{n=1}^k (-1)^n (n-1)! B_{k,n}( 0 , \zeta_2 , \ldots , \zeta_{n-k+1}) \\[2mm]
\end{array}\right.\]
\end{lema}
%
...
bastante pesados, usando el mismo enfoque de derivada de funciones compuestas.
...
\begin{definicion}[Secunda funci\'on caracter\'istica]
  ...
  %
  \[
  \Psi_X(\omega) = \log \Phi_X(\omega) = \log\left( \Esp\left[ e^{\imath \omega^t X} \right] \right)
  \]
\end{definicion}
%
...
relaci\'on siguiente:
%
\begin{lema}[Cumulantes a partir de la secunda funci\'on caracter\'istica]
  ...
  %
  \[
  \kappa_{k_1,\ldots,k_d}    \,   =   \,    (-\imath)^K   \left.\frac{\partial^K
      \Psi_X}{\partial    \omega_1^{k_1}    \ldots   \partial    \omega_d^{k_d}}
  \right|_{\omega   =    0},   \qquad    \begin{bmatrix}   k_1   &    \cdots   &
