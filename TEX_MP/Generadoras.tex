...
anillos~\footnote{Para \ $i =  1, \ldots, d$, sean los reales \  $r_i, \: R_i$ \
  ...
  $\rho_{j_i} = r_i \uno_{\{-1\}}(j_i)  + R_i \uno_{\{+1\}}(j_i)$.  Supongamos que
...
%
  ...
  momento  factorial~\footnote{Recuerdense que $\PocD{x}{n}  = \prod_{i=0}^{n-1}
    (x-i), \quad n > 0$ \  s\'imbolo de Pochhammer, con la convenci\'on $(x)_0 \igualc
...
%
  ...
  %
   \displaystyle \int_{\Rset^d} \prod_{j=1}^d \left(
     \uno_{\left(  a_j \;  b_j \right)}(x_j)  +  \frac12 \uno_{\left\{  a_j \;  b_j
...
\begin{proof}
  ...
  \uno_{\left(   a_j   \;  b_j   \right)}(x_j)   +   \uno_{\left\{   a_j  \;   b_j
    \right\}}(x_j)$.
\end{proof}
...
\begin{proof}
  ...
  \uno_{x_j}(y_j)$, lo cierra la prueba
\end{proof}
...
\begin{teorema}\label{Teo:MP:IgualdadDistribucionFuncionVA}
  ...
  %
  \[
  X  \iguald  Y \quad  \Longrightarrow  \quad  f(X) \, \iguald \, f(Y)
  \]
\end{teorema}
%
\begin{proof}
