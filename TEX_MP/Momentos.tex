...
\item  \modif{En el  caso escalar,}  el  cuarto momento  central da  lugar a  la
  ...
  ejemplo, \ $\mu_2=\nu_2-\nu_1^2$ \ \modif{que  es nada m\'as que la relaci\'on
    de           K\"onig-Huyggens},           mientras           que           \
  $\mu_3=\nu_3-3\nu_1\nu_2+2\nu_1^3$.  \modif{En  el  contexto multivariado,  la
    relaci\'on momentos-cumulantes toma la expresi\'on
  %
    \[
    \mu_{r_1,\ldots,r_d}  = \sum_{s_1  =  0}^{r_1} \cdots  \sum_{s_d =  0}^{r_d}
    \left( \prod_{i=1}^d  \binom{r_i}{s_i} \left( -  \mu_{X_i} \right)^{r_i-s_i}
    \right) \nu_{s_1,\ldots,s_d}
    \]
}
\end{itemize}
...
%
\[
\Sigma_{X,Y} \equiv  \Cov[X,Y] =  \Esp\left[ \left( X  - \mu_X\right) \left(  Y -
