...
>{\vspace{-2mm}\hspace{2mm}}p{.55\textwidth}|
}
%
\hline
%
...
\footnotetext{En el caso  l\'imite \ $z \to  1$, \ $\lim_{z \to 1}  \frac{ z^a -
    z^{b+1}}{1-z} =  b+1-a$ \  y similarmente para  la generadora de  momentos y
  funci\'on caracter\'istica (resp. \ $u \to 0$ \ y \ $\omega \to 0$).}
...
con  \  $n  \in  \Nset^*$  \  y  \  $p  =  \begin{bmatrix}  \end{bmatrix}^t  \in
\Simp_{k-1}$ \ the \ $(k-1)$-simplex estandar,
%
\[
\Simp_{k-1} = \left\{ u \in [ 0 \; 1 ]^k,  \: \sum_i u_i = 1 \right\}
\]
%
...
\hline
%
Dominio de definici\'on~\footnotemark & $\X = \left\{ m \in \{ 0 \, , \, \ldots \, , \, n\}^k, \:
\sum_{i=1}^k m_i = n \right\}$\\[2mm]
\hline
%
...
\SZ{Su masa  de probabilidad  y funci\'on de  repartici\'on son representadas  en la
figura Fig.~\ref{Fig:MP:Multinomial}.
%
\begin{figure}[h!]
%\begin{center} \input{TIKZ_MP/BinomialNegativa} \end{center}
%
  \leyenda{Ilustraci\'on de una  distribuci\'on de probabilidad Multinomial (a),
    y la funci\'on de repartici\'on asociada (b), con $n = 5, \quad k = 3, \quad
    p    =    \protect\begin{bmatrix}    \frac25    &    \frac15    &    \frac15
...
Esta distribuci\'on  es una extensi\'on  multivariada de las variables  beta. Se
nota \ $X \, \sim \, \Dir(a)$ \ con \  $a \in \Rset_+^{* \, k}$ \ y \ $X$ \ vive
sobre el $(k-1)$-simplex  estandar \ $\Simp_{k-1}$. $a$ es  llamado parametro de
forma. Como en el caso de vectores de distribuci\'on multinomial, a pesar de que
se escribe \ $X$ \ de manera $k$-dimensional, el vector partenece a una variedad
\ $d = k-1$ \ dimensional y en el caso  \ $k = 2$ \ se recupera la ley beta. Las
caracteristicas de un vector de Dirichlet son:
%
\begin{center}
\begin{tabular}
{
...
\hline
%
Dominio de definici\'on~\footnotemark & $\X = \Simp_{k-1}, \: k \in \Nset \setminus \{ 0 \, , \, 1 \}$\\[2mm]
\hline
%
Parametros & $a \in \Rset_+^{* \, k}$ (forma)\\[2mm]
\hline
%
Densidad   de    probabilidad   &   $\displaystyle    p_X(x)   =   \frac{\prod_{i=1}^k x_i^{a_i-1}}{B(a)}$\\[2mm]
\hline
%%
%%Momentos & $ \Esp\left[ X^k \right] = p^k$\\[2mm]
%%\hline
...
%%\hline
%%
Promedio & $\displaystyle m_X = \frac{a}{\sum_{i=1}^k a_i} \equiv \tilde{a}$\\[2.5mm]
\hline
%
Covarianza & $\displaystyle \Sigma_X = \frac{\diag\left( \widetilde{a} \right) -
\tilde{a} \tilde{a}^t}{1 + \sum_{i=1}^k a_k}$\\[2.5mm]
\hline
%%
%Asimetr\'ia & $\displaystyle \gamma_X = \frac{2 \, (b - a) \sqrt{a + b + 1}}{( a
%+ b + 2) \sqrt{a b}}$\\[2mm]
...
\footnotetext{De  hecho,  se puede  considerar  que  el  vector aleatorio  es  \
  ...
  hipertriangulo \  $\widetilde{\X} = \left\{ \widetilde{x} \in  [0 \; 1]^{k-1},
    \:  \sum_{i=1}^{k-1}  \widetilde{x}_i  \le  1  \right\}$.   Su  densidad  de
...
%
\leyenda{Ilustraci\'on del dominio  $\X$ de definici\'on de la  ley de Dirichlet
  para  $k   =  3$  (a),  y   densidad  de  probabilidad   de  $\widetilde{X}  =
  \protect\begin{bmatrix}   X_1  &  X_2   \protect\end{bmatrix}^t$  \   ($X_3  =
  1-X_1-X_2$)  \ sobre  \ $\widetilde{\X}$  \ con  las marginales  (ver  nota de
  pie~\ref{Foot:MP:Dirichlet}). Los parametros son \ $a = \protect\begin{bmatrix} 3 & 2 &
    2 \protect\end{bmatrix}^t$  (b) y \ $a  = \protect\begin{bmatrix} 3 &  1 & 2
...
\Rset_+^{* \, k}$ \ y \ $J^{(i,j)} = \begin{bmatrix} I_{j-1} & \un_i & 0 \\[1mm]
  0 & 0 & I_{k-j}\end{bmatrix}$ \ matriz \ $(k-1) \times k$ \ donde \ $I_l$ \ es
la identidad $l \times l$, $\un_i$ el vector con component $i$-\'esima igual a 1
(0 si no) y $0$ matrices de 0 (todas de tama\~no adcuado).  En \ $J^{(i,j)} X$ \
se saca la  componente $j$-\'esima, se re-emplaza la  componente $i$-\'esima por
la suma $X_i + X_j$ \ y se conservan las otras componentes. Entonces,
  % , \ie $J^{(i,j}_{l,l} = 1, \: 1 \le l < j, \: J^{(i,j}_{l,l+1} = 1, \: j \le
  % l  < k,  \: $.  Entonces, para  cualquier $1  \le i  < j  \le k$,  notando \
  % $\widetilde{X}$ \ el vector de  componentes $\widetilde{X}_l = X_l, \: 1 \le
  % l < j,  \, l \ne i$, \  $\widetilde{X}_i = X_i + X_j$,  \ $\widetilde{X}_l =
  % X_{l+1}, \: j+1 \le k-1$,
  %
  \[
...
\begin{proof}
  ...
  lema~\ref{Lem:MP:VinculoBetaGamma}.
\end{proof}

\SZ{
densidad con respeto a la medida de Haar? sobre el simplex; ver tilde X, 1 dim continuo sobre hipertriangulo

Ref. Dirichlet, Liouville 1839

Conjugate priori of a multinomial (beta es para la binomial; ver Robert por ejemplo)
