\SZ{Poner antes la notation $P_d^+(\Rset)$}
...
%%
Coarianza &
%  $\displaystyle \sigma_X^2  = \frac{a b}{(a  + b)^2  (a + b  + 1)}$
\SZ{A ver bien}\\[2mm]
\hline
%%
...
%\hline
%%
Funci\'on caracter\'istica~\footnote{Se puede salir de la
vectorizaci\'on~\cite{Har08} de \ $X$, \ie poniendo las columnas de una bajo la
la otra, y usar la definici\'on usual. Eso es equivalente a definirla como \
$\Phi_X(\omega) = \Esp \left[ e^{\imath \Tr\left( \omega^t X \right)} \right]$ \
con \ $\omega \in \Rset^{d \times d}$~\cite{And03}. \SZ{Pasando, se proba que la
funci\'on generadora de momentos no existe, excepto en el productos de ejes
imaginarios puros}.\label{Foot:MP:CaracteristicaWishart}} & $\displaystyle
\Phi_X(\omega) = \Esp \left[ e^{\imath \Tr\left( \omega^t X \right)} \right] =
\left| I - 2 \imath \omega^t V \right|^{-\frac{\nu}{2}}, \quad \omega \in
\Rset^{d \times d}$\\[2mm]
\hline
\end{tabular}
\end{center}
...
\frac{d-1}{2}$, \ es la funci\'on gamma multivariada~\cite{And03, GupNag99}.
...
Inmediatamente, si  $d = 1$,  la distribuci\'on de  Whishart \ $W_1(V,nu)$  \ se
reduce  a  la  distribuci\'on   Gamma  $\G\left(\frac{\nu}{2}  \,  ,  \,  V^{-1}
\right)$. De este  hecho, se la podr\'ia ver como  extensi\'on matriz-variada de la
distribuci\'on  gamma.   La  distribuci\'on   de  Wishart  tiene   varias  otras
...
%
  ...
  En particular, si $d'  = 1$, \ $A^t X A \, \sim  \, G\left( \frac{\nu}{2} \, ,
    \, \left( A^t V A \right)^{-1}  \right)$. M\'as all\'a, tomando $A = \un_j$,
  aparece de  que las componentes  diagonales de \  $X$ \ son  de distribuci\'on
  gamma, \  $X_{j,j} \, \sim \,  \G\left( \frac{\nu}{2} \,  , \, \frac1{V_{j,j}}
  \right)$.
\end{lema}
%
\begin{proof}
...
de  los elementos no-diagonales~\cite{Seb04}  o~\cite[Teo.~3.3.4]{GupNag99}.  De
eso resuelte delicado llamar la distribuci\'on como gamma matriz-variada.
...
\sim \, \N(0, A^t V A)$ \ son independientes~\cite{Seb04}.
