...
%%\hline
%%
Promedio\footnote{Para calcularlas, lo m\'as sencillo es salir de la funci\'on
caracter\'istica y ver que \ $\frac{\partial \Phi_X}{\partial \Omega_{i,j}} =
\imath \Esp\left[ X_{i,j} \right]$ \ (cuidado con las simetrias) y usar las
reglas de derivaci\'on matricial de un determinente~\cite[Cap.~8]{MagNeu99}.} &
$\displaystyle m_X = \nu \, V$\\[2mm]
\hline
%%
Covarianza~\footnote{Para calcularlas, lo m\'as sencillo es salir de nuevo de la
funci\'on caracter\'istica y ver que \ $\frac{\partial^2 \Phi_X}{\partial
\Omega_{i,j} \partial \Omega_{k,l}} = \imath \Esp[X_{i,j} X_{k,l}]$ \ y usar las
reglas de derivaci\'on matricial de un determinente~\cite[Cap.~8]{MagNeu99}} &
$\displaystyle \Cov\left[ X_{i,j} \, , \, X_{k,l} \right] = \nu \left( V_{i,k}
V_{j,l} + V_{i,l} V_{j,k} \right)$\\[2mm]
\hline
%%
...
vectorizaci\'on~\cite{Har08} de \ $X$, \ie poniendo las columnas una bajo la
...
\Tr\left( X \Omega \right)} \right]$ \ con \ $\Omega \in S_d(\Rset)$ conjunto de
matrices reales s\'imetricas de $\Rset^{d \times d}$~\cite{PedRic91,
And03}. \SZ{Pasando, se proba que la funci\'on generadora de momentos no existe
en general}.\label{Foot:MP:CaracteristicaWishart}} & $\displaystyle
\Phi_X(\Omega) = \Esp \left[ e^{\imath \Tr\left( X \Omega \right)} \right] =
\left| I - 2 \imath \Omega^ V \right|^{-\frac{\nu}{2}}, \quad \Omega \in
S_d(\Rset)$\\[2mm]
\hline
\end{tabular}
\end{center}
