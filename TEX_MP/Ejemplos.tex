...
%\hline
%
Promedio & $ m_X = \frac12 \begin{bmatrix} 1 & \cdots & 1 \end{bmatrix}^t$\\
\hline
%
Covarianza & $\Sigma_X = \frac1{12} I$\\
\hline
%
Asimetr\'ia (caso escalar) & $\gamma_X = 0$\\
\hline
%
Curtosis (caso escalar) & $\kappa_X = -\frac65$\\
\hline
%
%Generadora de probabilidad & $G_X(z) = e^{\lambda (z-1)}$ \ para \ $z \in \Cset$\\
%\hline
%
Generadora de momentos  & $M_X(u) = \prod_{k=1}^d \frac{  e^{u_k} - 1}{u_k}$ \
para~\footnote{Con el l\'imite $\lim_{u_k \to 0} \frac{ e^{u_k} - 1}{u_k} = 1$.}
\ $u \in \Cset^d$\\ \hline
%
Funci\'on caracter\'istica & $\Phi_X(\omega) = (-\imath)^d \prod_{k=1}^d \frac{ e^{\imath \omega_k} - 1}{\omega_k}$\\
\hline
\end{tabular}
\end{center}
%
...
de repartici\'on asociada  (b). $\alpha = 0.5$ (linea  punteada), $1$ (guiones),
