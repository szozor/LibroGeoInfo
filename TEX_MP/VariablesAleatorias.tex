...
A  continuaci\'on,  damos unos  teoremas  que  ser\'an  muy \'util  m\'as
  adelante, sin detallar las pruebas. Por eso, se puede referirse a~\cite{toto}.
%
...
la  integraci\'on. Este  teorema  dice dan  una  condici\'on simple  permitiendo
intercambia integraci\'on y limite.
...
\modif{Fijense  de  que dos  funciones  pueden  cumplir  esta definici\'on,  por
  ejemplo si  son diferentes en punto  de medida $\mu$  cero. M\'as geenralmente
  viene ac\'a la noci\'on de igualdad casi siempre:}
%
...
dicha {\it absolutamente continua} con  respeto a $\mu$ \modif{y se denota $$\nu
  \ll   \mu$$  De   hecho,  se   muestra  la   reciproca  del   la  definici\'on
  Def.~\ref{def:MP:DensidadMedida} resultado  a trav\'es  de lo que  es conocido
  como teorema de Radon-Nikod\'ym~\cite{Nyk30, AthLah06, Bog07:v1, Coh13}:
%
\begin{teorema}[Radon-Nikod\'ym]\label{Th:MP:RadonNikodym}
Sean dos medidas $\mu$ \ y \ $\nu$, entonces
%
\[
\nu \ll \mu \quad \Longleftrightarrow \quad \nu \: \mbox{admite una densidad con
  respeto a } \: \mu
\]
%
...
\]
%
\modif{En este  caso, $P_X$ es  conocida como  {\it medida de  Dirac} centrada en \  $x =
x_k$, \  denotada \ $\delta_{x_k}$, \ \ie \
%
\[
\delta_{x_k}(B) = \int_B d\delta_{x_k}(x) = \left\{\begin{array}{ll}
1 & \mbox{si} \quad x_k \in B\\[2.5mm]
%
0 & \mbox{si no}
\end{array}\right.
\]
...
\]
%
Se proba saliendo de \  $f = \un_{B'}$ \ y del teorema~\ref{Th:MP:MedibleLimite}
conjuntamente  a la definici\'on  Def.~\ref{Def:MP:IntegracionReal}.  }  Vamos a
volver m\'as adelante sobre esta medida particular.
...
y \ $1$ \ si no.  Entonces  de la formula de probabilidades totales, \ $F_X(x) =
P(X  \le x)  = P\left(  (X \le  x)  \left| \left(  U <  \frac12 \right)  \right.
\right) \, P\left( U < \frac12 \right) \, + \, P\left( (X \le x) \left| \left( U
      \ge \frac12 \right) \right.  \right) \,  P\left( U \ge \frac12 \right) $ \
\ie \  $F_X(x) = \frac12  P\left( (V  \le x) \left|  \left( U <  \frac12 \right)
  \right.   \right) \,  +  \, P\left(  (1 \le  x)  \left| \left(  U \ge  \frac12
    \right)\right. \right)$. \ Ahora, de la independencia  de \ $U$ \ y \ $V$, \
tenemos  \  $F_X(x)  =  \frac12  F_V(x)  + \frac12  \un_{[1  \,  +\infty)}(x)  =
\frac{x}{2}  \un_{[0 \,  ;  \, 1)}(x)  +  \un_{[1 \,  ;  \, +\infty)}(x)$.  Esta
funci\'on de repartici\'on  es representada figura Fig.~\ref{fig:MP:ProbaMixta}:
es ni discreta,  ni continua.  Entonces, a pesar de  que $\X = [0 \,  ; \, 1]$ \
sea un intervalo, $X$ no es continua (y tampoco no puede ser discreta).
%
\begin{figure}[h!]
\begin{center} \input{TIKZ_MP/ReparticionMixta} \end{center}
%
\leyenda{Funci\'on de repartici\'on $F_X(x) = \frac{x}{2} \un_{[0 \, ; \, 1)}(x)
  + \un_{[1 \, ; \, +\infty)}(x)$ asociada a \ $X = V \un_{U < \frac12} + \un_{U
    \ge \frac12}$ \  con \ $U$ \  y \ $V$ \ variables  continuas uniformes sobre
  $\X =  [0 \, ; \, 1)$.   No es tipo escalon,  as\'i que $X$ no  es discreta. A
  pesar de  que $\X =  [0 \, ; \,  1]$ \ sea  un intervalo, de la  presencia del
  salto en $x = 1$, tampoco $X$ no es continua.}
...
\SZ{No se  todav\'ia si  ser\'a \'util tratar  del caso  de limite de  series de
  funciones        medibles        (quizas,        tratando        de        los
  momentos/integraci\'on). Cf. Fubini tambi\'en.}
...
\modif{Un  ejemplo de  cambio  de  transformaci\'on puede  sevir  a calcular  la
  densidad de probabilidad de un vector $$V = X + Y$$ a partir de la densidad de
  $(X,Y)$,  con $X$ e  $Y$ $d$-dimensionales.  Por eso,  se puede  considerar la
  transformaci\'on biyectiva
%
\[
...
\]
%
Si $X$ e $Y$ son independientes, $p_{U,V}(u,v) = p_X(u) p_V(v-u)$ y la formula integral se escribe
%
\[
p_V(v) = \int_{\Rset^d} p_X(u) p_Y(v-u) \, du = \int_{\Rset^d} p_Y(u) p_X(v-u) \, du
\]
%
(por cambio  de variable  en la secunda  expresi\'on). Esta formula  es conocida
...
(\Rset^{d_X} \times \Rset^{d_Y} \, \B(\Rset^{d_X} \times \Rset^{d_Y}) )$ tal que
$X$  sea $d_X$-dimensional  e $Y$  sea $d_Y$-dimensional  (incluyendo  los casos
escalares).
...
1, o  usando la  regla de l'H\^opital~\footnote{\modif{De  hecho, esta  regla es
    debido  al  suizo  J.  Bernoulli  que  tuvo  un  acuerdo financiero  con  el
    Guillaume Fran\c{c}ois  Antoine, marqu\'es de  l'H\^opital, permitiendolo de
    publicar unos resultados de Bernoulli  bajo su nombre.}}  para re-obtener la
