...
elementos de la forma $\displaystyle  \optimes_{i=1}^d (-\infty \; b_i]$, \ie la
funci\'on de repartici\'on  multivariada~\cite{AshDol99, AthLah06, Coh13, Bre88,
  HogMck13}:
%
...
\begin{ejemplo}\label{Ej:MP:Rearreglo}
  Consideramos la densidad  de probabilidad $d$-dimensional mezcla~\footnote{Una
    ...
    \alpha_i f_i$ de leyes  $f_i$ con $\alpha = \protect\begin{bmatrix} \alpha_1
      &  \cdots  &  \alpha_k  \end{bmatrix}^t\protect \in  \Simp{k-1}$.}  \SZ{de
    Student-r (ver secci\'on~\ref{Sec:MP:EjemplosDistribucionesProb})}
%
\[
