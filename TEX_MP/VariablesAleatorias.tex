...
\begin{definicion}[Variable aleatoria continua admitiendo una densidad de probabilidad]
  ...
  Fijense de que  $\X$ es el soporte de  $p_X$, \ie $p_X(\X) \ne 0$.   En lo que
  sigue,  denotaremos  $|\X|$  el  volumen  (o  medida  de  Lebesgue)  de  $\X$,
...
$\displaystyle P(a < X \le b) = \int_a^b p_X(x) \, dx = F_X(b) - F_X(a)$ \ y que claramente
%
\[
...
%$$
%
Fijense de  que si  $0 \le F_X  \le 1$, no  $p_X$ puede  ser mayor que  uno, por
...
una probabilidad, sino que $\modif{p_X(x)} \, dx$ \modif{es esquematicamente} la
probabilidad de hallar a la variable con valores en el ``intervalo infinitesimal
entre $x$  y $x+dx$''.  \modif{Al  final, l}a condici\'on de  normalizaci\'on se
escribe
%
\[
...
\]
%
donde $\otimes$ denota el producto tensorial o externo, $p_{X_j} \otimes p_{X_k}
(x_j,x_k) = p_{X_j} (x_j) p_{X_k} (x_k)$.  Cuando los $\alpha_i$ son finito y la
notaci\'on  vectorial de  la definici\'on  es adoptada,  esta  expresi\'on queda
valide donde  $\otimes$ representa el producto de  Kronecker~\footnote{Para \ $p
  =   \begin{bmatrix}  p_1  &   \cdots  &   p_n  \end{bmatrix}^t$   \  y   \  $q
  =  \begin{bmatrix}  q_1 &  \cdots  & q_m  \end{bmatrix}^t$  \  el producto  de
  Kronecker es dado por \ $p \otimes q$ \ vector de tama\~no $n m$ de componente
  \ $(j-1) m + k$-esima \ el producto \  $p_j q_k, \quad 1 \le j \le n, \: 1 \le
  k  \le   m$.  Fijense  de  que   este  producto  es  asociativo   pero  no  es
...
\begin{definicion}[Vector aleatorio continuo y densidad de probabilidad multivariada]
  ...
  %
  \[
  \forall \, B \in \B(\Rset^d), \quad P_X(B) = \int_B p_X(x) \, dx
  \]
  %
  (la   integral  multiple   debe  ser   entendido   como  en   el  sentido   de
  Lebesgue~\footnote{Ver        nota        de       pie~\ref{foot:MP:Lebesgue},
    pagina~\pageref{foot:MP:Lebesgue}.} y  \ $dx = dx_1  \cdots dx_d$), entonces
  $X$ es dicha {\it admitiendo una densidad} y $p_X$ es llamada {\it densidad de
    probabilidad} de \ $X$, o  tambi\'en {\it densidad de probabilidad conjunta}
  ...
  Usaremos  todav\'ia la terminolog\'ia  (por abuso)  de {\it  distribuci\'on de
    probabilidad}; $\X$ es el soporte de $p_X$, denotaremos todav\'ia \ $|\X|$ \
  el volumen (o medida de Lebesgue) de \ $\X$, posiblemente infinito.
\end{definicion}
}
...
\SZ{Reordenamiento y relaci\'on de mayorizaci\'on  : a ver como trasladar lo que
  ya estaba en en cap. 2 mas los ejemplos.}
...
\'unico por definic\'on),
%
\[
\forall \,  B \in  \B(\Rset^d), \quad P_Y(B)  = \int_B p_X(g^{-1}(y))  \, \left|
  \Jac_{g^{-1}}(y) \right| \, dy
\]
%
...
funci\'on de repartici\'on, notando de que $g$ es necesariamente monotona:
%
\[
...
%
P(X \ge g^{-1}(y)) = 1 - F_X(g^{-1}(y)) & \mbox{si} & g \quad \mbox{es decreciente~\footnote{\modif{Fijense de que $P(X \ge x) = 1 - P(X < x) = 1 - P(X \le x) + P(X = x)$, pero $X$ siendo continua, $ P(X = x) = 0$.}}}
\end{array}\right.
}
\]
%
...
\modif{ Si $g$  no es inyectiva, $g^{-1}$ es multivaluada  o multiforme. En este
  caso,  se  puede  todav\'ia  tratar  el  problema,  particiendo  $\Rset^d$  en
...
%
\[
F_Y(y)  =  P(Y \le  y)  =  P\left(  g(X) \in  (-\infty  \,  ;  \, y]  \right)  =
\sum_{k=1}^m P\left( X \in g_k^{-1}(-\infty \, ; \, y] \right)
\]
%
\noindent (Ver figura~\ref{fig:MP:TransformacionVA}).
...
sencillamente $F_Y(y) = \frac23 \left(  1 - \frac1\pi \acos(y-1) \right) \un_{[0
  \,  ;   \,  2)}  +   \un_{[  2  \,   ;  \,  +\infty)}(y)$,   ilustrada  figura
...
\SZ{Tratar el caso de leyes condicionales}