...
n\'umeros  reales:   \  $\modif{\X  =  \{   x_i  \}_i}$~\cite{Ath06,  HogMck13}.
\modif{En este caso, las probabilidades $P_X(\{ x_i \}) = P(X = x_i), \: x_i \in
  \X$ caracterisan completamente la variable aleatoria~\cite{Ath06, HogMck13}:
%
...
\begin{definicion}[Variable aleatoria continua admitiendo una densidad de probabilidad]
  ...
  Lebesgue~\footnote{\SZ{Decir un poquito sobre la medida de Lebesgue y ref.}}),
  entonces $X$  es dicha {\it admitiendo  una densidad} y $p_X$  es llamada {\it
    densidad de probabilidad} de $X$. En particular,
  %
  \[
...
\modif{De  hecho, en el  caso continu,  discreto, o  cualquiera, se  conserva la
  ...
  integraci\'on~\cite{AthLah06, Coh13}. En el caso discreto cierto $X = x_0$, la
  distribuci\'on discreta es  dada por $p_X(x) = \un_{\{ x_0  \}}(x) = \un_{\{ 0
    \}}(x-x_0)$. En este caso, $P_X$ es  dicha {\it medida de Dirac} y $dP_X(x)$
  es denotado \ $\delta_{x_0}(x)$ \ o \ $\delta(x-x_0)$ \ tambi\'en llamado {\it
    delta de Dirac}. Se puede ver este Dirac como una}
%De aqu\'i  se observa que  la 
densidad de  probabilidad $p_X(x)$ \modif{pero} no es
% puede no  ser 
una funci\'on ``ordinaria''~\footnote{\SZ{El Dirac es tambi\'en definida como una distribuci\'on dicha de Schwarz, o funci\'on generalizada, cuya representaci\'on integral es \ $\displaystyle \delta(x) = \frac{1}{2\pi} \int_{-\infty}^{\infty} e^{itx} dt$, lo que muestra claramente su caracter no ordinario (la inetrgral siendo divergente en el sentido usual). Eso va m\'as all\'a de la meta del capitulo y el lector se podr\'a referir a~\cite{Sch, Kol, GelShi}}}.
