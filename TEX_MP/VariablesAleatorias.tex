...
\begin{definicion}[Integraci\'on de una funci\'on real]\label{Def:MP:IntegracionReal}
  ...
  que tambi\'en  $|g|$ \modif{(valor absoluto)} es medible  y, por definici\'on,
  $g$ es dicha $\mu$-integrable si la integral de $|g|$ es finita,
  %
  \[
...
\modif{A  continuaci\'on, dos lemas  van a  ser muy  \'util especialmente  en el
...
\begin{proof}
  ...
  aplicando el lema anterior dos veces  se obtiene \ $\displaystyle \forall \: A
  \in  \A, \quad  \nu(A) =  \int_A \frac{d\nu}{d\mu}(\omega)  \,  d\mu(\omega) =
...
\modif{Volvemos a las variables  discretas \ $X$ \ sobre $\X =  \{ x_j \}_j$, de
  medida  de  probabilidad  de la  forma  \  $P_X  = \sum_j  p_j  \delta_{x_j}$.
  Considerando la medida discreta \  $\mu_\X = \sum_j \delta_{x_j}$, es claro de
  que \ $P_X \ll \mu_\X$.   Entonces, formalmente, $P_X$ admite una densidad con
  respeto a la medida discreta $\mu_\X$  y esta densidad, definida sobre $\X$ es
  $p_X(x) = P(X  = x)$. A pesar  de que sea una tautolog\'ia,  este justifica de
  que usamos la escritura $p_X$ (minuscula)  en el caso discreto como en el caso
  continuo, y de que hablamos  (por abus de terminolog\'ia) de distribuci\'on de
  probabilidad en ambos caso.\label{Pagina:MP:DensidadDiscreta}}
...
$p_X(x) =  \sum_j p_j  \, \delta(x-x_j)$. \modif{Insistamos  en el hecho  de que
  ...
  Terminamos      mencionando      el      resultado     siguiente,      probado
  en~\cite[Ec.~(2.5)~p.~47 \& teorema~4.1.1]{AthLah} por ejemplo:
%
\begin{teorema}[Descomposition de una medida de probabilidad]\label{Th:MP:DescompositionMixta}
  ...
  combinaci\'on convexa de  una funci\'on de repartici\'on \  $F_d$ \ discreta y
  de una funci\'on de repartici\'on \ $F_c$ \ continua:
  %
  \[
  \exists \: a  \in [0 \, ; \, 1]  \quad \mbox{tal que} \quad F(x)  = a F_d(x) +
  (1-a) F_c(x)
  \]
  %
  ...
  convexa de una medida discreta $P_d$ y de una continua $P_c$,
  %
  \[
  \exists \:  a \in [0  \, ; \,  1], \: \widetilde{\X} \:  \mbox{discreto} \quad
  \mbox{tal que} \quad  P = a P_d  + (1-a) P_c \qquad \mbox{con}  \qquad P_d \ll
  \mu_{\widetilde{\X}} \quad \mbox{y} \quad P_c \ll \mu_L
  \]
  %
...
\modif{El                                teorema~\ref{Th:MP:DescompositionMixta},
...
importante. El el caso contrario, $Y$  no queda continua \modif{como se lo puede
