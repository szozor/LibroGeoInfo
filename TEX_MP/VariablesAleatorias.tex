...
\modif{Dicho de  otra manera, en  el caso finito  $|\X| = \alpha <  +\infty$, el
  ...
  probabilidad  $p_X \in  \Simp{\alpha-1}$ (ver  notaciones). La  condici\'on de
  normalizaci\'on se escribe tambi\'en de manera vectorial bajo la forma
  %
 \[
 \un^t p_X = 1
 \]
 %
 donde  $\un =  \begin{bmatrix} 1  & \cdots  & 1  \end{bmatrix}^t$ de  tama\~{n}o
 $\alpha$ (ver notaciones).}
...
\Nset_0$.  La  forma de la distribuci\'on  es: \ $p_X(x_j)  = \frac1\alpha \quad
...
\un_{\Rset_+}(x-x_k)$  \modif{($\un_{\Rset_+}$ es  conocido tambi\'en  como {\it
...
%
  ...
  $d   =  2$,  \modif{una   ``tabla''  3   dimensonal  para   $d=3$,\ldots;  ver
    notaciones}).  Pero es posible usar una notaci\'on vectorial, recordando que
  $\Nset^d$ puede ser puesto en  biyecci\'on con $\Nset$, y dada una biyecci\'on
  usarla para etiquetar los componentes de  $p_X$ puestos en vector.  En el caso
  finito \ $\X_i = \{ x_{j_i}  \}_{j=1}^{\alpha_i}$ \ con \ $\alpha_i = |\X_i| <
  +\infty$, \ se  puede organizar los componentes tales  que $p_X(x_{j_1}\, , \,
  \ldots \,  , \,  x_{j_d})$ sea  la $j$-\'esima componente  del vector  $p_X$ con
...
\begin{ejemplo}\label{Ej:MP:Rearreglo}
  Consideramos la densidad  de probabilidad $d$-dimensional mezcla~\footnote{Una
    mezcla  de ley  es definida  como  combinaci\'on convexa  $f =  \sum_{i=1}^k
    \alpha_i f_i$ de leyes  $f_i$ con $\alpha = \protect\begin{bmatrix} \alpha_1
      &   \cdots  &  \alpha_k   \end{bmatrix}^t\protect  \in   \Simp{k-1}$.}  de
  Student-$r$ (ver secci\'on~\ref{Sssec:MP:StudentR})
  %
  \[
  %\displaystyle   \un_{\Bset_d(-m,1)}(x)   \un_{\Bset_d(m,1)}(x)
  ...
  para  $d = 2,  \: \nu  = 6,  \: m  = \frac{1}{\sqrt{d}}  \un$.  El  dominio de
  ...
  % \un_{\Bset_d\left(0 ,      2^{\frac1d} \right)}(x)
  \]
  %
 ...
 Finalmente, la curva de Lorentz es dada por
  ...
  \leyenda{(a) Densidad de  probabilidad $p_X$ del ejemplo~\ref{Ej:MP:Rearreglo}
    ...
    \frac{1}{\sqrt{d}}  \un$. (b)  Densidad rearreglada  $p^{\downarrow}_X$. (c)
