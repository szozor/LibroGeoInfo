...
{\it variable aleatoria} $X$ a valores reales, que puede ser discreta, continua
o mixta.
...
masa en mec\'anica por ejemplo~\cite{Leb04, Leb18, KolFom61, AshDol99, AthLah06,
  Bog07:v1, Coh13}.
...
sin detallar las pruebas. Por esto, el lector se puede referir a~\cite{AshDol99,
  LieLos01, AthLah06, Bog07:v1, Coh13}.
%
...
%
  ...
  $\frac{\partial}{\partial t_1} , \ldots  , \frac{\partial}{\partial t_d}$ (ver
  notaciones),  entonces la  funci\'on  \ $\displaystyle  t \mapsto  \int_\Omega
  f(\omega,t) \, d\mu(\omega)$ \ es diferenciable sobre \ $I$, \ y
  %
  \[
...
funci\'on de repartici\'on \modif{(a  veces llamada funci\'on distribuci\'on por
  abuso de denominaci\'on)}~\cite{AshDol99, AthLah06, Coh13, Bre88, HogMck13}:
%
...
%
  ...
  aleatoria (o  vector aleatorio)  \ $X$. Como  en el caso  escalar, \modif{para
    cualquier $B \in \B(\Rset^d)$}
  %
 \[
...
``debajo''  de  su columna  antecedente).  \modif{Volveremos  m\'as adelante  en
  matriz aleatorias.}
%, no desarrollaremos  m\'as este  caso, a
...
pre-imagen.  \modif{Adem\'as, a veces,  y por abuso, llamaremos frecuentemente a
  $\X$ {\it dominio de definici\'on}  de la medida de probabilidad $P_X$, siendo
  $P_X(\Rset^d \setminus \X) = 0$.}
...
%
\modif{
Dicho  de  otra manera,  en  el  caso finito  $|\X|  <  +\infty$, el  vector  de
  probabilidad $p_X$  partenece al  simplex estandar  o simplex  de probabilidad
  $p_X \in \Simp{|\X|-1}$ (ver  notaciones). La condici\'on de normalizaci\'on
  se escribe tambi\'en de manera vectorial bajo la forma
  %
 \[
 \uno^t p_X = 1
 \]
 %
 donde  $\uno =  \begin{bmatrix} 1  & \cdots  & 1  \end{bmatrix}^t$ de  tama\~{n}o
 $|\X|$ (ver notaciones).}
...
la noci\'on de mayorizaci\'on~\cite{MarOlk11, BenZyc06}:
%
...
conocimiento completo.  \modif{Veremos en el cap\'itulo~\ref{Cap:SZ:Informacion}
  el rol importante de este orden sobre las medidas informacionales.}
...
en  funci\'on  de  $k$,  llamadas {\it  curvas  de  Lorentz~\footnote{Se  prueba
sencillamente que  estas curvas  son crecientes  y c\'oncavas.}}~\cite{MarOlk11,
Lor05, BenZic06}.  Gr\'aficamente,  $p \prec q$ es equivalente a  tener la curva
de Lorentz asociada a $p$ por debajo de la asociada a $q$.
%
\begin{figure}[h!]
...
%
  ...
  $p_X$ es necesariamente $\X = X(\Omega)$  (\ie $p_X(\widebar{\X}) = \{ 0 \}$ \
  y \ $p_X(\X) > 0$), y
  %
  \[
  ...
  \]
  %
  (esta expresi\'on es valida para  cualquier medida $\mu$, densidad con respecto
  ...
  distribuci\'on  de  probabilidad}~\footnote{\modif{En   resumen,  usaremos  la
  denominaci\'on   distribuci\'on  de   probabilidad  indistintamente   para  la
  distribuci\'on $P_X$ mismo, la funci\'on de  masa $p_X$ en el caso discreto, o
  la densidad de  probabilidad $p_X$ en el  caso continuo.}}, a pesar  de que no
  tiene el mismo sentido que la masa de probabilidad del caso discreto.
\end{definicion}
%
...
Notar que  a\'un cuando $0 \le  F_X \le 1$, \  $p_X$ puede ser mayor  que uno. Por
...
%
  ...
  probabilidad \  $p_U = p_V  = \uno_{[0 \; 1)}$  \ ($U$ \  y \ $V$ \  son
  uniformes sobre $[0  \; 1)$) \ y sea \  $X = V \uno_{U <  \frac12} + \uno_{U \ge
  ...
  \frac12 \uno_{[1 \; +\infty)}(x)$ \ es decir
  %
  \[
  F_X(x) = \frac{x}{2} \uno_{[0 \; 1)}(x) + \uno_{[1 \; +\infty)}(x).
  \]
  %
  ...
  %
  \leyenda{Funci\'on de repartici\'on $F_X(x)  = \frac{x}{2} \uno_{[0 \; 1)}(x) +
    \uno_{[1 \; +\infty)}(x)$ asociada a \ $X  = V \uno_{U < \frac12} + \uno_{U \ge
...
%  \uno_{U \notin \Qset}$. Claramente  $X$ no es continua, pero $\X =  [0 \;
...
\uno_{\Rset_+}(x-x_k)$  \modif{($\uno_{\Rset_+}$ es  conocido tambi\'en  como {\it
    funci\'on  de  Heaviside}~\footnote{\modif{Viene  del  nombre  del  f\'isico
      ingles Oliver  Heaviside quien estudio  las ecuaciones electromagn\'eticas
      de Maxwell.}})} y en el sentido de las distribuciones, $\frac{d F_X}{dx} =
...
% Vr tambien AshDol99 2.2.6 p. 70
Terminamos mencionando el  resultado siguiente, probado en~\cite[Ec.~(2.5)~p.~47
\& teorema~4.1.1]{AthLah06} por  ejemplo~\footnote{B\'asicamente, se muestra que
  ...
  \uno_{(-\infty  \;  \widetilde{x}]}(x)$  \   es  continua,  y  se  recupera  la
...
%
  ...
  \]
  %
  En t\'erminos  de medida, o  como corolario de~\cite[teorema~4.1.1]{AthLah06},
...
ejemplo~\ref{Ej:MP:Mixta}.  Del teorema, es discreta  cuando $a = 1$, y continua
cuando $a = 0$.
...
%
  ...
  {\it discreto} cuando $\X \subset \Nset^d$, \ es discreto, finito o infinito
  numerable.
\end{definicion}

Obviamente, la medida de probabilidad  en los $\displaystyle x = \begin{bmatrix}
  x_1 &  \cdots & x_d  \end{bmatrix}^t \in \optimes_{i=1}^d \X_i$  \ caracteriza
completamente este vector aleatorio:
%
...
%
  ...
  %,  de  hecho   una  probabilidad  conjunta  para  este   producto  puede  ser
  nula.\newline  En  el caso  multivariado,  la  notaci\'on vectorial  es  m\'as
  delicada   de   usar:   $p_X$   ser\'ia  un   ``tensor''   \modif{(o   tabla)}
  $d$-dimensional (una  matriz para $d  = 2$, \modif{una ``tabla''  3 dimensonal
  para  $d=3$,\ldots; ver  notaciones}).  Pero  es posible  usar una  notaci\'on
  vectorial,  recordando  que $\Nset^d$  puede  ser  puesto en  biyecci\'on  con
  $\Nset$, y dada una biyecci\'on usarla para etiquetar los componentes de $p_X$
  puestos    en    vector.    En    el    caso    finito    \   $\X_i    =    \{
  x_{j_i} \}_{j=1}^{|\X|_i}$  \ con \ $|\X_i|  < +\infty$, \ se  puede organizar
  los componentes tales que $p_X(x_{j_1}\, , \,  \ldots \, , \, x_{j_d})$ sea la
  $j$-\'esima  componente  del vector  $p_X$  con  \modif{$\displaystyle j  =  1
  + \sum_{i=1}^d (j_i - 1) \, \prod_{k=i+1}^d |\X|_k$}.
\end{definicion}
%
...
\modif{producto externo (ver notaciones)}.
...
\modif{tiene como} probabilidad marginal o distribuci\'on marginal
%
\[
...
$X_{I_k}   =  \begin{bmatrix}   X_{i_1}  &   \cdots   &  X_{i_k}\end{bmatrix}^t$
\modif{tiene la} {\it densidad de probabilidad marginal}
%
\[
...
%
  ...
  %
  \[
  p^\downarrow_X(x)  =  \int_0^{+\infty}  \uno_{\P_t^\downarrow}(x) \,  dt.
  \]
  %
\end{definicion}
%
De  \ $\forall  \, t  < \tau  \: \Leftrightarrow  \: \P_\tau  \subset  \P_t \:
\Leftrightarrow  \: \P_\tau^\downarrow  \subset \P_t^\downarrow$,  es sencillo
...
representaci\'on de  la definici\'on es conocida como  representaci\'on torta en
capas  (``layer cake  representation''  en  ingl\'es).  Esto  es  ilustrado en  la
...
\leyenda{(a):  Ilustraci\'on  del rearreglo  sim\'etrico  $\P^\downarrow$ de  un
  ...
  sim\'etrico    $p_X^\downarrow$   \modif{en    un   contexto    escalar   (ver
  ejemplo~\ref{Ej:MP:Rearreglo} para $d  = 1, \: \nu  = 5, \: m =  1$)}: dado un
  $\tau$,  se  busca  $\P_\tau$  (izquierda) y  se  deduce  $\P_\tau^\downarrow$
  (derecha); dado un $x$, se busca el mayor $u$ tal que $x \in \P_u^\downarrow$,
  siendo  entonces este  $u$  supremo igual  a $p_X^\downarrow(x)$,  \modif{\ie
  $\displaystyle   p_X^\downarrow(x)    =   \sup   \{   u   > 0   \tq
  x  \in  \P_u^\downarrow  \}$}  (derecha); adem\'as,  por  construcci\'on,  las
  superficies en gris son iguales.}
  %
\label{Fig:MP:ensemblerearreglado}
\end{figure}
%
\modif{Notar  que, por  construcci\'on, $p_X^\downarrow$  cae el  la  familia de
  densidades                            esf\'ericas                           (ver
  secci\'on~\ref{Sec:MP:EjemplosDistribucionesProb})~\cite{Lor54,       FanKot90,
    CamHua81, Eat81}.}
...
%
Una  densidad  de probabilidad  $p$  se  dice  mayorizada  por una  densidad  de
probabilidad $q$ sii:
  %
  \[
...
es necesariamente  concava). M\'as all\'a,  sea $Y$ de densidad  de probabilidad
$p_Y  \equiv p^\downarrow$  invariante  por rotaci\'on  por construcci\'on.   Se
ver\'a  m\'as adelante,  en la  secci\'on~\ref{Ssec:MP:FamiliaEliptica}, (prueba
del             teorema~\ref{Teo:MP:MezclaUniforme}             o            del
teorema~\ref{Teo:MP:DensidadRayo}), que $\L_p$ es nada m\'as que la funci\'on de
repartici\'on de $\| Y \|$, que llamaremos el rayo, $\L_p \equiv F_{\| Y \|}$.
...
\begin{ejemplo}\label{Ej:MP:Rearreglo}
  ...
  %
  \[
  %\displaystyle   \uno_{\Bset_d(-m,1)}(x)   \uno_{\Bset_d(m,1)}(x)
  ...
  % \uno_{\Bset_d\left(0 ,      2^{\frac1d} \right)}(x)
  \]
  %
 ...
 Finalmente, la curva de Lorentz es dada por
  ...
  %
  \leyenda{(a) Densidad de  probabilidad $p_X$ del ejemplo~\ref{Ej:MP:Rearreglo}
    en  el  contexto  bi-dimensional  $d  =  2$   y  para  $\nu  =  6,  \:  m  =
    \frac{1}{\sqrt{d}}  \uno$. (b)  Densidad rearreglada  $p^{\downarrow}_X$. (c)
    Equivalente continua  de la curva de  Lorentz para la  densidad $p_X$ (linea
    llena), la  densidad gausiana esf\'erica \  $g_X$ \ con  covarianza de misma
    traza que  la de  $p_X$ (linea con  guiones) y  la densidad Student-t  \ con
    covarianza de  misma traza que la  de $p_X$ \ y  \ $\nu' = 2.25$  \ grado de
    libertad (linea punteada): \ $g_X \prec  p_X$ \ pero \ $s_X \not\prec p_X$ \
    y \ $p_X \not\prec s_X$.}
