...
\subseccion{Consideraciones  preliminarias  --  teoria  de  la medida  y  de  la
  integraci\'on.}
...
una $\sigma$-\'algebra  particular es  conocido como {\it  $\sigma$-\'algebra de
...
\begin{definicion}[Integraci\'on de una funci\'on real]\label{Def:MP:IntegracionReal}
  ...
  que tambi\'en  $|g|$ (valor absoluto) es  medible y, por  definici\'on, $g$ es
  dicha $\mu$-integrable si la integral de $|g|$ es finita,
  %
  \[
...
detallar las  pruebas. Por eso,  se puede referirse  a~\cite{LieLos01, AthLah06,
  Bog07:v1, Coh13}.
%
...
Fijense de  que dos funciones pueden  cumplir esta definici\'on,  por ejemplo si
son  diferentes  en  un conjunto  de  medida  \  $\mu$  \  igual a  cero.  M\'as
...
A  continuaci\'on,  dos  lemas  van   a  ser  muy  \'util  especialmente  en  el
cap\'itulo~\ref{Cap:SZ:Informacion},  tratando  de   dos  (o  m\'as)  medidas  y
densidades.
%
\begin{lema}\label{Lem:RelacionIntegracionDerivadasRadon}
  Sean \ $\nu$  \ y \ $\mu$ \  dos medidas sobre \ $(\Omega,\A)$ \ tales que \ $\nu  \ll \mu$. Entonces,
  para cualquier funci\'on medible $f$,
  %
  \[
...
%
\begin{proof}
  Tomando     \    $f     =     \un_A$    \     da     de    la     definici\'on
  Def.~\ref{Def:MP:DensidadMedida}, pagina~\pageref{Def:MP:DensidadMedida} de la
  densidad se obtiene
  %
  \[
  \int_A   \frac{d\nu}{d\mu}(\omega)   \,  d\mu(\omega)   =   \nu(A)  =   \int_A
  ...
  \]
  %
  Se    cierra    la    prueba   por    el    teorema~\ref{Th:MP:MedibleLimite},
  pagina~\pageref{Th:MP:MedibleLimite} y  tratando $f$  con su parte  positiva y
  negativa separamentemente.
\end{proof}
%
\begin{lema}\label{Lem:RelacionesDerivadasRadon}
  ...
  \item $\quad \nu \ll \mu$;
  %
  \item  equivalentemente, el  soporte (ensemble  de  puntos que  no cancela  la
    funci\'on)  de   \  $\displaystyle   \frac{d\nu}{d\mu}$  \  es   \  incluido
    ($\mu$-casi siempre) en el de \ $\displaystyle \frac{d\lambda}{d\mu}$;
  %
...
%
\begin{proof}
  El resultado viene  de la definici\'on de la absoluta  continuidad $\mu(A) = 0
  \quad \Rightarrow \quad \lambda(A) = 0 \quad \Rightarrow \quad \nu(A) = 0$ (el
  secundo item viniendo  escribiendo la medida en su  forma integral). Ad\'emas,
  por definici\'on de  la densidad, \ $\displaystyle \forall \:  A \in \A, \quad
  \nu(A) =  \int_A \frac{d\nu}{d\mu}(\omega) \,  d\mu(\omega)$. Luego, aplicando
  el  lema  anterior  a \  $f  =  \un_A  \frac{d\nu}{d\mu}$  \ se  obtiene  que,
  tambi\'en,   \   $    \nu(A)   =   \int_A   \frac{d\nu}{d\lambda}(\omega)   \,
  d\lambda(\omega)      =      \int_A      \frac{d\nu}{d\lambda}(\omega)      \,
  \frac{d\lambda}{d\mu}(\omega) \, d\mu(\omega)$, lo que cierra la prueba.
\end{proof}
...
\item $\mu_L$ es $\sigma$-finita. Viene de $\Rset^d = \cup_{i=1}^d \cup_{j_i \in
    \Zset}   \optimes_{i=1}^d   (   j_i   \,   ;   \,   j_i+1]$   \   conjuntament   a
...
\end{itemize}
%
La medida de  Lebesgue siendo as\'i natural tratando de  la integraci\'on, en lo
que sigue, tratando de igualdad $\mu_L$-casi siempre, diremos simplemente ``casi
siempre'' $(c.s.)$, subentiendo  que es con respeto a la  medida de Lebesgue. De
la misma  manera, hablando de densidad,  sin precisiones, ser\'a  a entender con
respeto a $\mu_L$.
...
\begin{definicion}[Medida de Dirac y medida discreta]\label{Def:MP:Dirac}
  ...
  \]
  %
  (en general, son definidas  como combinaci\'on lineales positivas, este siendo
  un caso particular).
\end{definicion}
%
Fijense de que,
...
\item $\forall  \: A \in \A,  \quad \mu_\X(A) =  |\X \cap A|$ \  donde $|\cdot|$
  denota el cardenal de un conjunto, equivalente discreto del volumen (puede ser
  infinito tambi\'en).
%
\item Para una funci\'on \ $g$ \ medible,
  ...
  \]
  %
  la integraci\'on se vuelve  ser una suma.  Se proba saliendo de  \ $g$ \ de la
  forma \ $g = \un_C$  \ y del teorema~\ref{Th:MP:MedibleLimite} conjuntamente a
  la definici\'on Def.~\ref{Def:MP:IntegracionReal}.
\end{itemize}
...
\mu_\X$.   Entonces, formalmente,  $P_X$ admite  una densidad  con respeto  a la
...
densidad de probabilidad $p_X(x)$ con respeto a la medida de Lebesgue pero no es
una funci\'on ``ordinaria'' dando de que $P_X$ no es diferenciable con respeto a
...
de densidad  (con respeto a  la medida de  Lebesgue): si una  variable aleatoria
...
\&  teorema~4.1.1]{AthLah} por ejemplo~\footnote{\modif{Basicamente,  se muestra
    de que \ $\displaystyle  \widetilde{X} = \{ x \in \X, \quad  p(x) = F_X(x) -
    \liminf_{u  \to  x} F(u)  >  0  \}$ \  es  numerable.   A continuaci\'on,  \
    $\displaystyle    F(x)    -    \sum_{\widetilde{x}    \in    \widetilde{\X}}
    p(\widetilde{x}) \un_{(-\infty \, ;  \, \widetilde{x}]}(x)$ \ es continua, y
    se recupera  la descomposici\'on con $\displaystyle  a = \sum_{\widetilde{x}
      \in \widetilde{\X}} p(\widetilde{x})$.}}:
%
\begin{teorema}[Descomposition de una medida de probabilidad]\label{Th:MP:DescompositionMixta}
  ...
  con respeto a la medida $\sigma$-finita $\mu_{\widetilde{\X}} + \mu_L$.
\end{teorema}
%
...
\end{definicion}
%
\noindent De nuevo, $p_X$ puede ser  vista como densidad con respeto a la medida
