...
Vajda72,
Voir OstVaj en termes de metrique pour les Arimoto, Vajda 2009, Kafka 91
}
...
para  \ $\phi(u)  =  u^\lambda$ \  y \  $h(u)  = \frac{\log  u}{\lambda-1}$ \  o
divergencias  relacionadas   como  la  de   Chernov~\cite{CreRea84,  Che52},  la
$J$-divergencia  para \ $\phi(u)  = u  \log u  - \log  u$ \  y \  $h =  \id$, la
distancia  de Bhattacharyya~\cite{Bha43, Bha46:07}  \ $\displaystyle  -\log \int
\sqrt{p q}$  \ para \ $\phi(u)  = \sqrt{u}$ \ y  \ $h(u) = -  \log u$, instancia
particular de la de R\'enyi ($\lambda = \frac12$), entre otros.
...
\item Pensando a $\cphi{}$  con respecto a $\phi$, es lineal en  el sentido de que
  $D_{\lambda_1   \phi_1   +    \lambda_2   \phi_2}^{\mathrm{c}}   =   \lambda_1
  D_{\phi_1}^{\mathrm{c}}  +  \lambda_2  D_{\phi_2}^{\mathrm{c}}$ (con  $\phi_i$
  convexas y $\lambda_i \ge 0$).
%
...
\item $\chphi{}$ es  sim\'etrica si y solamente si $\phi =  \phi^* + c (\id-1)$;
  ...
  %
  \[
  \chphi{} + D^{\mathrm{c}}_{\phi^*} = D^{\mathrm{c}}_{\phi+\phi^*}
  \]
  %
...
en~\cite{Vaj72, Csi74, LieVaj87, KafOst91, Ost96, OstVaj03, KumChi05, LieVaj06}.
Como  en el  caso de  las  divegencias de  Jensen, en  general las  divergencias
sim\'eticas  ($\phi  =  \phi^*$)  no  satisfacen en  general  a  la  desigualdad
triangular,  y entonces no  dan lugar  a una  distancia entre  distribuciones de
probabilidad,  \SZ{aparte en casos  particulares~\cite{toto}}.  Sin  embargo, se
prob\'o  en~\cite[Teoremas~1~\&~2,   Remark~6]{KafOst91}  \  y   \  \cite{Ost96,
  OstVaj03} el lema  siguiente, condici\'on suficiente para que  una potencia de
la divergencia satisfaga a la desigualdad triangular:
%
\begin{lema}
  ...
  %
  \[
  \left( \cphi{} \right)^s \quad s \in  (0 \, , \, \kappa] \quad \mbox{satisface
    la desigualdad triangular}
  \]
  %
  \noindent   y  entonces   es   una  metrica   entre   dos  discribuciones   de
  probabilidades.
\end{lema}
%
...
\right)$ \  y \ $\kappa  = \min\left( \beta  \, , \,  \frac12 \right)$ \  es una
m\'etrica~\cite{OstVaj03}.

\SZ{voir borne sup dphi et avec h quand phi = phi*; Kumar China ou Vajda 72, Liese Vajda 03 eq. 35}
...
\SZ{
Chernov 52 Che52, dans les ref de Liese Vajda, cf Bha 43 egalement
}
