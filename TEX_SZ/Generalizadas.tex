...
A pesar de que la entrop\'ia de Shannon y sus cantidades asociadas demostraron sus
...
informacionales, de  tipo entrop\'ias o  divergencias, aparecieron luego.  En esta
...
generalizaciones de la  entrop\'ia de Shannon por un lado, y  de la divergencia de
Kullback-Leibler por el otro lado. No son siempre vinculados, a pesar de que sea
desirable que a cada entrop\'ia sean asociados nociones de entrop\'ias condicionales
y relativas.
...
Shannon fue el punto de salida fundamental  en todo el desarollo de la teor\'ia de
...
buscar una  entrop\'ia elemental $H^\ren(p_i)$  y remplazar la  media aritm\'etica
...
\ne 1$,  probando que  la entrop\'ia que  los axiomas  (i)-(ii)-(iii)-(iv)-(v') se
cumplen y conduce a la entrop\'ia de R\'enyi de un vector de probabilidad $p$,
%
\[
...
secci\'on siguiente sobre esta entrop\'ia  relativa, o divergencia de R\'enyi. Por
axiomas,     las     propiedades    \ref{prop:SZ:continuidad}     (continuidad),
\ref{prop:SZ:permutacion}       (invarianza      por       permutaci\'on)      y
\ref{prop:SZ:aditividad} (additividad)  de la  entrop\'ia de Shannon  se conservan
...
(p_{n-1}+p_n)^\beta H^\hc\left( \frac{p_{n-1}}{p_{n-1} + p_p},\frac{p_n}{p_{n-1}
  ...
  cualquiera par $(p_i, p_j)$ sin imponer la invarianza por permutaci\'on, pero es
...
a la entrop\'ia (teorema~1)
%
\[
H_\beta^\hc(p) = \frac{1}{1-2^{1-\beta}} \left( 1 - \sum_i p_i^\beta \right)
\]
%
que nombraron {\it $\beta$-entrop\'ia structural}.  De nuevo, relaxando el axioma
(iv),  se puede  remplazar en  el coeficient  $2^{1-\beta}$  por $a^{1-\beta},
\quad a >  0, \quad a \ne 1$. De  nuevo, parae que la entrop\'ia  de Shannon es un
caso particular,
%
\[
...
que hay  un mapa  uno-uno entre  las dos entrop\'ias.  Se notara  en un  marco m\'as
general otras propiedades.

Independiente  de Havrda  \&  Charv\'at, todav\'ia  en  el este,  en la  escuela
h\'ugara, Z.  Dar\'oczy en~\cite{Dar70} defino la entrop\'ia $H^f$ a partir de una
...
f_\beta\left(  \frac{y}{1-x} \right)  = f_\beta(y)  +  (1-y)^\beta f_\beta\left(
  \frac{x}{1-y}  \right)$,   lo  que   condujo  necesariamente  a   la  entrop\'ia
(teoremas~2 y~3)
%
\[
...
el mapa  entre su  entrop\'ia y la  de R\'enyi. Adicionalmente  a Havdra-Charv\'at
...
tambi\'en es se puede definir una entrop\'ia condicional en el mismo modelo que en
...
y    que    \ref{prop:SZ:condicionar}    condicionar    reduce    la    entrop\'ia
...
$\phi$-entrop\'ias contienen Shannon como caso  particular ($\phi(x) = x \log x$),
...
hecho, las $\phi$-entrop\'ias se enmarcan en una clase un poco m\'as amplia, llamada
$(h,\phi)$-entrop\'ias~\cite{SalMen93, MenMor97}. Cambiamos ac\`a substancialmente
su escritura por razones homogeneidad con la $\phi$-entrop\'ia (y las divergencias
que se introducira luego)~\footnote{En la literatura, no hay el signo $-$, y hay
  que invertir c\'oncava y convexa.}
%
\begin{definicion}[$(h,\phi)$-entrop\'ia]\label{def:SZ:HPhiEntropia}
La $(h,\phi)$-entrop\'ia de una  distribuci\'on de probabilidad $p_X$ definida sobre
...
tamb\'ien  en esta  familia ($\phi(x)  = -  x^\beta$ \  y \  $h(x)  = \frac{\log
  x}{1-\beta}$) as\'i que todas las entrop\'ias evocadas en el parafo anterior.

Como en el caso de  Shannon, para $X = (X_1,\ldots,X_d)$, la $(h,\phi)$-entrop\'ia
de $X$ es una $(h,\phi)$-entrop\'ia conjunta de los $X_i$.

Obviamente,  de  las  propiedades  de  la  entrop\'ia  de  Shannon,  se  conservan
...
Adem\'as  se conserva  la  Schur-convavidad \ con una reciproca:
%
...
\item Schur-convavidad:
  ...
  relaci\'on  de ordre  entr\'opicas para  cualquiera par  de  funciones entr\'opicas
...
a $\phi(0)  = 0$ y  $h(\phi(1)) = 0$  (alcanzado en el caso  determinista), la
...
\item\label{prop:SZ:concavidadHPhi} Si $h$  es c\'oncava, entonces $\hhphi[p]$ es
  c\'oncava con respeto a $p$.  Eso es una consecuencia de la concavidad de $\phi$
...
\item  Sean \  $X$ \  definido  sobre \  $\X$ \  y  \ $\overline{X}$  \ sobre  \
  ...
  Havdra-Charv\'at-Dar\'oczy, no hay un  v\'inculo inmediato entre $\hhphi[p_X]$ \
  y \ $\hhphi[p_{\overline{X}}]$.
\end{propiedadesPhi}

Se  conserva  la  superadditividad~\ref{prop:SZ:superaditividad}. De  hecho,  si
$\phi$ es convexa (resp. c\'oncava) con \ $\phi(0) = 0$, \ $\forall \: 0 \le a \le
...
\begin{proof}
  ...
  \]
  %
  (para $\phi$  c\'oncava se  remplaza $\hphi$ por  $-H_{-\phi}$ con  la igualdad
  inversa), donde
  %
  \begin{equation}
  ...
  (resp. c\'oncava)  hace falta tener $\lambda  > 0$ (resp.  $\lambda  < 0$) as\'i
  ...
  cualquiera funci\'on creciente de $H$  va obviamente quedar subadditiva, lo que
  cierra la prueba.
  %
\end{proof}
%
...
\hhphi[p_X]   +   \hhphi[p_Y]$:   Tampoco   las   $(h,\phi)$-entrop\'ia   no   son
super-additivas.
...
superadditividad garantiza la positividad  de la entrop\'ia condicional, se pierde
...
Como en el caso de Shannon, se puede extender la generalizaci\'on de la entrop\'ia
...
\begin{definicion}[$(h,\phi)$-entrop\'ia diferencial]
  ...
  de $X$, La $(h,\phi)$-entrop\'ia diferencial de la variable $X$ es definida por
  %
  \[
...
De nuevo para $X =  (X_1,\ldots,X_d)$, la $(h,\phi)$-entrop\'ia diferencial de $X$
es una $(h,\phi)$-entrop\'ia diferencial conjunta de los $X_i$.
...
cota  superior~\ref{prop:SZ:cotamaxima}  (salvo si  se  pone  v\'inculos, ver  m\'as
adelante), en adici\'on de las que ya la versi\'on discreta perdi\'o.

Sin embargo, se conservan unas propidedades, y entre otros si $h$ es c\'oncava, la
$(h,\phi)$-entrop\'ia  diferencia el  c\'oncava~\ref{prop:SZ:concavidadHPhi}.  M\'as
sopredentemente a  primer vista,  se conserva la  $(h,\phi)$-entrop\'ia diferencia
...
$\phi$-entrop\'ias. Entonces,  de $h$  creciente (para $\phi$  c\'oncava desigualdad
...
vice-versa. Adem\'as, no cumple tampoco  la desigualdad triangular. A pesar de sus
...
%  usa casi siempre la formulaci\'on dada. La formulaci\'ion con la entrop\'ia (con
...
\frac12]^t$. Adem\'as,  en este caso, a pesar  de que la divergencia  no cumpla la
...
figura Fig.~\ref{fig:SZ:Concavidad}-(b). Sea $Z$  la variable aleatoria binaria sobre
...
M\'as all\'a, en  el campo de la  clasificaci\'on, se puedre tratar de  m\'as de dos
...
\SZ{ (0)  Jensen generalizada  caso H  phi Ref. Nielsen ArXiv 2010, Nielsen Boltz 2011 -> cualquiera funcion c\'oncava $\hgene$ de la distribucion de proba., en particular h phi

  Se  ilustra  a  que  corresponde  esta  cantidad  con  respeto  a  $f$  en  la
  figura Fig.~\ref{fig:SZ:BregmanFJensen} m\'as adelante.  }
...
problemas de  minimizaci\'on convexa~\footnote{A\'un que aparece  en una revista
  ...
  toma  el  ejemplo  de  maximizaci\'on  de  la entrop\'ia  de  Shannon  sujeto  a
...
\begin{definicion}[Divergencia de Bregman]
  ...
  respeto a $f$ en la figura Fig.~\ref{fig:SZ:BregmanFJensen} m\'as adelante.
\end{definicion}
%
...
tratando posiblemente de  densidad de probabilidad, nos interesa  a funciones de
funciones~\cite{FriSri08, NieNoc17}:
%
\begin{definicion}[Divergencia de Bregman funcional]
  ...
  ``punto'' (una funci\'on)  \ $v \in \Omega$ \ relativamente  a un ``punto'' (\
  $u \in \Omega$ \ es definida por
  %
  \[
  B_f(v\|u) = f(v) - f(u) - \lim_{t \to 0}\frac{f( u + t (v-u) ) - f(u)}{t}
  \]
  %
  El \'ultimo de esta formula es connocida como derivada de G\^ateau (o derivada
  direccionel)  de  $f$  en  $u$   en  la  direcci\'on  $v-u$  (siendo  $u$  una
...
Para $(h,\phi)$-entrop\'ias discretas \underline{c\'oncavas} (ej.  con $h$ c\'oncava),
...
La figura Fig.~\ref{fig:SZ:BregmanFJensen} ilustra  a que corresponden \ $D_f$  \ y \
...
\item Positividad:
  ...
  ilustrado \SZ{figura Fig.~\ref{fig:SZ:ConvexidadDerivada}.}
%
...
la  distancia  $L^2$  ($\displaystyle f(p)  =  \int  p^2$),  el cuadrado  de  la
distancia $L^1$ ($\displaystyle  f(p) = \left( \int p  \right)^2$), la distancia
de Itakura-Saito cuando $\phi(u) = - \log u$ (asociado a la entrop\'ia de Burg),
entre otros.
...
\paragraph{Principio de entrop\'ia m\'axima} Si este principio naci\'o en el marco
de la termodynamica o f\'isica,  con la entrop\'ia de Shannon (Boltzman), tratando
...
El problema se formaliza como en  el caso Shannon, buscando la entrop\'ia m\'axima
sujeto  a  v\'inculos: sea  $X$  variable  aleatoria  viviendo sobre  $\X  \subset
\Rset^d$ con $K$ momentos \ $\Esp\left[ M_k(X) \right] = m_k$ \ fijos, con $M_x:
\X  \to \Rset$,  el problema  de $(h,phi)$-entrop\'ia  m\'axima se  formula  de la
...
cuenta los v\'inculos, el problema variacional consiste a resolver~\cite{GelFom63,
...
$- \phi'(p(x)) + \lambda^t M(x) = 0$. La funci\'on entr\'opica $\phi$ es c\'oncava y
...
caen  en  general en  la  familia exponencial.  De  una  forma, usando  entrop\'ia
...
\end{proof}
%
Este lema  prueba que, dando v\'inculos ``razonables'',  la $(h,\phi)$-entrop\'ia es
...
\item \SZ{Con  $K =  1$, \ $\X  = \Rset^d$  \ y \  $M(x) = x  x^t$ (visto  con $d^2$
  v\'inculos), y $\phi(u) = u^\beta$ (R\'enyi o Havrda-Charv\'at-Dar\'oczy), la  distribuci\'on  de  entrop\'ia  m\'axima  es \SZ{Student; Costa and son on. Gausiana se recupera caso l\'imite}.}
\end{itemize}
