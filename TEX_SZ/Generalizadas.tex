...
la informaci\'on,  un poco mas  de una  decada despues de  su papel clave  y muy
completo,  R\'enyi propuso  una  medida generalizada~\cite{Ren61}.  Su punto  de
vista  fue mas  matematico  que fisico  o  ingeniero.  Retom\'o  los axiomas  de
...
\paragraph{Una  clase  mas  general   y  sus  propiedades}  Las  entropias  tipo
Havdra-Charv\'at-Dar\'oczy  fueron  (re)descubiertos   varios  otras  veces  y/o
estudiados  mas  detenidamente en  varios  campos  y  varios extensiones  fueron
introducidas~\cite[entre  otros]{Var66, Oni66,  Kap67,  Vaj68, LinNie71,  Ari71,
  ...
  Rat91, Kan01, Bec09}.   Un primer enfoque mas general es  debido a S.  Arimoto
en  los  primeros  a\~nos  de  la decada  1970~\cite{Ari71}  y  rediscubierto  y
estudiado con mas detaller y una decada despues por Burbea y Rao~\cite{BurRao82}
...
\begin{proof}
  ...
  \leyenda{$\phi$   convexa:   la    variaci\'on   (cuerda)   $\frac{\phi(v)   -
    ...
    \phi'(p_i)$   con  $\hphi   \equiv   H_{(\id,\phi)},  \:   \id$  siendo   la
    identidad. La  cantidad positiva marcada por  la dupla-flecha $B_\phi(v\|u)$
    representa la divergencia de Bregman (ver mas adelante).}
  %
...
cota  superior~\ref{prop:SZ:cotamaxima}  (salvo si  se  pone  vinculos, ver  mas
...
$(h,\phi)$-entropia  diferencia   el  concava~\ref{prop:SZ:concavidadHPhi}.  Mas
...
lo   que   es    conocido   como   {\it   $J$-divergencia}~\cite{Kul51,   Kul68,
...
obtiene esta propiedad  cuando $\pi = [\frac12 \quad  \frac12]$. Ademas, en este
caso, a pesar de que la divergencia no cumpla la desigualdad triangular, aparece
que $\Big( J_{\mathrm{js}}^{[\frac12 \quad \frac12]}(p_1,p_2) \Big)^s, \quad 0 <
s   \le   \frac12$   es   una  metrica~[para   $s   =   \frac12$]\cite{EndSch03}
o~\cite[Teorema~1]{OesVaj03} (ver  tambi\'en~\cite{KafOes91}).  Si puede parecer
mas logico definir tal divergencia con a priori/proporciones $\pi_i$ iguales, de
hecho la versi\'on no simetrica, con pesos $\pi_i$ vuelve natural en el marco de
la   discriminaci\'on  donde  apareci\'o   implicitamente  esta   cantidad.   En
particular cuando  estamos frente a  dos hypotesis  $i = 1,  2$ o clases,  a las
cuales  la distribuci\'on  de las  observaciones  es $p_i$,  con probabilidad  a
priori $\pi_i$.  A partir de observaciones  $x$ hay que elegir si eran sorteando
de  $p_1$  o  $p_2$  (distribuciones  de sampleos,  \ie  condicionalmente  a  la
hypotesis).  El enfoque Bayesiano mas natural consiste maximizar la probabilidad
a  posteriori (probabilidad  de estar  en  hypotesis $i$  condicionalmente a  la
observaci\'on), y  se prueba  que la probabilidad  de error  es dada por  $P_e =
\sum_x \min( \pi_1 p_1(x)  \, , \, p_2 p_2(x) )$ (o con  una integral en el caso
...
observaci\'on $X$ y la variable de  selecci\'on $Z$, justificando aun mas su uso
natural     en    problemas    de     clasificaci\'on    o     selecci\'on    de
modelos. Incidentalmente, de $I(X;Z) = H(Z)  - H(Z|X) \le H(Z) \le \log(2)$ ($Z$
siendo discreta) se recupera la cota mayor de $\Djs$, y una cota aun mas fina
%
\[
\Djs(p_1,p_2) \le H(\pi)
\]
...
Mas alla,  en el campo  de la  clasificaci\'on, se puedre  tratar de mas  de dos
...
%
\[
\Djs(p_1,\ldots,p_n) = H\left( \sum_i \pi_i p_i\right) - \sum_i H(p_i)
\]
%
...
% ----- (h,phi)-Jensen
\paragraph{Clase de Burbea-Rao o  $\phi$-Jensen divergencia y generalizaci\'on a
  la $(h,\phi)$}

\SZ{ (0)  Jensen generalizada  caso H concava  avec phi,
  puis h phi. Ref. Nielsen ArXiv 2010, Nielsen Boltz 2011}
...
problemas de  minimizaci\'on convexa~\footnote{A\'un que aparece  en una revista
  ...
  momentos\ldots}  De manera  general,  sale  de \  $f:  \Omega \subset  \Rset^m
\mapsto \Rset$  \ convexa  y de  clase $C^1$ \  sobre \  $\Omega$, \  un cerrado
convexo de $\Rset^d$.  La divergencia de Bregman  de un punto \ $v \in \Omega$ \
relativamente a un punto \ $u \in  \Omega$ \ es definida por \ $B_f(v\|u) = f(v)
-  f(u)  -  (v-u)^t  \nabla  f(u)$~\cite{Bre67, Bas89,  Bas13,  toto}.   Eso  es
ilustrado figure~\ref{fig:SZ:ConvexidadDerivada}. Aplicado a  \ $f = \phi$, \ $u
= p(x)$  \ y  \ $v  = q(x)$ \  donde \  $p$ \  y \ $q$  \ son  distribuciones de
probabilidad, sumando en \ $x$, o, de manera equivalente, aplicado a \ $-\hphi$
\ siendo el conjunto de  distribuciones de probabilidades un convexo cerrado, se
define la divergencia de Bregman asociada a la \ $\phi$-entropia,
%
\begin{definicion}[$\phi$-divergencia de Bregman]
  Sean \ $p$ \ y \ $q$ \  dos distribuciones definidas sobre \ $\X$ \ y \ $\phi$
  convexa sobre $[0  \, ; \, 1]$.   La $\phi$-divergencia de Bregman de  \ $q$ \
  relativamente a \ $p$ \ es definida por
  %
  \[
  \bphi[q]{p}  = \hphi[p]  - \hphi[q]  - (p-q)^t  \nabla \hphi[p]
  \]
  %
lo que se escribe en el contexto discreto
  %
  \[
  \hphi[p] - \hphi[q] - \sum_{x \in  \X} \big( q(x) - p(x) \big) \phi'\big( p(x)
  \big)
  \]
  %
  y en el contexto continuo,
  %
  \[
  \hphi[p] - \hphi[q] - \int_\X \big( q(x) - p(x) \big) \phi'\big( p(x) \big) \,
  dx
  \]
  %
\end{definicion}
...
y a  la de  Bregman. \SZ{Tambien la JS; squared  Mahalanobis distance $u^t  Q u$  con distancia
...
\item Positividad:
  %
  \[
  \bphi[q]{p} \ge 0 \quad \mbox{con igualdad si y solamente si} \quad p = q
  \]
  %
  Esta propiedad  es la  consecuencia directa de  la convexidad de  $\phi$, como
  ilustrado figure~\ref{fig:SZ:ConvexidadDerivada}.
%
\item $\bphi[q]{p}$  es convexa  con respeto a  $q$, pero no  necesariamente con
  respeto a $p$. Es tambi\'en consecuencia directa de la convexidad de $\phi$.
%
\item  Pensando a  la $\phi$-divergencia  de Bregman  con respeto  a  $\phi$, es
  lineal  en el  sentido  de  que $D^{\mathrm{b}}_{\lambda  \phi_1  + \phi_2}  =
  \lambda  D^{\mathrm{b}}_{\phi_1}   +  D^{\mathrm{b}}_{\phi_2}$  (con  $\phi_i$
  convexas y $\lambda \ge 0$).
%
...
\SZ{vinculo con  Jensen:

Nielsen \&  Boltz IEEE IT 2011

Nielsen\&  Nock IEEE SPL 2017}
