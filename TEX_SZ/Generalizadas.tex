...
\begin{proof}
 ...
 \[
  ...
  que $\phi$ es necesariamente de la forma $\phi(x) = \eta \, x \ln x + \theta x
  +  \vartheta$. Debido  a la  continuidad de  $\phi$, queda  v\'alido  sobre el
  cerrado  $[0 \,  ; \,  1]$.  De  que se  aplica a  un vector  de probabilidad,
  sumando a uno, se puede reducir  el problema a $\theta = 0$ (poniendo $\theta$
  adentro  de  $\vartheta$  sin   cambiar  el  valor  de  entrop\'ia  obtenida).
  Adem\'as, del requisito $\phi(0) = 0$
...
son dichas {\it medidas equivalentes} en este caso).
...
\le \frac12$  es una metrica~\footnote{Se  necesita sol\'o de que  los $P_{(i)}$
  admiten una densidad con respeto a una medida $\sigma$-finita; nos referiremos
  al                                   resultado~\ref{Th:MP:DescompositionMixta},
  pagina~\pageref{Th:MP:DescompositionMixta}.}~\cite[Teorema~1                 \&
...
\begin{definicion}[$(h,\phi)$-divergencia]\label{Def:SZ:HPhiDivergencia}
  ...
  respeto a una distribuci\'on de referencia $P$ tal que $Q \ll P$
