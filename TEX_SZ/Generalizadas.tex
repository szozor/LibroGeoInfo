...
\item\label{Prop:SZ:concavidadHPhi} Si $h$ es c\'oncava, entonces $\hhphi[p]$ es
  ...
  existe  un  $\lambda^c(\alpha)  >  1$  tal que  para  cualquier  $\lambda  \le
  \lambda^c(\alpha)$  se conserva  la  concavidad, a  pesar  de que  $h$ no  sea
...
\]
%
donde  los   dos  primeros  v\'inculos  aseguran  de   que  $p_{\mathrm{em}}$  (positividad,
...
%
\[
p_{\mathrm{em}} =  \argmax_p \int_\X  \left( - \phi\left(  p(x) \right)  + \eta_0 \,  p(x) +
...
de $P  \ll Q$  con respeto a  $Q$.  La  densidad obtenida es  $p_{\mathrm{em}} =
