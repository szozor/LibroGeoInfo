...
%
\begin{teorema}
  Sea $p_{X,Y}$ distribuci\'on conjunta  de variables aleatorias discretas $X$ y
  $Y$ y $p_x$ y $p_Y$ las de $X$ y de $Y$ (marginales) cualquieras.
%
\[
\forall \: p_{X,Y} \quad H_{(h,\phi)}\left( p_{X,Y} \right) \: \le \:
H_{(h,\phi)}\left (p_X \otimes p_Y \right) \qquad \Leftrightarrow \qquad \phi(x)
= - x \log x
\]
%
...
\begin{proof}
  ...
  propidad~\ref{prop:SZ:subaditividad} con el hecho de que $h$ es decreciente en
  este caso.

  %Si $\phi$ es convexa (resp. concava), equivalentemente la desigualdad esperada
  %es  $H_\phi\left( p_{X,Y} \right)  \ge H_\phi\left(  p_X \otimes  p_Y \right)$
  %(resp.    desigualdad   reversa)    debido   a    la   decrecencia    de   $h$
  %(resp.  crecencia).
  ...
  %
  \[
  \forall \, u,v \quad \phi(v)  - \phi(u) \:  \le \:  (v-u) \, \phi'(u),
  \]
  %
  \ie la variaci\'on  (cuerda) es menore que la derivada  en $b$, como ilustrado
  figura~\ref{fig:SZ:ConvcavidadDerivada}   (desigualdad  reversa   para  $\phi$
  convexa).
  %
  \begin{figure}[h!]
  %
  \begin{center} \input{TIKZ_SZ/ConcavidadDerivada} \end{center}
  %
  \leyenda{$\phi$   concava:   la    variaci\'on   (cuerda)   $\frac{\phi(v)   -
      \phi(u)}{v-u}$  es menore  que  la derivada  $\phi'(u)$.   Aplicado a  dos
    distribuciones  $p$  y  $q$,  de  componentes  $p_i$  y  $q_i$,  se  obtiene
    $H_\phi(q) -  H_\phi(p) \le \sum_i (q_i-p_i) \phi'(q_i)$  con $H_\phi \equiv
    H_{(\id,\phi)}, \: \id$ siendo la identidad.}
  %
  \label{fig:SZ:ConcavidadDerivada}
  \end{figure}
  %
  ...
  interior de un conjunto, se obtiene para $\phi$ concava,
  %
  \[
  H_\phi\left( p_X \otimes p_Y \right)  - H_\phi\left( p_{X,Y} \right) \: \le \:
  c \: g(a,\alpha,b,\beta,c),
  \]
  %
  ...
  $H_\phi\left(   p_{X,Y}  \right)$   puede   ser  major   (resp.   menor)   que
  $H_\phi\left( p_X  \otimes p_Y \right)$,  y entonces, con la  crecencia (resp.
  decrecencia) de  $h$ que si  $g(a,\alpha,b,\beta,0)$ no es  identicamente cero
  sobre  $\mathring{D}$, $H_{(h,\phi)}$  no puede  ser subadditiva  (conjunta vs
  product of marginales).
  ...
  que $\phi$ es necesariamebte de la forma $\phi(x) = - \lambda \, x \ln x + \mu
  ...
  $h$   (sin  cambia   la   monotonicidad).   Para   que   $\phi$  sea   concava
  (resp. convexa)  hace falta tener $\lambda  > 0$ (resp.  $\lambda  < 0$) as\'i
  que,  sin perdida  de generalidad,  $\lambda$  puede ser  puesta tambi\'en  en
  $h$. Tomar $\phi (x) = - x \, \ln  x$ con $h$ creciente o $\phi (x) = x \, \ln
  x$ con $h$ decreciente es  completamente equivalente, as\'i que se puede fijar
  $\phi  (x) =  -  x \,  \ln x$  satisfaciendo  la ecuaci\'on  funcional, y  $h$
  creciente.

  $H_\phi   =  H$  siendo   subadditiva  (propidad~\ref{prop:SZ:subaditividad}),
  cualquier funci\'on queda subadditiva, lo que cierra la prueba.
  %
\end{proof}
...
Aplicaciones poner ac\'a  MaxEnt nous, Kesavan,  Kagan 63 A.M. \ MinDiv revisited...
