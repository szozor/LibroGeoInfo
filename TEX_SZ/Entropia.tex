...
\item\label{prop:SZ:cotamaxima} {\it Maximalidad:}  La entrop\'ia es acotada por
  ...
  notando $p_i  = p_X(x_i)$,  hay que minimizar  $\sum_i (-  p_i \log p_i  + \mu
  p_i)$ donde  el factor de Lagrange  $\mu$ se determinar\'a  para satisfacer el
  v\'inculo.   Se  obtiene  sencillamente  que  $\log  p_i  =  -\mu$,  dando  la
...
\item\label{prop:SZ:concavidad} {\it Concavidad:} La entrop\'ia es c\'oncava, en
  ...
  \]
  %
  Esta relaci\'on  es conocida como desigualdad de  Jensen~\cite{Jen06}.  Es una
  consecuencia directa de la convexidad de  la funci\'on $\phi: t \mapsto t \log
  t$, como  ilustrado en la figura  Fig.~\ref{fig:SZ:Concavidad}-(a).  La figura
  Fig.~\ref{fig:SZ:Concavidad}-(b) ilustra  como se puede obtener  una mezcla de
  distribuciones  de dos  probabilidad  $p_{(1)}$ (dado  izquierda) y  $p_{(2)}$
  (dado derecho)  haciendo una  elecci\'on aleatoria a  partir de una  moneda en
  este  ejemplo   (probabilidad  $\pi_1  =  1   -  \pi_2$  de   elegir  el  dado
  izquierda).\newline
  %
  \begin{figure}[h!]
  %
...
\item\label{prop:SZ:aditividad} {\it Aditividad:}  La entrop\'ia conjunta de dos
  ...
  \]
  %
  donde $\otimes$ es el producto  de Kronecker~\footnote{$p_X \otimes p_Y$ es un
    vector  de tama\~no  $\alpha \beta$  de component  $(i-1) \alpha  + j$-esima
    $p_X(x_i)  p_Y(y_j),   \quad  1  \le   i  \le  \alpha,   \:  1  \le   j  \le
    \beta$.\label{foot:SZ:Kronecker}} Se generaliza  sencillamente a un conjunto
  de variables aleatorias $\{ X_i  \}_{i=1}^n$ (o, equivalmente a un producto de
  Kronecker de un conjunto de vectores de probabilidades).
%
...
\equiv [ g_1(x_1 , \ldots  , x_d) \quad  \cdots \quad g_d(x_1 ,  \ldots ,
  x_d) ]^t$) \  y  \  $|\cdot|$ representa  el  valor absoluto  del
...
Al      final,     \underline{se      conservan      las     propiedades      de
  concavidad~\ref{prop:SZ:concavidad}~\footnote{Es  nuevamente  una consecuencia
    de la  desigualdad de Jensen $\Esp[\phi(X)] \ge  \phi\left( \Esp[X] \right)$
    para  $\phi$ convexa.  Si  $\phi$ es  estrictamente en  $1$, la  igualdad se
    alcanza si  y solamente si $X$  es determinista casi  siempre.  Se refier\'a
    a~\cite{Jen06} en  el caso discreto  y integral, o~\cite{Hol89,  Had93} para
    las  primeras semillas  de  esta desigualdad,  y entre  otros~\cite{Jes31:I,
      Jes31:II,       Per74,       Rud91}       para       versiones       m\'as
    generales.\label{foot:SZ:Jensen}},  de aditividad~\ref{prop:SZ:aditividad} y
