...
\item\label{prop:SZ:cotamaxima} {\it Maximalidad:}  La entrop\'ia es acotada por
  ...
  p_i \log  p_i + \mu  p_i)$.  Se obtiene  sencillamente que $\log p_i  = -\mu$,
  dando       la      distribuci\'on      uniforma.\newline       La      figura
  Fig.~\ref{fig:SZ:EntropiaBinaria} representa la entrop\'ia de un sistema a dos
  estados, de probabilidades \ $p_X = \begin{bmatrix} r & 1-r \end{bmatrix}^t$ \
  (ley de Bernoulli de parametro  $r$), entrop\'ia a veces dicha {\it entrop\'ia
    binaria}, en funci\'on  de $r$.  Esta figura ilustra ambas cotas  ($r = 1$ o
  1, $r  = \frac12$) as\'i  que la invariancia  bajo una permutaci\'on  ($h(r) =
  H(r,1-r) = H(1-r,r) = h(1-r)$).
  %
  \begin{figure}[h!]
  %
...
\item\label{prop:SZ:recursividad} {\it Recursividad:} Juntar dos estados baja la
  ...
  \ie  sean \  $X$ \  definido  sobre \  $\X$ \  y  \ $\bar{X}$  \ sobre  \
  $\bar{\X}$,
  %
  \[
  ...
  %
  \[
  H(p_X)  = H(p_{\bar{X}})  +  p_{\bar{X}}(\bar{x}_{\alpha-1}) \,
  H(\bar{q})
  \]
  %
