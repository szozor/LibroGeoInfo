...
%
\begin{definicion}[Escritura \'unica de la entrop\'ia discreta como diferencial]\label{Def:SZ:ShanonMu}
  Sea \  $X$ \ variable  aleatoria definida sobre  $\X$ discreto o  continuo. La
  entrop\'ia (diferencial) de \ $X$ \  de densidad de probabilidad \ $p_X$ \ con
  respeto a $\mu  = \mu_\X$ \ o \  $\mu = \mu_L$ seg\'un el caso,  se escribe de
  manera \'unica como
  %
  \[
  H(X) \equiv H(p_X) = - \int_{\Rset^d} p_X(x) \, \log(p_X) \, d\mu(x)
  \]
  %
  Se  puede entender  esta  definici\'on  para cualquier  $\mu$  y densidad  con
  respeto a $\mu$, que sea discreta, de Lebesgue, o cualquiera.
\end{definicion}}