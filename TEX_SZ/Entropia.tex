...
\item\label{prop:SZ:cotamaxima} {\it Maximalidad:}  la entrop\'ia es acotada por
  ...
  notando $p_i = p_X(x_i)$, hay que minimizar $\sum_i (- p_i \log p_i + \zeta \,
  p_i)$ donde el factor de Lagrange \ $\zeta$ \ se determinar\'a para satisfacer
  el  v\'inculo.  Se  obtiene  sencillamente que  $\log  p_i =  -\zeta$, dando  la
...
\begin{definicion}[Entrop\'ia conjunta]\label{def:SZ:EntropiaConjunta}
  ...
  \ie $ \forall \, (x,y) \in \X \times  \Y, \quad p_{X,Y}(x,y) = P( (X = x) \cap
  (Y = y)  )$.  La entrop\'ia conjunta de  Shannon de las variables \ $X$  \ e \
  $Y$ \ es definida por
  %
  \[
...
tal que  \ $P(X^\delta =  x_k) = p_{X^\delta}(x_k)  = p_X(x_k) \delta$.   \ Se
