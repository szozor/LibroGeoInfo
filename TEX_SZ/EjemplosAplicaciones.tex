...
Ya que la informaci\'on de  Fisher, contra-parte estadistica de la informaci\'on
n t\'erminos entr\'opicos,  una pregunta natural es estudiar como  se traduce el
principio de entrop\'ia m\'axima  en el marco de Fisher. La  respuesta no es tan
sencilla del hecho que en general la  informaci\'on de Fisher es matricial en su
marco  general,  y  m\'as  all\'a,  implica   a  la  vez  la  distribuci\'on  de
probabilidad y su gradiente. El problema de minimisac\'on de la informaci\'on de
Fisher bajo varios v\'inculos  no es un problema my antiguo  y no hay soluciones
bastantes generales como para el problema de entrop\'ia m\'inima. Fue estudia en
marcos particulares como se lo puede ver por ejemplo en~\cite{}.

\SZ{Caso no parametrica,
}

Siendo  $J$ una  matriz,  se enfoca  sobre su  traza,  informaci\'on escalar  de
Fisher.  Sea   entonces  \   $X$  \  variable   aleatoria  viviendo   sobre  (de
distribuci\'on de probabilidad de soporte) \  $X(\Omega) = \X \subset \Rset^d$ \
con \  $K \ge 0$  \ momentos \  $\Esp\left[ M_k(X) \right]  = m_k$ \  fijos, con
$M_x: \X  \to \Rset$. Suponemos que  $X$ tiene una densidad  \ $p$ \ de  clase \
$C^1$. El problema  de informaci\'on de Fisher m\'inima se  formula de la manera
siguiente:   sean    \   $M(x)   =    \begin{bmatrix}   M_1(x)   &    \cdots   &
M_K(x)  \end{bmatrix}^t$   \  y  \   $m  =   \begin{bmatrix}  m_1  &   \cdots  &
m_K \end{bmatrix}^t$ (si $K = 0$ \ $M$ desaparece)
%
\[
...
%
\begin{itemize}
\item El  primero  consiste  a  escribir  \  $p(x) =  \exp\left(  -  \int  \psi(x)  \,
dx \right)$.  Por una parte, eso  permite cancelar el v\'inculo  de positivdad y
luego  conduce a  una ecuaci\'on  conocida en  varios casos  como ecuaci\'on  de
Riccati. Esa es muy estudiada~\cite{}.
%
\item El secundo enfoque  consiste a escribir \ $p(x) = \left| \psi(x) \right|^2$. De nuevo, eso permite cancelar el v\'inculo de positivdad. M\'as all\'a, $\Psi$ juega un rol de funci\'on de onda en mec\'anica cu\'antica y precisamente va a satisfacer una acuaci\'on tipo Schr\"odinger~\cite{}.
\end{itemize}
...
Sea $p(x)  = \psi^2(x)$. Entonces,  se nota  inmediatamente que
$\left\| \nabla_x p \right\|^2 = 4 \, p \, \left\| \nabla_x p \right\|^2$ dando el problema de minimizaci\'on (minimizar $J$ o $J/4$ es equivalente)
%
\[
...
\]
%
En otros t\'erminos, $\displaystyle \L(x , \psi , \psi') = \left( \left\| \nabla \psi(x) \right\|^2 + \left( \eta_0 \,  
  + \eta^t M(x) \right) \, \psi^2(x) \right)$ que da la ecuaci\'on de Euler-Lagrange
%
\[
\div\left( \nabla_x \psi \right) - \eta^t M \, \psi =  \eta_0 \, \psi
\]
%
Esta ecuaci\'on es nada m\'as que la ecuaci\'on multidimensional de Schr\"odinger~\cite{} con un potencial de la forma
%
\[
V(x) =  \eta^t M(x)
\]
...
% Riccati 1720
% https://fr.wikipedia.org/wiki/Équation_de_Riccati
