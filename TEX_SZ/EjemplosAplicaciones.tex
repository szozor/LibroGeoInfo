...
%
\[
\varepsilon = \Pr[Y = 1 | X = 0] = 1 - \Pr[Y = 0 | X = 0] \qquad \mbox{y} \qquad
\vartheta = \Pr[Y = 0 | X = 1] = 1 - \Pr[Y = 1 | X = 1]
\]
%
...
\leyenda{(a): Canal  binario. La entrada  $X$ definida sobre  $\X = \{ 0  , 1\}$
  ...
  $\eta = \frac{v_0 + v_1}{2}+\Esp[\xi]$.   En este ejemplo, $\varepsilon$ \ y \
  $\vartheta$ \ van a ser caracterizados completamente por la distribuci\'on del
  ruido  (y  de  los  dos  niveles  posibles  de la  entrada),  pero  no  de  la
  distribuci\'on $p_X$.}
%~\cite{Sha48}.}
...
dada a partir de $s = \Pr[Y = 0] = \Pr[Y = 0 | X = 0] \Pr[X = 0] + \Pr[Y = 0 | X
= 1] \Pr[X = 1]$ es decir
%
\[
s = \Pr[Y  = 0] = \vartheta +  r (1-\varepsilon-\vartheta)
\]
%
...
hace falta maximizar  $I_2$ con respeto a $r$.  Diferenciando  $I_2$ en $r$, \ie
$\frac{\partial  I_2(X;Y)}{\partial  r}  =  \frac{\partial  h_2(s)}{\partial  s}
\frac{\partial s}{\partial r} - h_2(\varepsilon) + h_2(\vartheta)$, es decir
%
\[
...
\item Claramente,
  ...
  \vartheta$,  dando $p_{Y|X}  = p_Y$,  dando  una informaci\'on  mutua nula,  y
...
\item Si $\vartheta  \ne 1-\varepsilon$, la derivada de $I_2$  con respeto a $r$
  ...
  m\'aximo.  Poniendo el  $r^\opt$ en la formula de  $I_2(X;Y)$, luego de muchos
  c\'alculos (b\'asicos), se obtiene
  %
  \[
...
\ie la capacidad es menor que 1 bit~\footnote{De manera general, de la escritura
  ...
  $p_X$.   $p_X$  parteneciendo a  un  convexo, $I$  tiene  un  m\'aximo que  es
  \'unico.}: para transmitir informaci\'on  en este canal, hace falta introducir
redundancia en  el mensaje.   Se alcanza \  $C_2 =  1$ bit si,  (i) por  un lado
$H_2(Y|X) = 0$, es  decir \ $r h_2(\varepsilon) + (1-r) h_2(\vartheta)  = 0$ \ y
adem\'as (ii) \  $h_2(s) = 1$.  Estudiando cada caso  (ej.  con \ $r =  0$ \ y \
$\vartheta = 0$  \ se satisface (i) pero  no (ii) porque \ $s =  0$), se obtiene
que
%
\[
...
\leyenda{Informaci\'on mutua  (en bits) entrada-salida \ $I_2(X;Y)$  \ del canal
  binario en funci\'on de \ $r = \Pr[X  = 0]$.  \ (a): \ $\varepsilon = 0.4$ \ y
...
\leyenda{Ejemplos de canales discretos usuales.  (a): canal borrador, donde un 0
  ...
  uniforme.   (b):  canal tipo  machina  de escribir,  donde  cada  letra de  un
  ensemble de $n$ letras (ac\'a con $n = 26$) se transmite correctamente con una
  probabilidad $1-\varepsilon$ o a la  letra siguiente (de manera c\'iclica) con
  una probabilidad $\varepsilon$.   De $I_n(X;Y) = H_n(Y) -  H_n(Y|X) = H_n(Y) -
  h_n(\varepsilon)$ se deduce  que $I_n$ es m\'axima si $Y$  es uniforme, lo que
  es posible si $X$ es uniforma, dando $C_n = 1 - h_n(\varepsilon)$.}
%~\cite{Sha48}.}
...
% En  conclusi\'on,   tomando  en  cuenta   el  l\'imite  de  potencia,   $$C  =
...
importante del  c\'odigo es la  taza de codificaci\'on~\footnote{En~\cite{Rio07}
  ...
  codificado por el  mismo n\'umero de bits. La taza es  entonces el n\'umero de
  bits por s\'imbolo.}
%
\[
...
\begin{proof}
  ...
  existe   y   es  \'unico.    El   problema   se   resuelva  con   el   enfoque
