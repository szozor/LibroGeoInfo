...
\item Si $\vartheta  \ne 1-\varepsilon$, la derivada de $I_2$  con respeto a $r$
  ...
  m\'aximo.   Poniendo  \modif{la expresi\'on  de}  $r^\opt$  en  la formula  de
  $I_2(X;Y)$, luego de muchos c\'alculos (b\'asicos), se obtiene
  %
  \[
...
\ie la capacidad es menor que 1 bit~\footnote{De manera general, de la escritura
  ...
  $p_X$. \modif{A  continuaci\'on,} $p_X$ parteneciendo a un  convexo, $I$ tiene
  un m\'aximo  que es  \'unico.}: para transmitir  informaci\'on en  este canal,
hace falta introducir redundancia en el mensaje.  Se alcanza \ $C_2 = 1$ bit si,
(i)  por  un lado  $H_2(Y|X)  =  0$, es  decir  \  $r  h_2(\varepsilon) +  (1-r)
...
\end{figure}
%
\footnotetext{\modif{Se  mencionar\'a de que  en toda  esta subsecci\'on,  no se
    necesita de  que $X$  y/o $Y$ sean  variables aleatorias reales,  \ie pueden
    tomar  sus  valores  sobre   cualquier  espacio  discreto  (por  ejemplo  de
    letras).}}
...
\prod_i \left(  R_X + \Lambda_\Gauss  \right)_{i,i} = \prod_i \left(  \left( R_X
  \right)_{i,i}  +  \lambda^\Gauss_i  \right)$  \  \modif{donde  $(\cdot)_{i,i}$
  denota la  componente $i,i$ de  la matriz}, con  igualdad si y solamente  si \
...
se\~nal/proceso, canal filtrando,  canal con \modif{retroacci\'on (o feedback)},
etc.).
...
finito \modif{no necesariamente real ($X$ puede tomar cualquier etiqueta)}
%
\[
...
base  \ $d$  \ de  un entero,  \ie para  $1 \le  i \le  \alpha$  \modif{se puede
  escribir de manera  \'unica} \ $i-1 =  (i_0-1) + (i_1-1) d +  \cdots + (i_K-1)
d^K$ \  donde \ $K  = \Big\lceil \log_d  |\X| \Big\rceil$ \ y  \ $1 \le  i_k \le
\alpha$\modif{.   Entonces,   se   puede}   asignar  la   palabra   c\'odigo   \
$\modif{c(x_i) = }\cod_{i_0} \cdots \cod_{i_k}$ \ al s\'imbolo $x_i$.  Haciendo
eso, cada  palabra c\'odigo  tieno el mismo  largo.  Pero, es  m\'as econ\'omico
hacer  una codificaci\'on  dicha de  largos  variables, teniendo  en cuenta  las
probabilidades  de aparici\'on  de cada  $x_i$. Implicitamente,  es la  idea del
c\'odigo de Moorse, que asigna un  punto o series de puntos o c\'odigo peque\~no
a las letras muy  frecuentes (ej. un punto para el `e',  dos puntos para el `i',
etc.),  y   barras  o   combinaciones  largas  a   las  letras  que   son  raras
(ej. bara-bara-punto-bara para el `q' o cinco baras para el `0').  Dicho de otra
manera, el  c\'odigo ingenuo ser\'ia  ``eficaz'' para $x_i$ apareciendo  con las
mismas frecuencias/probabilidades.
...
%
\begin{ejemplo}[C\'odigo non singular, pero no decifrable]
...
\begin{proof}
  ...
  \]
  %
  \modif{$\widetilde{l}(x)$  no   es  necesariamente  entero,   as\'i  que  u}na
  posibilidad \modif{para volver  a largos enteros} puede ser  de tomar la parte
  entera superior \modif{de $\widetilde{l}(x)$},
  %
  \[
...
entrop\'ia de  Shannon.  Se notar\'a \modif{tambi\'en que}  de la demostraci\'on
precediente  de que  aparece un  c\'odigo  particular \modif{a  trav\'es de  los
  $\Big\lceil\!  - \log_d p_X(x) \Big\rceil$}:
%
\begin{definicion}[C\'odigo de Shannon]\label{Def:SZ:ShannonCode}
...
\]
%
Al lo contrario  de primer vista, un c\'odigo de  Shannon no es \'optimo\modif{,
  como se lo puede ver con el ejemplo siguiente.}
%
\begin{ejemplo}\label{Ej:SZ:CodigoShannonNoOpt}
  Sea \  $\X = \C  = \{ 0 \,  , \, 1  \}$ \ y  \modif{una fuente $X$ tal  que} \
  $p_X(0) = 0.999 = 1 - p_X(1)$.  Los  largos de Shannon van a ser \ $l^\sh(0) =
  1$ \ y \ $l^\sh(1) = 10$, y  el largo promedio vale \ $L\left( c^\sh \right) =
  1.009$.   Obviamente, un  c\'odigo \'optimo  es \  $c(x) =  x$ \  de  largos \
  $l_c(x) = 1$ \ dando \ $L^\opt = 1$ bit.
\end{ejemplo}
%
...
%
\begin{proof}
  \modif{Por  definici\'on de  la  parte entera  superior,  $a +  1  > \lceil  a
    \rceil$, as\'i  que $\lceil a  \rceil \ge  b \: \Rightarrow  \: a >  b-1$. A
    continuaci\'on, de  la implicaci\'on  de eventos $(Y  \ge a) \subset  (Y \ge
    b-1)$ \  dando $P(A \ge a)  \le P(Y \ge b-1)$  \ y de la  definici\'on de un
    c\'odigo de Shannon} se obtiene
  %
  \begin{eqnarray*}
  ...
  %
  & = &  \sum_{x \in \X : p_X(x) \le d^{-l_c(\modif{x})  - k  + 1}} p_X(x)
  \end{eqnarray*}
  %
  Pero, sumando sobre  lo $x$ tal que $p_X(x) \le d^{-l_c(\modif{x})  - k + 1}$,
  se obtiene
  %
  \[
  %  \begin{eqnarray*}
  P\Big( l^\sh(X)  \ge l_c(X) + k  \Big) \le \frac{1}{d^{k-1}} \sum_{x  \in \X :
    p_X(x) \le d^{-l_c(\modif{x}) - k + 1}} d^{-l_c(\modif{x})}
  %\\[2.5mm]
  %
  %P\Big( l^\sh(X) \ge l_c(X) + k \Big) & \le & 
  \le \frac{1}{d^{k-1}} \sum_{x \in \X} d^{-l_c(\modif{x})}
  \]
  %\end{eqnarray*}
  %
...
\end{proof}
%
Este  teorema  traduce  el hecho  de  que  \modif{a  pesar  de que}  $c^\sh$  no
\modif{sea} \'optimo,  tomando cualquier otro c\'odigo,  incluyendo el \'optimo,
la probabilidad que $c^\sh(X)$ tenga  un largo m\'as grande que $c(X)+k$ decrece
...
%
\begin{definicion}[C\'odigo de Fano]\label{Def:SZ:FanoCode}
...
%
\begin{definicion}[C\'odigo de Huffman]\label{Def:SZ:HuffmanCode}
...
\item Si \ $\frac{\alpha  - d}{d-1}$ \ no es entero, en  lugar de completar $\X$
  ...
  Kraft-McMillan\modif{,  pero no es  contradictorio con  el punto  anterior que
    contaba los largos de estados fictivos(que no codificamos en realidad).}
%
...
\item  El algoritmo  no  permite conocer  los  largos de  manera anal\'itica  en
  ...
  solamente  implementando el  algoritmo (una  vez que  es  construido \modif{el
...
$H_d(X) = \log_d |\modif{\X}|$, precisamente la incerteza del enfoque de Hartley
...
Para codificar una fuente, que se haga el c\'odigo \'optimo\modif{de Fano,} o de
