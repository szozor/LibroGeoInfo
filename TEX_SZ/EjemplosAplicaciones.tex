...
salida del canal.   Lo m\'as $I$ es grande, lo m\'as  de informaci\'on se transmite.
Dado el canal,  se puede arreglar $X$ (su distribuci\'on)  de manera a maximizar
$I(X;Y)$, es decir la cantidad m\'axima  que se puede transmitir en este canal. Es
...
simbolo. En  este marco, un  canal de comunicaci\'on  lo m\'as simple  es conocido
...
figura Fig.~\ref{fig:SZ:CanalBinario}-(a).   La  figura Fig.~\ref{fig:SZ:CanalBinario}-(b)
...
En este caso, trabajando con bits, el logaritmo lo m\'as l\'ogico es el de base 2.
Sea
%
\[
\alpha = \Pr[X = 0]
...
es la entrop\'ia binaria en bits.   Para calcular la capacidad $C_2$ en bits, hace
falta  maximizar  $I_2$  con  respeto  a  $\alpha$.   Diferenciando  $I_2$  en
...
binario, de entrop\'ia m\'axima en el caso uniforme), aparece sin calculos que
%
\[
C_2 \le 1 \: \mbox{bit}
\]
...
\ie la capacidad es menor que 1 bit~\footnote{De manera general, de la escritura
  ...
  $\B$, da $0 \le C \le \min( \log  |\X| \, , \, \log |\B| )$. Adem\'as, $p_{Y|X}$
  ...
  concavidad de  $H$ se obtiene  que $I$ es  c\'oncava con respeto a  $p_X$. $p_X$
...
$\alpha h_2(p)  + (1-\alpha) h_2(q)  = 0$  \ y adem\'as  (ii) \ $h_2(\beta)  = 1$.
...
La figura Fig.~\ref{fig:SZ:ICanalBinario} representa  la informaci\'on mutua $I(X;Y)$
para unos canales  ($p$ y $q$ dados)  en funci\'on de $\alpha$.  Se  nota que la
curva   es  c\'oncava   y  tiene   un   m\'aximo,  capacidad   del  canal.    La
figura Fig.~\ref{fig:SZ:CCanalBinario} representa la capacidad del canal en funci\'on
...
discretos,  binario  o   con  m\'as  estados.   Unos  sont   representados  en  la
...
\leyenda{Ejemplos de canales discretos usuales.  (a): canal borrador, donde un 0
  (de probabilidad de  ocurrencia $\alpha$) o 1 (de  probabilidad de occurrencia
...
figura Fig.~\ref{fig:SZ:CanalGaussiano}.  Se supone  conicida la matriz de covarianza
...
entrop\'ia diferencial  implica que $H(Y)$  sea m\'axima si  y solamente si  $Y$ es
...
\right|$. Tomando en cuenta el  l\'imite de potencia, hace falta maximizar $\left|
...
$R_X$ \ debe ser diagonal (dada una  diagonal, se alcanza el m\'aximo si los otros
...
se resolva con el enfoque de Karush-Kuhn-Tucker~\footnote{Se introduce el factor
  ...
  $\lambda^X_i = \left( \lambda - \lambda^\Gauss_i \right)_+$. Deben ser los m\'as
  grande,  es decir $\lambda$  lo m\'as  grande que  se puede,  pero satisfaciendo
...
se\~nale-ruido~\footnote{Esta formula es muy  parecida a la de Shannon, Laplume,
  ...
  \frac{1}{2  T}$ (caso  l\'imite). Si  el ruido  es blanco  en la  banda  $B$, de
  densidad espectral de potencia por unidad de frecuencia igual a $N_0$, para un
  simbolo la  relaci\'on se\~nal-ruido se escribe $\frac{P}{N_0  B}$. Adem\'as, se
...
general todo se codifica en bits $\C = \{ 0 \, , \, 1\}$.  M\'as formalmente, sean
%
\[
...
c\'odigo  tieno  el   mismo  largo.   Pero,  parece  m\'as   economico  hacer  una
...
ilustradas           en           la           figura Fig.~\ref{fig:SZ:ClasesCodigos}
...
Adem\'as de  la decodificaci\'on sin ambiguedad,  una caracterizaci\'on importante
...
\]
%
Antes de ir m\'as adelante, falta traducir en ecuaci\'on el v\'inculo de que $c$ sea
...
\begin{proof}
  ...
  $c_k$ debe ser  inyectiva, imponiendo $\#(m) \le d^m$ (no  hay m\'as palabras de
  ...
  figura Fig.~\ref{fig:SZ:ArbolKraft}  para \ $d  = 2$  \ y  \ $\C  = \{0  \, ,  \, 1
...
\item Los largos  de un c\'odigo unicamente decodable  satisfacen la desigualdad
  ...
  de optimalidad (buscar  el la clase m\'as amplia de  los unicamente decodable no
...
\leyenda{Arbol de Kraft  en el caso binario  ($d = 2$). De la  raiz, de c\'odigo
  ...
  $1$.  Etc.  En cada  nudo,  en  el c\'odigo,  se  marca  en  negrita la  letra
  correspondiente al  bit a\~nadido al  c\'odigo padre.  Para hacer  un c\'odigo
  libre de  prefijo, una vez  que un nudo  es seleccionado para ser  una palabra
  c\'odigo (encuadrado  en la figura), no  se puede tener  nudo ``hijos'' siendo
  tambi\'en una palabra c\'odigo: se boran las ramas saliendo de un nudo-palabra
  c\'odigo (ramas punteadas).}
%
\label{fig:SZ:ArbolKraft}
\end{figure}

El formalismo dado, se va a ver  ahora reapacer la entrop\'ia de Shannon como cota
...
\begin{teorema}[Cota inferior de c\'odigos unicamente decodable]
  ...
  largo promedio esta acotado por debajo  por la entrop\'ia de Shannon de base $d$
  de $X$,
  %
  \[
...
puede ser m\'as bajo que el contenido informacional de la fuente. En este sentido,
...
\begin{teorema}[Cota superior del c\'odigo unicamente decodable \'optimo]
  ...
  de largo promedio m\'inimo esta acotado  por arriba por la entrop\'ia de Shannon
  de base $d$ de $X$ m\'as un {\it dit} (1 simbolo de $\C$), 
  %
  \[
  L^\opt \: < \: H_d(X) + 1
  \]
...
veces dicha {\it codificaci\'on entr\'opica} y da un rol operacional a la entrop\'ia
...
tiene un largo m\'as importante  que $c(X)+k$ decrece exponencialmente con $k$. El
...
a lo  m\'as equiprobables  que se  puede (\ie de  probabilidad a  lo m\'as  cerca de
$d^{-1}$) y de asignar $\zeta_i$ al conjunto $i$.  Luego, se repite el proceso a
cada sub-conjunto (para tener sub-conjuntos  de probabilidades a lo m\'as cerca de
$d^{-2}$) y  al subconjunto  $j$ del conjunto  $i$ se  va a asignar  le c\'odigo
$\zeta_i      \zeta_j$,      etc.      Eso      es      ilustrado     en      la
figura Fig.~\ref{fig:SZ:FanoHuffmanCodes}-(a).  \SZ{Probar/mencionar que tambi\'en $$
...
\begin{definicion}[C\'odigo de Huffman]
  Suponemos que existe un $q \in \Nset$ tal que~\footnote{Si no, se puede elegir
    ...
    va a cambiar ni la entrop\'ia,  ni el largo promededio del c\'odico aferente.}
  ...
  %
  \item   Si  quedan   m\'as   de   un  (super-)simbolo   fuente,   volver  a   la
...
\begin{proof}
  ...
  binario, pero la  extensi\'on para $d >  2$ es un poco m\'as  subtile. La prueba
  m\'as general es dada por Huffman~\cite{Huf52} y se consigue tambi\'en por parte
  ...
  \item  Como descrito  antes, se  puede permutar  las letras  c\'odigos  de una
    ...
    los $m'$ simbolos de probabilidades  las m\'as peque\~nas tienen el mismo nudo
  ...
  \item  Para   c\'odificar  $\X^{(2)}$,  se  necesita  por   lo  menos  $\lceil
    ...
    los $d$ simbolos de probabilidad m\'as  debiles son el la \'ultima profundez y
  ...
  \item Notando que \ $|\X^{(2)}| = n-(q-1) (d-1)$, \ el razonamiento se progaga
    ...
    juntando los  $d$ super-simbolos de probabilidades m\'as  debiles, hasta tener
...
\item Si si \ $\frac{n - d}{d-1}$ \ no es entero, en lugar de completar $\X$ con
  ...
  de  probabilidades m\'as debiles  en un  super-simbolo, y  luego hacer  la bucla
  descrita (juntando por super-simbolos de  $d$ simbolos en cada bucla); en este
  caso, no se satura m\'as la desigualdad de Kraft-McMillan.
%
...
\leyenda{Construcci\'on de  un c\'odigo binario  sobre $\C =  \{0 \, , \,  1 \}$
  ...
  dividiendo  los conjunto  en sub-conjuntos  a  lo m\'as  equiprobables. En  esta
...
Eso es  descrito figura Fig.~\ref{fig:SZ:CodigosExtensiones}.  Lo  mismo puede ocurir
con el c\'odigo  de Shannon \SZ{y lo de Fano}. Adem\'as,  el cardinal del alfabeto
...
\leyenda{$\frac{L^{\opt,n}}{n}-H_d(X)$  (puntos),   diferencia  entre  el  largo
  ...
  sirve como guia. En  esta ilustraci\'on se usa el ejemplo lo  m\'as simple con \
...
da una  interpretaci\'on operacional  a la entrop\'ia  relativa, o  divergencia de
...
caracteristicas  del  canal  de  transmisi\'on.    Estos  van  m\'as  all\'a  de  la
ilustraci\'on  de  esta  secci\'on.   Ver~\cite{Ber74, Gal78,  Say03,  CovTho06,
  Rio07}  entre  otros  para  tener   m\'as  detalles  sobre  varios  esquemas  de
...
\SZ{Feder Merhav IT'94 et lien avec discrimination; Vacisek en test de Gaussianite et cf plus loin avec generaloses Go75 etc}