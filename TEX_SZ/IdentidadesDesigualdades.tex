...
\begin{proof}
  ...
  \    $\Dkl[p_{(n+1),(n)}]{q_{(n+1),(n)}}   =    \Dkl[p_{(n+1)}]{q_{(n+1)}}   +
  \Dkl[p_{(n+1)|(n)}]{q_{(n+1)|(n)}}       =       \Dkl[p_{(n)}]{q_{(n)}}      +
  \Dkl[p_{(n)|(n+1)}]{q_{(n)|(n+1)}}$.       Adem\'as,      $p_{(n+1)|(n)}     =
  p_{X_{n+1}|X_n}         =         q_{(n+1)|(n)}$,        conduciendo         a
  $\Dkl[p_{(n+1)|(n)}]{q_{(n+1)|(n)}}  =   0$  \  con  consecuencia   de  que  \
  $\Dkl[p_{(n+1)}]{q_{(n+1)}}         =         \Dkl[p_{(n)}]{q_{(n)}}         +
  \Dkl[p_{(n)|(n+1)}]{q_{(n)|(n+1)}}$.   \ $p_{(n)|(n+1)}$ no  es necesariamente
  igual a \ $q_{(n)|(n+1)}$, pero  la divergencia siendo no negativa, se obtiene
  la primera desigualdad.  La segunda  desigualdad se obtiene tomando \ $q_{(n)}
  = p^*$.   Adem\'as, si  \ $p^*$ \  es uniforme  \ $p^*(x) =  \frac{1}{|\X|}$ \
  dando \ $\Dkl[p_{(n)}]{p^*} = -H(X_n)  + \log |\X|$, conduciendo a la \'ultima
  desigualdad.
\end{proof}
