...
$\Esp\left[   M_k(X)   \right]   =   m_k$    \   fijos,   con   $M_x:   \X   \to
\Rset$. \modif{Suponemos de que $X$ tiene una densidad \ $p$ \ con respeto a una
  medida $\mu$, basicamente \ $\mu_L$ \ en el contexto continuo y $\mu_\X$ en el
  caso       discreto. E}l problema de  entrop\'ia m\'axima se formula de
la  manera siguiente:  sean \  $M(x) =  \begin{bmatrix} 1  & M_1(x)  &  \cdots &
  M_K(x)  \end{bmatrix}^t$ \  y  \  $m =  \begin{bmatrix}  1 &  m_1  & \cdots  &
  m_K \end{bmatrix}^t$ (si $K = 0, \: M = m = 1$), \ se busca,
%
\[
...
\]
%
donde $\eta$ ser\'a determinado para  satisfacer a los v\'inculos.  \modif{En el
  caso    continuo    $\mu    =    \mu_L$    se   usa}    la    ecuaci\'on    de
Euler-Lagrange~\cite{GelFom63, Bru04}, esquematicamente anulando la ``derivada''
del  integrande  con  respeto a  $p$\modif{;  en  el  caso discreto,  se  anula}
realmente un  gradiente con respeto  a los componentes de  $p$. Reparametrizando
los factores de Lagrange, se obtiene \modif{as\'i}
%
\[
p^*(x) = \e^{\eta^t M(x)},
\]
...
Esta  distribuci\'on  cae  en  la  \modif{familia exponencial  que  hemos  visto
  secci\'on~\ref{Sec:MP:},}  donde  los   $M_k$  son  \modif{las  estad\'isticas
  suficientes}  y los  $\eta_k$ \modif{los  par\'ametros naturales}
...
\begin{lema}\label{Lem:SZ:MaxEntPruebaInfo}
  Sea \ $\displaystyle  \P_m = \left\{ p \ge 0: \:  \int_\X M(x) \, \modif{p(x)}
    \, \modif{d\mu(x)} = m \right\}$ \ \modif{conjunto de densidades con respeto
    a $\mu$, con los  mismos momentos $M$,} y sea \ $p^* \in  \P_m$ \ que sea de
...
\begin{proof}
  ...
  solamente si \modif{$p = p^* \quad (\mu$-c.s.).}
\end{proof}
%
...
\end{itemize}
%
\modif{Con  el  enfoque   del  lema~\ref{Lem:SZ:MaxEntPruebaInfo},  se  necesita
  solamente  que $p$  sea una  densidad  con respeto  a una  medida $\mu$  fija,
  cualquiera. En  particular, si es  una medida de probabilidad  (de referencia)
  $\widetilde{P}$, el problema de entrop\'ia  m\'axima vuelve ser un problema de
  minimizaci\'on de la divergencia de  Kullback-Leibler entre $P$ y la medida de
  referencia, siendo $p$ la densidad con respeto a esta medida (ver definici\'on
  Def.~\ref{Def:SZ:entropiarelativa}). Es decir, tomando \ $\mu = \widetilde{P}$
  \  medida   de  probabilidad  aparece   inmediatamente  \[  P^*   =  \argmax_P
  \Dkl[P]{\widetilde{P}} \quad \mbox{sujeto a} \quad  \int_\X M(x) \, dP(x) = m
  \qquad  \Leftrightarrow  \qquad  \frac{dP^*}{d\widetilde{P}}(x)  =  \e^{\eta^t
    x}. \] }
...
%
\begin{teorema}[Desigualdad de la potencia entr\'opica]\label{Th:SZ:EPI}
...
\item\label{EPI:SZ:PresCov}    {\it    Desigualdad    de    preservaci\'on    de
  ...
  covarianza proporcionales.   \modif{Claramente, se  cumple la igualdad  para \
    $\widetilde{X}$ \  e \  $\widetilde{X}$, entonces $H\left(  \sqrt{a} \,  X +
      \sqrt{1-a}  \,  Y   \right)  \ge  a  \,  H(X)  +   (1-a)  \,  H(Y)  \qquad
    \Leftrightarrow \qquad H\left(  \sqrt{a} \, X + \sqrt{1-a}  \, Y \right) \ge
    H\left( \sqrt{a} \, \widetilde{X} +  \sqrt{1-a} \, \widetilde{Y} \right) $ \
    lo que es nada m\'as que la desigualdad anterior} reemplazando \ $X$ \ por \
...
\begin{lema}[Versi\'on informacional de la segunda ley de la termodin\'amica]
  ...
  Markov, con probabilidades  de transici\'on \modif{$r_{X_{n+1}|X_n=x_n}$ dadas
    (independientemente de la  condici\'on inicial)}.  Estas \'ultimas modelizan
...
\begin{proof}
  ...
  \modif{$p_{X_{n+1}|X_n=x_n}  = r_{X_{n+1}|X_n=x_n}  =  q_{X_{n+1}|X_n =  x_n}$
...
%{\it Antes de  ir m\'as adelante, mencionamos que en  esta secci\'on, se usar\'a
...
\begin{definicion}[Matriz informaci\'on de Fisher par\'ametrica]
  ...
  probabilidad $p_X(\cdot;\theta)$ \modif{(con respeto a una medida $\mu$ dada)}
  sobre $\X \subseteq  \Rset^d$ su soporte. Suponga que  $p_X$ sea diferenciable
  \modif{con  respeto a}  $\theta$  sobre  $\Theta$.  La  matriz  de Fisher,  de
  ...
  donde \  $\nabla_\theta = \left[ \cdots  \: \frac{\partial}{\partial \theta_i}
    \: \cdots \right]^t$ \  es el gradiente en \ $\theta$ \modif{\  y \ $\log$ \
    el  \underline{logaritmo natural}}.   Es la  matriz de  covarianza  del {\it
  ...
  condiciones de regularidad, se puede mostrar~\footnote{Es una consecuencia del
    ...
    p_X(X;\theta)]$}
% \  vale tambi\'en  menos el  promedio  de la
...
\modif{En  el caso  continuo, $\mu  =  \mu_L$, con  una densidad  diferenciable,
  t}omando el  gradiente en \  $x$ \ en  lugar de \ $\theta$  \ da la  matriz de
informaci\'on de Fisher no param\'etrica,
%
\begin{definicion}[Matriz informaci\'on de Fisher no param\'etrica]
  Sea \ $X$ \ una  variable aleatoria continua \modif{admitiendo una densidad de
    probabilidad}  \  $p_X$ \  definida  sobre \  $\X  \subseteq  \Rset^d$ \  su
  soporte.  Suponga que \ $p_X$ \ sea diferenciable \modif{con respeto a \ $x$}.
  ...
  condiciones   de    regularidad,   \modif{$J(X)   =    -   \Esp[\Hess_x   \log
    p_X(X;\theta)]$}.
...
\item Es sencillo ver  que, por definici\'on \ $J_\theta(X)$ \ y  \ $J(X)$ \ son
  ...
  positivas).
  %  \ donde  estas  desiguladades  significan que  las  matrices son  definidas
  % positivas (los autovalores son positivos).
  Adem\'as,
  %
  \[
  ...
  \]
  %
  \modif{con  $A^{-t}  =  \left(  A^{-1}  \right)^t =  \left(  A^t  \right)^{-1}
    $}~\cite{CovTho06,  DemCov91, Bar86}.  Esta relaci\'on  da a  \ $J(X)$  \ un
  sabor de informaci\'on en  el sentido de que, cuando \ $A$  \ es real y tiende
  al infinito,  \ $J(AX)$ \ tiende  a 0; \ $A  X$ \ tiende a  ser muy dispersada
  as\'i que no hay informaci\'on sobre su posici\'on.
%
...
\begin{teorema}[Desigualdad de Cram\'er-Rao]
  ...
  \]
  %
  %donde $A \ge 0$ significa que  $A$ es definida no negativa.  
...
\begin{proof}
  ...
  se obtiene~\footnote{Se supone que  los integrandes sean \ $\theta$-localmente
    ...
    integraci\'on;  \modif{ver  tambi\'en teorema~\ref{Th:MP:IntegralParametro},
      pagina~\pageref{Th:MP:IntegralParametro}.}}
  %
  \begin{eqnarray*}
  ...
  % ~\footnote{De hecho, fue probada por Cauchy para sumas en 1821~\cite{Cau21},
  %   para integrales  por Bunyakovsky  en 1859~\cite{Bou59} y  m\'as elegamente
  %   por Schwarz en 1888~\cite{Sch88}. Ver tambi\'en~\cite{Ste04}.}
  conduce a
  %
  \[
  ...
  \modif{Para $\theta$ parametro de  posici\'on, $\widehat{\Theta} = X$, c}on la
...
\begin{proof}
  ...
  \frac{\partial \left( T \Sigma_\Gauss  T^t \right)}{\partial T_{i,j}} \left( T
    \Sigma_\Gauss T^t \right)^{-1}$.  \modif{Usando le vector \ $\un_i$  \ con 1
    en su \ $i$-\'esima} componente, y cero si no, se obtiene
  %
  \begin{eqnarray*}
  ...
  \end{eqnarray*}
  %
  usando \modif{la relaci\'on}  \ $x^t A \un_k \un_l^t  B x = \un_l^t B  x x^t A
  \un_k =  \un_k^t A^t x  x^t B^t  \un_l$ \ (escalares  comutan y un  escalar es
  igual a  su transpuesta) y  usando la simetr\'ia  de \ $T  \Sigma_\Gauss T^t$.
  Eso significa que
  %
  \[
  ...
  Suponiende que  se puede intervertir derivadas  y integrales (ver~\cite{Bar84,
    Bar86}    donde    se     dan    condiciones    \modif{rigurosas,    y    el
    teorema~\ref{Th:MP:IntegralParametro},
    pagina~\pageref{Th:MP:IntegralParametro}}),    $\displaystyle    p_Y(y)    =
  \int_{\Rset^d} p_X(x) \, p(y-x) \, dx$ \ \modif{(ver ejemplo~\ref{Ej:MP:Suma},
...
\begin{definicion}[Matriz informaci\'on de Fisher par\'ametrica condicional]
  ...
  \]
  %
  \modif{donde  $p_{X|Y}(\cdot;\theta) = \frac{p_{X,Y}(\cdot,Y;\theta)}{p_Y(Y)}$
    es ac\'a una variable aleatoria.}
\end{definicion}
...
\begin{proof}
  ...
  distribuciones de  \ $X$  \ y de  \ $Y$  \modif{(ver ejemplo~\ref{Ej:MP:Suma},
