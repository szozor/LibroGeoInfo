...
Sea $X$ variable aleatoria viviendo  sobre (de distribuci\'on de probabilidad de
...
\item Es sencillo ver  que, por definici\'on \ $J_\theta(X)$ \ y  \ $J(X)$ \ son
  ...
  \]
  %
  \cite{CovTho06, DemCov91, Bar86}. Esta relaci\'on da  a \ $J(X)$ \ un sabor de
  informaci\'on  en el  sentido de  que, cuando  \  $A$ \  es real  y tiende  al
...
%~\footnote{De  hecho, fue  probada  por Cauchy  para
...
\begin{definicion}[Matriz informaci\'on de Fisher par\'ametrica condicional]
  ...
  sobre   $\X   \times   \Y$   su  soporte,   $p_{X|Y}(\cdot,\cdot;\theta)$   la
  distribuci\'on condicional  de $X$ connociendo  $Y$ y $p_Y$  la distribuci\'on
  ...
  $p_{X|Y}(\cdot,\cdot;\theta)$, es decir
  %
  \[
  J_\theta(X|Y)  =  \Esp\left[  \Big(  \nabla_\theta \log  p_{X|Y}(X,Y;\theta)  \Big)
    \Big( \nabla_\theta \log p_{X|Y}(X,Y;\theta) \Big)^t \right]
  \]
\end{definicion}
