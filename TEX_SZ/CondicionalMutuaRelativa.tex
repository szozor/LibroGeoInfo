...
Siendo  $H(X|Y=y)$ una  entrop\'ia, va  a heredar  todas las  propiedades de  la
entrop\'ia  (o  entrop\'ia  diferencial).    Adem\'as,  de  $p_{X,Y}(\cdot,y)  =
p_{X|Y=y}(\cdot) p_Y(y)$ se deduce la propiedad siguiente:
%
\begin{propiedades}
...
De la regla de la cadena $H(X,Y) = H(X|Y)  + H(Y) = H(Y|X) + H(X)$ surge que las
