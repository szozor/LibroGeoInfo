...
condicional         de          probabilidad         \modif{$p_{X|Y=y}         =
  \frac{p_{X,Y}(\cdot,y)}{p_Y(y)}$.}  Con  esta restricci\'on, se  puede evaluar
una incerteza sobre $X$, sabiendo que $Y=y$,
%
\[
H(X|Y=y) = H\left( \modif{p_{X|Y=y}} \right).
\]
%
Entonces, condicionalmente a la variable aleatoria $Y$, la incerteza va a ser el
promedio    estad\'istico   sobre    todos    los   estados    $Y$   es    decir
\modif{$\displaystyle H(X|Y) = \int_{\Rset^d} p_Y(y) H(X|Y=y) \, d\mu(y)$ \ (con
  la medida $\mu$ adecuada).}
%
\begin{definicion}[Entrop\'ia condicional]\label{Def:SZ:entropiacondicional}
  ...
  $d_Y$-dimensionales. La entrop\'ia condicional de \ $X$, \modif{con respeto a}
  \ $Y$, es definida por\modif{
  %
  \[
...
Si $X$ e $Y$ son indepedientes, $p_{X|Y=y}$ se reduce a $p_X$, as\'i que vale cero
la entrop\'ia condicional: \modif{obviamente,}
%
\begin{propiedades}
...
%
Esta  propiedad
%  vale  en ambos  casos,  discreto  como  continuo.  En  el  casodiscreto, 
se interpreta como el hecho  de que $Y$ no lleva ninguna informaci\'on
...
%  (v\'alida para  la  entrop\'ia  como  para su  extensi\'ondiferencial)
%
\begin{propiedades}
\item\label{Prop:SZ:cadena}  {\it Regla de  cadena}
  ...
  %
  Esta regla
  % , v\'alida en ambos casos, discreto como continuo,
...
\begin{definicion}[Informaci\'on mutua]\label{Def:SZ:mutua}
  ...
  \]
  %
  %En el caso discreto s
  Se expresa\modif{
  %
  \[
  I(X;Y)  =  \int_{\Rset^{d_X} \times \Rset^{d_X}}   p_{X,Y}(x,y)  \log  \left(  \frac{p_{X,Y}(x,y)}{p_X(x)
      p_Y(y)} \right) \, d\mu(x,y)
  \]
  %
  con \ $\mu$ \  medida adecuada (discreta en el caso discreto  o de Lebesgue en
  el caso diferencial).}
...
ensemblista, como descrita  en la figura Fig.~\ref{Fig:SZ:Venn}\modif{, diagrama
  de    Venn    o    de    Euler   (ver    nota    de    pie~\ref{Foot:MP:Euler}
...
\begin{definicion}[Entrop\'ia relativa]\label{Def:SZ:entropiarelativa}
  La  entrop\'ia relativa, o  divergencia de  una \modif{medida  de probabilidad
    $Q$}, con  respeto a  una \modif{medida de  probabilidad} de  referencia $P$
  \modif{tal que \underline{$Q \ll P$}, ambas definidas sobre $\Rset^d$,}
  % donde \underline{el soporte de $p$ incluye lo de $q$} ($p(x) = 0 \Rightarrow
  % q(x) = 0$),
  es definida como\modif{
  %
  \[
...
linea que Shannon, interpretando \ \modif{$\log\left(\frac{dQ}{dP}(x)\right)$} \
como   una  informaci\'on   de   discriminaci\'on  entre   dos  hip\'otesis   de
distribuciones \  $Q$ \  y \  $P$ \  a partir de  la observaci\'on  \ $x$,  \ la
divergencia siendo la  informaci\'on de discriminaci\'on promedia.  Introdujeron
tambi\'en  una versi\'on  sim\'etrica,  que veremos  m\'as adelante.   \modif{Se
  notara de que, $\Dkl[Q]{P} = -  \int_{\Rset^d} q(x) \, \log(q(x)) \, d\mu(x) +
  \int_{\Rset^d} p(x) \,  \log(q(x)) \, d\mu(x)$. El primer  termino es nada mas
  que   la  entrop\'ia   de  $q$,   que   se  puede   ver  como   distribuci\'on
  ``presupuesta''. Menos el secundo termino se interpreta como el promedio de la
  incerteza elemental  $\log(q)$ con respeto  a la distribuci\'on  de referencia
  (``verdadera''),  a veces  llamado {\it  entrop\'ia cruzada}.   Por  eso, esta
  divergencia es  una entrop\'ia  relativamente a la  distribuci\'on $p$.  En la
  misma  linea,   se  puede  inmediatamente  ver  de   la  definici\'on  general
  Def.~\ref{Def:SZ:ShanonMu},  pagina~\pageref{Def:SZ:ShanonMu}, que $\Dkl[Q]{P}
  \equiv -H\left( \frac{dQ}{dP} \right)$ \ con respeto a la medida $\mu = P$.}
%Esta medida puede  ser vista tambi\'en como una  entrop\'ia de la distribuci\'on
%$q$, relativamente  a una distribuci\'on de  referencia $p$.
...
\begin{lema}[Positividad de la entrop\'ia relativa]
  %
  \[
  \modif{\Dkl[Q]{P} \ge 0 \quad \mbox{con igualdad ssi} \quad P = Q}. %\quad (c.s.)
  \]
  %
...
\begin{proof}
  ...
  $X$ \ de \modif{medida de probabilidad $P$.  Se escribe la entrop\'ia relativa
    \  $\Dkl[Q]{P} =  \Esp\left[ \frac{dQ}{dP}(X)  \log  \left( \frac{dQ}{dP}(X)
      \right) \right]$.  Sea  \ $Y = \frac{dQ}{dP}(X)$} y $\phi(u)  = u \log u$,
  funci\'on   estrictamente   convexa.    Entonces   \   $\modif{\Dkl[Q]{P}}   =
  \Esp[\phi(Y)] \ge  \phi(\Esp[Y])$.  \  Pero \ \modif{$\displaystyle  \Esp[Y] =
    \Esp\left[  \frac{dQ}{dP}(X) \right]  =  \int_{\Rset^d} \frac{dQ}{dP}(x)  \,
    dP(x)  =  1$  seg\'un  el  lema~\ref{Lem:RelacionIntegracionDerivadasRadon},
    pagina~\pageref{Lem:RelacionIntegracionDerivadasRadon}.   \  Se   cierra  la
    prueba con el hecho de que} \  $\phi(1) = 0$ El caso de igualdad apareciendo
  si y solamente si \  $Y$ \ es determinista, es decir \modif{$\frac{dP}{dQ}(X)$
...
%, \ \ie \ $p = q \quad (c.s.)$ \ porque ambas suman a uno.
\end{proof}

Esta propiedad
%, v\'alida en el  caso discreto como continuo, 
...
\modif{(obviamente $P_{X,Y} \ll  P_X P_Y$). Con este enfoque,  no se necesita de
  que \ $(X,Y)$ \ sea discreta o continua admitiendo una densidad}.
%
\begin{propiedades}
...
\begin{enumerate}
\item \modif{
%A  pesar de que la forma  diferencial de 
$\Dkl[q]{p}$}
% depende  de los estados,
queda invariante  bajo una  misma transformaci\'on biyectiva  sobre ambos  $p$ y
$q$. \modif{Es trivial en el caso  discreto y si no s}e prueba sencillamente por
un cambio de variables en la \modif{forma} integral.
%
\item $\Dkl{}$ es convexa con respecto al par $(P,Q)$ en el sentido de que, para
  $\pi_i \ge 0, \: \sum_i \pi_i = 1$, , \modif{y dos conjuntos $\{ P_{(i)} \}_i,
    \:  \{ Q_{(i)} \}_i$  de medidas  de probabilidades  tales que  $Q_{(i)} \ll
    P_{(i)}$,}
  %
  \[
...
\item Para $Q$ fijo, $\Dkl[Q]{P}$ es convexa con respecto a $P$ en el sentido de
  que, para $\pi_i \ge 0, \: \sum_i \pi_i = 1$, \modif{y un conjunto $\{ P_{(i)}
    \}_i$ de medidas de probabilidades tales que $Q \ll P_{(i)}$,}
  %
  \[
